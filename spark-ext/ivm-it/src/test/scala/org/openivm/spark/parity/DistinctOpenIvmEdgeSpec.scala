...
/** 1:1 ScalaTest port of openivm `test/sql/distinct.test` — join / union /
  ...
  */
class DistinctOpenIvmEdgeSpec extends AnyFunSpec with Matchers with BeforeAndAfterAll {

  private val warehouseDir: String = {
    val d = new File(s"target/test-warehouse-distinct-openivm-edge-${UUID.randomUUID().toString.take(8)}")
    d.mkdirs()
    d.getAbsolutePath
  }
  ...
  override def beforeAll(): Unit = {
    ...
    spark = SparkSession
      .builder()
      .master("local[1]")
      .appName("openivm-spark-DistinctOpenIvmEdgeSpec")
      .config(
