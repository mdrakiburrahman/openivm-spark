...
/** Parity port of `openivm/test/sql/concurrency.test`.
  ...
  */
class ConcurrencySpec extends AnyFunSpec with Matchers with BeforeAndAfterAll {

  private val warehouseDir: String = {
    val d = new File(s"target/test-warehouse-conc-${UUID.randomUUID().toString.take(8)}")
    d.mkdirs()
    d.getAbsolutePath
  }
  ...
  override def beforeAll(): Unit = {
    ...
    spark = SparkSession
      .builder()
      .master("local[2]")
      .appName("openivm-spark-ConcurrencySpec")
      .config(
  ...
  /** Run a body that may legitimately encounter an OCC conflict / refresh
    ...
    */
  private def silentlyTry(body: => Unit, errSink: AtomicReference[Throwable]): Unit =
    try body
    catch {
      case t: Throwable =>
  ...
  private def runAll(parallelism: Int, timeout: Duration)(tasks: Seq[() => Unit]): Unit = {
  ...
  describe("(1) Reads coexist with writer's INSERT+REFRESH on the same aggregate MV") {
    it("MV is correct after the writer finishes 5 INSERT/REFRESH cycles") {
      spark.sql("CREATE TABLE IF NOT EXISTS sales_c1(region STRING, amount INT) USING DELTA")
      val seed = (1 to 200).map(i => s"('region_${i % 10}', $i)").mkString(", ")
      spark.sql(s"INSERT INTO sales_c1 VALUES $seed")
      ...
      val writer: () => Unit = () =>
        for (k <- 0 until 5) {
          ...
          silentlyTry(spark.sql(s"INSERT INTO sales_c1 VALUES $rows"), errSink)
  ...
  describe("(2) Two threads call REFRESH on the SAME MV — Delta OCC + idempotency") {
    it("after both threads finish, MV is bag-equal to the live view body") {
      spark.sql("CREATE TABLE IF NOT EXISTS items_c2(id INT, val INT) USING DELTA")
      val seed = (1 to 100).map(i => s"($i, ${i * 10})").mkString(", ")
      spark.sql(s"INSERT INTO items_c2 VALUES $seed")
      ...
      spark.sql(s"INSERT INTO items_c2 VALUES $seed2")
  ...
  describe("(3) Projection MV — one writer + multiple readers") {
    it("multi-thread reads succeed while writer drives INSERT+REFRESH cycles") {
      ...
      spark.sql(s"INSERT INTO employees_c3 VALUES $seed")

      spark.sql("CREATE MATERIALIZED VIEW mv_emp_c3 AS SELECT id, name FROM employees_c3")
      ...
      val writer: () => Unit = () =>
        for (k <- 0 until 5) {
          silentlyTry(
            spark.sql(s"INSERT INTO employees_c3 VALUES (${1000 + k}, 'new_${1000 + k}', 'dept_0')"),
            errSink
          )
  ...
  describe("(4) DROP one MV while refreshing another — both succeed independently") {
    it("after DROP, the target is gone; the kept MV stays correct") {
      ...
      spark.sql("CREATE TABLE IF NOT EXISTS t4a_c4(id INT, val INT) USING DELTA")
      spark.sql("CREATE TABLE IF NOT EXISTS t4b_c4(id INT, val INT) USING DELTA")
      spark.sql(s"INSERT INTO t4a_c4 VALUES ${(1 to 100).map(i => s"($i, $i)").mkString(", ")}")
      spark.sql(s"INSERT INTO t4b_c4 VALUES ${(1 to 100).map(i => s"($i, $i)").mkString(", ")}")
      ...
      val dropper: () => Unit = () => silentlyTry(spark.sql("DROP MATERIALIZED VIEW mv_drop_target_c4"): Unit, errSink)
  ...
  describe("(5) Stress — conflicting INSERTs from 3 threads, single REFRESH after") {
    it("after all writers finish and a single REFRESH runs, MV is bag-equal") {
      spark.sql("CREATE TABLE IF NOT EXISTS stress_c5(id INT, grp STRING, val INT) USING DELTA")
      val seed =
        (1 to 100).map(i => s"($i, 'g${i % 5}', ${i * 10})").mkString(", ")
      spark.sql(s"INSERT INTO stress_c5 VALUES $seed")
      ...
      def mkWriter(grp: String, idBase: Int): () => Unit = () =>
        for (k <- 0 until 3) {
          val sql = s"INSERT INTO stress_c5 VALUES (${idBase + k}, '$grp', ${100 + k})"
          // Retry on OCC: appends to the same Delta table from different
          // threads can throw a ConcurrentAppendException; the openivm-spark
          // idempotency guarantee (consumed_by set-merge) protects the final
          // state, not individual mid-flight writes, so we retry.
          var attempt   = 0
          var succeeded = false
          ...
          if (!succeeded) silentlyTry(spark.sql(sql), errSink)
        }
  ...
  describe("(6) consumed_by idempotency under MANY sequential REFRESH calls (single thread)") {
    it("twenty sequential REFRESHes after one INSERT produce the correct MV exactly once") {
      spark.sql("CREATE TABLE IF NOT EXISTS idem_c6(id INT, val INT) USING DELTA")
      spark.sql(s"INSERT INTO idem_c6 VALUES ${(1 to 50).map(i => s"($i, $i)").mkString(", ")}")

      spark.sql("CREATE MATERIALIZED VIEW mv_idem_c6 AS SELECT id, val FROM idem_c6")
      spark.sql(
