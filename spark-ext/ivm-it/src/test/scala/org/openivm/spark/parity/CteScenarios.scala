...
/** P5.cte — Non-recursive Common Table Expression coverage.
  ...
  */
abstract class CteScenarios extends IvmParitySpecBase("cte") {
  ...
  describe("CTE-1: CTE wrapping AGGREGATE_GROUP → RefreshType 0") {
    ...
    it("handles NULL amounts correctly in the CTE predicate") {
      ...
      sql("INSERT INTO sales_c1n VALUES ('east', 200), ('west', NULL), ('north', 50)")
      sql(
      ...
      sql("INSERT INTO sales_c1n VALUES ('south', NULL), ('east', 300)")
      refreshMv("mv_c1n")
      assertMvCorrect(
        "mv_c1n",
  ...
  describe("CTE-2: CTE wrapping SIMPLE_PROJECTION → RefreshType 2") {
    ...
    it("classifies as SimpleProjection and refreshes incrementally") {
      ...
      sql("INSERT INTO users_c2 VALUES (1, 'Alice', 30), (2, 'Bob', 16), (3, 'Carol', 25)")
      sql(
      ...
      sql("INSERT INTO users_c2 VALUES (4, 'Dave', 17), (5, 'Eve', 22)")
      refreshMv("mv_c2")
      assertMvCorrect(
        "mv_c2",
      ...
      sql("DELETE FROM users_c2 WHERE name = 'Bob'")
      refreshMv("mv_c2")
      assertMvCorrect(
        "mv_c2",
        "WITH adults AS (SELECT * FROM users_c2 WHERE age >= 18) SELECT name, age FROM adults"
      ...
      sql("UPDATE users_c2 SET age = 18 WHERE name = 'Dave'")
      refreshMv("mv_c2")
      assertMvCorrect(
        "mv_c2",
        "WITH adults AS (SELECT * FROM users_c2 WHERE age >= 18) SELECT name, age FROM adults"
      ...
      // Batched mix
      sql("INSERT INTO users_c2 VALUES (6, 'Frank', 40)")
      sql("DELETE FROM users_c2 WHERE name = 'Carol'")
      sql("UPDATE users_c2 SET age = 17 WHERE name = 'Eve'")
      refreshMv("mv_c2")
      assertMvCorrect(
        "mv_c2",
        "WITH adults AS (SELECT * FROM users_c2 WHERE age >= 18) SELECT name, age FROM adults"
    ...
    it("propagates NULL age correctly — NULLs fail WHERE age >= 18") {
      ...
      sql("INSERT INTO users_c2n VALUES (1, 'Alice', 30), (2, 'Bob', NULL)")
      sql(
      ...
      sql("INSERT INTO users_c2n VALUES (3, 'Carol', NULL), (4, 'Dave', 20)")
      refreshMv("mv_c2n")
      assertMvCorrect(
        "mv_c2n",
  ...
  describe("CTE-3: Multiple CTEs joined together") {
    ...
    it("falls back to full-recompute refresh for a two-CTE JOIN") {
      ...
      sql("INSERT INTO users_c3 VALUES (4, 'Dave', 22)")
      sql("INSERT INTO activity_c3 VALUES (4, 1)")
      refreshMv("mv_c3")
      assertMvCorrect(
        "mv_c3",
        """WITH adults AS (SELECT id, name FROM users_c3 WHERE age >= 18),
      ...
      sql("DELETE FROM activity_c3 WHERE id = 2")
      refreshMv("mv_c3")
      assertMvCorrect(
        "mv_c3",
        """WITH adults AS (SELECT id, name FROM users_c3 WHERE age >= 18),
      ...
      sql("UPDATE activity_c3 SET last_seen_days_ago = 5 WHERE id = 1")
      refreshMv("mv_c3")
      assertMvCorrect(
        "mv_c3",
        """WITH adults AS (SELECT id, name FROM users_c3 WHERE age >= 18),
      ...
      sql("INSERT INTO users_c3 VALUES (5, 'Eve', 19)")
      sql("INSERT INTO activity_c3 VALUES (5, 4)")
      sql("DELETE FROM users_c3 WHERE name = 'Carol'")
      sql("UPDATE activity_c3 SET last_seen_days_ago = 20 WHERE id = 3")
      refreshMv("mv_c3")
      assertMvCorrect(
        "mv_c3",
        """WITH adults AS (SELECT id, name FROM users_c3 WHERE age >= 18),
  ...
  describe("CTE-4: CTE referenced in multiple aggregate expressions") {
    ...
    it("propagates NULL amounts in aggregation correctly") {
      sql("CREATE TABLE IF NOT EXISTS sales_c4n(region STRING, amount INT) USING DELTA")
      sql("INSERT INTO sales_c4n VALUES ('east', 100), ('east', NULL), ('west', 200)")
      sql(
      ...
      sql("INSERT INTO sales_c4n VALUES ('east', NULL), ('north', 50)")
      refreshMv("mv_c4n")
      assertMvCorrect(
        "mv_c4n",
  ...
  describe("CTE-5: Nested CTEs — t2 reads from t1") {
    ...
    it("double-layer CTE reduces to the same plan shape and refreshes incrementally") {
      sql("CREATE TABLE IF NOT EXISTS sales_c5(region STRING, amount INT) USING DELTA")
      sql(
      ...
      sql("DELETE FROM sales_c5 WHERE region = 'north' AND amount = 200")
      refreshMv("mv_c5")
      assertMvCorrect(
        "mv_c5",
        """WITH t1 AS (SELECT * FROM sales_c5 WHERE amount > 0),
      ...
      sql("UPDATE sales_c5 SET amount = 120 WHERE region = 'south'")
      refreshMv("mv_c5")
      assertMvCorrect(
        "mv_c5",
        """WITH t1 AS (SELECT * FROM sales_c5 WHERE amount > 0),
      ...
      // Batched mix
      sql("INSERT INTO sales_c5 VALUES ('east', 700)")
      sql("DELETE FROM sales_c5 WHERE region = 'west' AND amount = 10")
      sql("UPDATE sales_c5 SET amount = 1 WHERE region = 'east' AND amount = 50")
      refreshMv("mv_c5")
      assertMvCorrect(
        "mv_c5",
        """WITH t1 AS (SELECT * FROM sales_c5 WHERE amount > 0),
  ...
  describe("CTE-6: CTE feeding a DISTINCT → RefreshType 8") {
    ...
    it("classifies as DistinctIncremental and refreshes incrementally") {
      sql("CREATE TABLE IF NOT EXISTS sales_c6(region STRING, amount INT) USING DELTA")
      sql(
      ...
      sql("INSERT INTO sales_c6 VALUES ('east', 300), ('north', 10)")
      refreshMv("mv_c6")
      assertMvCorrect(
        "mv_c6",
      ...
      sql("DELETE FROM sales_c6 WHERE region = 'west'")
      refreshMv("mv_c6")
      assertMvCorrect(
        "mv_c6",
        "WITH t AS (SELECT region FROM sales_c6) SELECT DISTINCT region FROM t"
      ...
      sql("UPDATE sales_c6 SET region = 'south' WHERE region = 'north'")
      refreshMv("mv_c6")
      assertMvCorrect(
        "mv_c6",
        "WITH t AS (SELECT region FROM sales_c6) SELECT DISTINCT region FROM t"
      ...
      sql("INSERT INTO sales_c6 VALUES ('west', 5), ('central', 80)")
      sql("DELETE FROM sales_c6 WHERE region = 'east' AND amount = 100")
      refreshMv("mv_c6")
      assertMvCorrect(
        "mv_c6",
        "WITH t AS (SELECT region FROM sales_c6) SELECT DISTINCT region FROM t"
    ...
    it("handles NULL region values in DISTINCT CTE") {
      sql("CREATE TABLE IF NOT EXISTS sales_c6n(region STRING, amount INT) USING DELTA")
      sql("INSERT INTO sales_c6n VALUES ('east', 100), (NULL, 50), ('east', 200)")
      sql(
        """CREATE MATERIALIZED VIEW mv_c6n AS
          |WITH t AS (SELECT region FROM sales_c6n)
          |SELECT DISTINCT region FROM t""".stripMargin
      )
      sql("INSERT INTO sales_c6n VALUES (NULL, 999), ('west', 1)")
      refreshMv("mv_c6n")
      assertMvCorrect(
        "mv_c6n",
