...
/** P6f — 1:1 ScalaTest port of openivm's `test/sql/semi_anti_join.test`.
  ...
  */
class SemiAntiJoinSpec extends AnyFunSpec with Matchers with BeforeAndAfterAll {

  private val warehouseDir: String = {
    val d = new File(s"target/test-warehouse-saj-${UUID.randomUUID().toString.take(8)}")
    d.mkdirs()
    d.getAbsolutePath
  }
  ...
  override def beforeAll(): Unit = {
    ...
    spark = SparkSession
      .builder()
      .master("local[1]")
      .appName("openivm-spark-SemiAntiJoinSpec")
      .config(
  ...
  /** Bidirectional `EXCEPT ALL` equivalence check.  Projects the MV to the
    * same column set as `expectedSql` so that any hidden openivm bookkeeping
    * columns are excluded from the comparison.
    */
  ...
  describe("(1) SEMI JOIN with non-equi predicate — batched DML across L+R") {
    it("refresh tracks per-left-tuple match count under abs(l.x - r.y) <= 2") {
      spark.sql("CREATE TABLE IF NOT EXISTS saj_l_1(id INT, x INT) USING DELTA")
      spark.sql("CREATE TABLE IF NOT EXISTS saj_r_1(rid INT, y INT) USING DELTA")
      spark.sql("INSERT INTO saj_l_1 VALUES (1, 10), (1, 10), (2, 20), (3, 30), (4, 40)")
      spark.sql("INSERT INTO saj_r_1 VALUES (100, 8), (101, 100), (102, 41)")

      val viewBody =
        "SELECT l.id, l.x FROM saj_l_1 l " +
          "LEFT SEMI JOIN saj_r_1 r ON abs(l.x - r.y) <= 2"
      spark.sql(s"CREATE MATERIALIZED VIEW mv_saj_semi_1 AS $viewBody")
      ...
      spark.sql("INSERT INTO saj_r_1 VALUES (103, 21), (104, 32)")
      spark.sql("DELETE FROM saj_r_1 WHERE rid = 102")
      spark.sql("INSERT INTO saj_l_1 VALUES (3, 30)")
      spark.sql("DELETE FROM saj_l_1 WHERE id = 1 AND x = 10")
      spark.sql("INSERT INTO saj_l_1 VALUES (1, 10)")
      ...
      spark.sql("DELETE FROM saj_r_1 WHERE rid = 104")
      refreshMv("mv_saj_semi_1")
      assertMvCorrect("mv_saj_semi_1", viewBody)
    }
  }
  ...
  describe("(2) ANTI JOIN under GROUP BY — batched DML including UPDATE") {
    it("aggregate over ANTI-join survival set tracks INSERTs/DELETEs/UPDATEs") {
      spark.sql("CREATE TABLE IF NOT EXISTS saj_ag_l_2(id INT, grp INT, x INT) USING DELTA")
      spark.sql("CREATE TABLE IF NOT EXISTS saj_ag_r_2(rid INT, y INT) USING DELTA")
      spark.sql(
        "INSERT INTO saj_ag_l_2 VALUES (1, 1, 10), (2, 1, 20), (3, 2, 30), (4, 2, 40), (5, 3, 50)"
      )
      spark.sql("INSERT INTO saj_ag_r_2 VALUES (1, 10), (2, 41)")
      ...
      val viewBody =
        "SELECT l.grp, COUNT(*) AS cnt, SUM(l.x) AS total " +
          ...
          "GROUP BY l.grp"
      spark.sql(s"CREATE MATERIALIZED VIEW mv_saj_ag_anti_2 AS $viewBody")
      assertMvCorrect("mv_saj_ag_anti_2", viewBody)

      spark.sql("INSERT INTO saj_ag_r_2 VALUES (3, 20), (4, 50)")
      spark.sql("DELETE FROM saj_ag_r_2 WHERE rid = 2")
      spark.sql("UPDATE saj_ag_l_2 SET x = 11 WHERE id = 3")
      spark.sql("INSERT INTO saj_ag_l_2 VALUES (6, 3, 60), (7, 3, 20)")
  ...
  describe("(3) ANTI JOIN after JOIN chain — left input is a subplan, not a base table") {
    it("ANTI JOIN over a 3-table join chain handles batched DML across all 3 tables") {
      ...
      spark.sql("INSERT INTO saj_cj_customers_3 VALUES (10, 'north'), (20, 'south'), (30, 'west')")
      spark.sql("INSERT INTO saj_cj_blocked_3 VALUES (1, 'north', 100), (2, 'west', 10)")
      ...
      spark.sql(s"CREATE MATERIALIZED VIEW mv_saj_chain_anti_3 AS $viewBody")
      assertMvCorrect("mv_saj_chain_anti_3", viewBody)

      spark.sql("INSERT INTO saj_cj_blocked_3 VALUES (3, 'south', 50)")
      spark.sql("DELETE FROM saj_cj_blocked_3 WHERE bid = 2")
      spark.sql("UPDATE saj_cj_orders_3 SET amount = 105 WHERE oid = 1")
      spark.sql("INSERT INTO saj_cj_orders_3 VALUES (5, 30, 20)")
  ...
  describe("(4) SEMI / ANTI with subquery RHS — UPDATE on RHS flips the filter") {
    it("subquery-filtered RHS; UPDATEs to the filter propagate correctly") {
      spark.sql("CREATE TABLE IF NOT EXISTS saj_sq_l_4(id INT, x INT) USING DELTA")
      spark.sql(
        "CREATE TABLE IF NOT EXISTS saj_sq_r_4(rid INT, y INT, active BOOLEAN) USING DELTA"
      )
      spark.sql("INSERT INTO saj_sq_l_4 VALUES (1, 10), (2, 20), (3, 30), (4, 40)")
      spark.sql("INSERT INTO saj_sq_r_4 VALUES (1, 10, true), (2, 20, false), (3, 99, true)")
      ...
      spark.sql(s"CREATE MATERIALIZED VIEW mv_saj_subq_semi_4 AS $semiBody")
      spark.sql(s"CREATE MATERIALIZED VIEW mv_saj_subq_anti_4 AS $antiBody")
      assertMvCorrect("mv_saj_subq_semi_4", semiBody)
      assertMvCorrect("mv_saj_subq_anti_4", antiBody)

      spark.sql("UPDATE saj_sq_r_4 SET active = true WHERE rid = 2")
      spark.sql("INSERT INTO saj_sq_r_4 VALUES (4, 40, true)")
      spark.sql("DELETE FROM saj_sq_l_4 WHERE id = 1")
      spark.sql("INSERT INTO saj_sq_l_4 VALUES (5, 50)")
  ...
  describe("(5) NOT IN with CAST(x AS BIGINT) in output and predicate") {
    it("computed BIGINT cast columns are preserved across refresh") {
      spark.sql("CREATE TABLE IF NOT EXISTS saj_inc_l_5(id INT, x INT) USING DELTA")
      spark.sql("CREATE TABLE IF NOT EXISTS saj_inc_r_5(rid INT, y BIGINT) USING DELTA")
      spark.sql("INSERT INTO saj_inc_l_5 VALUES (1, 10), (2, 20), (3, 30), (4, 40)")
      spark.sql("INSERT INTO saj_inc_r_5 VALUES (1, 20), (2, 99)")

      val viewBody =
        "SELECT CAST(x AS BIGINT) AS item_id " +
          "FROM saj_inc_l_5 " +
          "WHERE CAST(x AS BIGINT) NOT IN (SELECT CAST(y AS BIGINT) FROM saj_inc_r_5)"
      spark.sql(s"CREATE MATERIALIZED VIEW mv_saj_not_in_cast_5 AS $viewBody")
      assertMvCorrect("mv_saj_not_in_cast_5", viewBody)

      spark.sql("INSERT INTO saj_inc_l_5 VALUES (5, 50), (6, 20)")
      spark.sql("DELETE FROM saj_inc_r_5 WHERE rid = 1")
      spark.sql("INSERT INTO saj_inc_r_5 VALUES (3, 30), (4, 50)")
    ...
    it("EXISTS/NOT EXISTS lower to SEMI/ANTI; outer l.keep filter is preserved") {
      ...
      val antiBody =
        "SELECT l.id, l.x FROM saj_ex_l_6 l " +
          ...
          "SELECT 1 FROM saj_ex_r_6 r WHERE r.active AND abs(l.x - r.y) <= 2) " +
          "AND l.keep"
      spark.sql(s"CREATE MATERIALIZED VIEW mv_saj_exists_semi_6 AS $semiBody")
      spark.sql(s"CREATE MATERIALIZED VIEW mv_saj_exists_anti_6 AS $antiBody")
      assertMvCorrect("mv_saj_exists_semi_6", semiBody)
      assertMvCorrect("mv_saj_exists_anti_6", antiBody)

      spark.sql("UPDATE saj_ex_r_6 SET active = true WHERE rid = 11")
      spark.sql("INSERT INTO saj_ex_r_6 VALUES (13, 41, true), (14, 30, true)")
      spark.sql("DELETE FROM saj_ex_r_6 WHERE rid = 10")
      spark.sql("UPDATE saj_ex_l_6 SET keep = true WHERE id = 3")
      spark.sql("INSERT INTO saj_ex_l_6 VALUES (5, 50, true), (6, 40, true)")
    ...
    it("aggregate over NOT EXISTS and UNION ALL of EXISTS+NOT EXISTS stay correct") {
      ...
      val unionBody =
        "SELECT l.id, l.x FROM saj_co_l_7 l " +
          ...
          "SELECT 1 FROM saj_co_r_7 r WHERE r.active AND abs(l.x - r.y) <= 1) " +
          "AND l.grp = 3"
      spark.sql(s"CREATE MATERIALIZED VIEW mv_saj_exists_group_7 AS $groupBody")
      spark.sql(s"CREATE MATERIALIZED VIEW mv_saj_exists_union_7 AS $unionBody")
      assertMvCorrect("mv_saj_exists_group_7", groupBody)
      assertMvCorrect("mv_saj_exists_union_7", unionBody)

      spark.sql("UPDATE saj_co_r_7 SET active = true WHERE rid = 12")
      spark.sql("INSERT INTO saj_co_r_7 VALUES (13, 31, true), (14, 50, true)")
      spark.sql("DELETE FROM saj_co_r_7 WHERE rid = 10")
      spark.sql("UPDATE saj_co_l_7 SET x = 18 WHERE id = 3")
      spark.sql("INSERT INTO saj_co_l_7 VALUES (6, 3, 60), (7, 1, 11)")
  ...
  describe("(8) ANTI JOIN basic — initial verify + second-round batched DML") {
    it("ANTI survives multi-round mixed DML on both sides") {
      spark.sql("CREATE TABLE IF NOT EXISTS saj_aaj_l_8(id INT, x INT) USING DELTA")
      spark.sql("CREATE TABLE IF NOT EXISTS saj_aaj_r_8(rid INT, y INT) USING DELTA")
      spark.sql(
        "INSERT INTO saj_aaj_l_8 VALUES (1, 10), (2, 20), (3, 30), (3, 30), (4, 40)"
      )
      spark.sql("INSERT INTO saj_aaj_r_8 VALUES (200, 8), (201, 41)")

      val viewBody =
        "SELECT l.id, l.x FROM saj_aaj_l_8 l " +
          "LEFT ANTI JOIN saj_aaj_r_8 r ON abs(l.x - r.y) <= 2"
      spark.sql(s"CREATE MATERIALIZED VIEW mv_saj_anti_8 AS $viewBody")
      ...
      spark.sql("INSERT INTO saj_aaj_r_8 VALUES (202, 20), (203, 29)")
      spark.sql("DELETE FROM saj_aaj_r_8 WHERE rid = 201")
      spark.sql("INSERT INTO saj_aaj_l_8 VALUES (5, 50)")
      spark.sql("DELETE FROM saj_aaj_l_8 WHERE id = 1")
  ...
  describe("(9) Duplicate RHS matches — SEMI keeps row until last RHS match deleted") {
    it("partial RHS retraction does not drop the SEMI row; full retraction does") {
      spark.sql("CREATE TABLE IF NOT EXISTS saj_dup_l_9(id INT, x INT) USING DELTA")
      spark.sql("CREATE TABLE IF NOT EXISTS saj_dup_r_9(rid INT, y INT) USING DELTA")
      spark.sql("INSERT INTO saj_dup_l_9 VALUES (1, 10), (2, 20), (3, 30)")
      spark.sql("INSERT INTO saj_dup_r_9 VALUES (10, 10), (11, 10), (20, 20)")
      ...
      spark.sql(s"CREATE MATERIALIZED VIEW mv_saj_dup_semi_9 AS $semiBody")
      spark.sql(s"CREATE MATERIALIZED VIEW mv_saj_dup_anti_9 AS $antiBody")

      // Remove one of two duplicate matches for x=10 — SEMI/ANTI must not flip.
      spark.sql("DELETE FROM saj_dup_r_9 WHERE rid = 10")
      ...
      spark.sql("DELETE FROM saj_dup_r_9 WHERE rid = 11")
      refreshMv("mv_saj_dup_semi_9")
      refreshMv("mv_saj_dup_anti_9")
      assertMvCorrect("mv_saj_dup_semi_9", semiBody)
      assertMvCorrect("mv_saj_dup_anti_9", antiBody)
  ...
  describe("(10) UPDATEs as delete+insert; NULL-safe predicate IS NOT DISTINCT FROM") {
    it("UPDATE / DELETE / INSERT across L and R; two refresh rounds; NULLs match NULLs") {
      spark.sql("CREATE TABLE IF NOT EXISTS saj_upd_l_10(id INT, x INT) USING DELTA")
      spark.sql("CREATE TABLE IF NOT EXISTS saj_upd_r_10(rid INT, y INT) USING DELTA")
      spark.sql(
      ...
      spark.sql(s"CREATE MATERIALIZED VIEW mv_saj_upd_semi_10 AS $semiBody")
      spark.sql(s"CREATE MATERIALIZED VIEW mv_saj_upd_anti_10 AS $antiBody")

      // Round 1: left-identity UPDATE, right-match UPDATE, right DELETE
      // (removing the NULL match), insert two duplicate NULL left rows.
      spark.sql("UPDATE saj_upd_l_10 SET x = 99 WHERE id = 3")
      spark.sql("UPDATE saj_upd_r_10 SET y = 40 WHERE rid = 102")
      spark.sql("DELETE FROM saj_upd_r_10 WHERE rid = 101")
      spark.sql(
      ...
      spark.sql("DELETE FROM saj_upd_l_10 WHERE id = 5")
      spark.sql("INSERT INTO saj_upd_r_10 VALUES (103, CAST(NULL AS INT))")
      spark.sql("INSERT INTO saj_upd_l_10 VALUES (6, CAST(NULL AS INT))")
      refreshMv("mv_saj_upd_semi_10")
      refreshMv("mv_saj_upd_anti_10")
      assertMvCorrect("mv_saj_upd_semi_10", semiBody)
      assertMvCorrect("mv_saj_upd_anti_10", antiBody)
  ...
  describe("(11) Stacked FILTER above SEMI / ANTI with identity-changing UPDATE") {
    it("outer WHERE l.id >= 2 is preserved through refresh under L+R DML") {
      spark.sql("CREATE TABLE IF NOT EXISTS saj_stk_l_11(id INT, x INT) USING DELTA")
      spark.sql("CREATE TABLE IF NOT EXISTS saj_stk_r_11(rid INT, y INT) USING DELTA")
      spark.sql(
        "INSERT INTO saj_stk_l_11 VALUES (1, 10), (2, 20), (3, 30), (4, 40), (5, 50)"
      )
      spark.sql("INSERT INTO saj_stk_r_11 VALUES (1, 10), (2, 21), (3, 70)")
      ...
      spark.sql(s"CREATE MATERIALIZED VIEW mv_saj_stack_semi_11 AS $semiBody")
      spark.sql(s"CREATE MATERIALIZED VIEW mv_saj_stack_anti_11 AS $antiBody")
      assertMvCorrect("mv_saj_stack_semi_11", semiBody)
      assertMvCorrect("mv_saj_stack_anti_11", antiBody)

      spark.sql("INSERT INTO saj_stk_r_11 VALUES (4, 40), (5, 51)")
      spark.sql("UPDATE saj_stk_l_11 SET id = 6 WHERE id = 1")
      spark.sql("DELETE FROM saj_stk_r_11 WHERE rid = 2")
      spark.sql("INSERT INTO saj_stk_l_11 VALUES (7, 70)")
