...
/** P5.rt0-minmax — Parity tests for grouped MIN/MAX (RefreshType 0 / 6 with `has_minmax`).
  ...
  * shape covered here.
  */
abstract class AggregateMinMaxScenarios extends IvmParitySpecBase("aggregate-min-max") {
  ...
  describe("(1) GROUP BY region, MIN(amount) — basic grouped MIN over insert-only delta") {
    it("classifier records AggregateGroup; MV reflects per-group MIN after INSERT") {
      sql("CREATE TABLE IF NOT EXISTS mm_sales_1(region STRING, amount INT) USING DELTA")
      sql("INSERT INTO mm_sales_1 VALUES ('east', 100), ('west', 50), ('east', 200)")
      sql(
      ...
      sql("INSERT INTO mm_sales_1 VALUES ('east', 75), ('south', 30)")
      refreshMv("mv_mm_1")
      assertMvCorrect(
        "mv_mm_1",
        "SELECT region, MIN(amount) AS lo FROM mm_sales_1 GROUP BY region"
  ...
  describe("(2) GROUP BY region, MAX(amount) — basic grouped MAX over insert-only delta") {
    it("classifier records AggregateGroup; MV reflects per-group MAX after INSERT") {
      sql("CREATE TABLE IF NOT EXISTS mm_sales_2(region STRING, amount INT) USING DELTA")
      sql("INSERT INTO mm_sales_2 VALUES ('east', 100), ('west', 50)")
      sql(
      ...
      sql("INSERT INTO mm_sales_2 VALUES ('east', 250), ('west', 30)")
      refreshMv("mv_mm_2")
      assertMvCorrect(
        "mv_mm_2",
        "SELECT region, MAX(amount) AS hi FROM mm_sales_2 GROUP BY region"
  ...
  describe("(3) GROUP BY region, MIN(amount), MAX(amount)") {
    it("both extremes maintained in a single MV") {
      sql("CREATE TABLE IF NOT EXISTS mm_sales_3(region STRING, amount INT) USING DELTA")
      sql("INSERT INTO mm_sales_3 VALUES ('east', 100), ('east', 200), ('west', 50)")
      sql(
      ...
      sql("INSERT INTO mm_sales_3 VALUES ('east', 75), ('west', 60), ('south', 33)")
      refreshMv("mv_mm_3")
      assertMvCorrect(
        "mv_mm_3",
  ...
  describe("(4) GROUP BY region, MIN(amount), SUM(amount) — mixed MIN with SUM") {
    it("MIN (non-additive) and SUM (additive) co-exist in one MV") {
      sql("CREATE TABLE IF NOT EXISTS mm_sales_4(region STRING, amount INT) USING DELTA")
      sql("INSERT INTO mm_sales_4 VALUES ('east', 100), ('east', 200), ('west', 50)")
      sql(
      ...
      sql("INSERT INTO mm_sales_4 VALUES ('east', 75), ('west', 60)")
      refreshMv("mv_mm_4")
      assertMvCorrect(
        "mv_mm_4",
  ...
  describe("(5) INSERT a value below current MIN of an existing group") {
    it("MV's MIN drops to the newly inserted value") {
      sql("CREATE TABLE IF NOT EXISTS mm_sales_5(region STRING, amount INT) USING DELTA")
      sql("INSERT INTO mm_sales_5 VALUES ('east', 100), ('east', 200)")
      sql(
      ...
      sql("INSERT INTO mm_sales_5 VALUES ('east', 10)")
      refreshMv("mv_mm_5")
      assertMvCorrect(
        "mv_mm_5",
        "SELECT region, MIN(amount) AS lo FROM mm_sales_5 GROUP BY region"
  ...
  describe("(6) INSERT a value above current MIN — MIN unchanged, MAX may update") {
    it("MIN stays put; MAX picks up the new high") {
      sql("CREATE TABLE IF NOT EXISTS mm_sales_6(region STRING, amount INT) USING DELTA")
      sql("INSERT INTO mm_sales_6 VALUES ('east', 100), ('east', 200)")
      sql(
        "CREATE MATERIALIZED VIEW mv_mm_6 AS " +
          "SELECT region, MIN(amount) AS lo, MAX(amount) AS hi " +
          "FROM mm_sales_6 GROUP BY region"
      )
      sql("INSERT INTO mm_sales_6 VALUES ('east', 500)")
      refreshMv("mv_mm_6")
      assertMvCorrect(
        "mv_mm_6",
  ...
  describe("(7) DELETE the current MIN of a group — MV reflects next-min") {
    it("MIN shifts up to the next-smallest surviving value") {
      sql("CREATE TABLE IF NOT EXISTS mm_sales_7(region STRING, amount INT) USING DELTA")
      sql(
      ...
      sql("DELETE FROM mm_sales_7 WHERE region = 'east' AND amount = 100")
      refreshMv("mv_mm_7")
      assertMvCorrect(
        "mv_mm_7",
        "SELECT region, MIN(amount) AS lo FROM mm_sales_7 GROUP BY region"
  ...
  describe("(8) DELETE a non-MIN, non-MAX row — extremes unchanged") {
    it("MIN and MAX both unchanged after a middle-value DELETE") {
      sql("CREATE TABLE IF NOT EXISTS mm_sales_8(region STRING, amount INT) USING DELTA")
      sql(
      ...
      sql("DELETE FROM mm_sales_8 WHERE region = 'east' AND amount = 200")
      refreshMv("mv_mm_8")
      assertMvCorrect(
        "mv_mm_8",
  ...
  describe("(9) INSERT into a brand-new group — MV gains a new row") {
    it("a previously-absent group appears with the correct MIN/MAX") {
      sql("CREATE TABLE IF NOT EXISTS mm_sales_9(region STRING, amount INT) USING DELTA")
      sql("INSERT INTO mm_sales_9 VALUES ('east', 100), ('east', 200)")
      sql(
        "CREATE MATERIALIZED VIEW mv_mm_9 AS " +
          "SELECT region, MIN(amount) AS lo, MAX(amount) AS hi " +
          "FROM mm_sales_9 GROUP BY region"
      )
      sql("INSERT INTO mm_sales_9 VALUES ('north', 42), ('north', 99)")
      refreshMv("mv_mm_9")
      assertMvCorrect(
        "mv_mm_9",
  ...
  describe("(10) ARG_MIN fallback — Spark has no ARG_MIN; emulate via MIN_BY-style join") {
    it("`MIN_BY(val, key)` semantics via Spark-native min_by built-in") {
      sql("CREATE TABLE IF NOT EXISTS mm_orders_10(grp STRING, val STRING, k INT) USING DELTA")
      sql(
      ...
      sql("INSERT INTO mm_orders_10 VALUES ('A', 'apricot', 0)")
      refreshMv("mv_mm_10")
      assertMvCorrect(
        "mv_mm_10",
        "SELECT grp, MIN_BY(val, k) AS best FROM mm_orders_10 GROUP BY grp"
  ...
  describe("(11) Batched DML mixing INSERT + DELETE that changes MIN of a group") {
    it("single REFRESH reconciles a delta that crosses the current MIN") {
      sql("CREATE TABLE IF NOT EXISTS mm_sales_11(region STRING, amount INT) USING DELTA")
      sql(
      ...
      sql("DELETE FROM mm_sales_11 WHERE region = 'east' AND amount = 100")
      sql("INSERT INTO mm_sales_11 VALUES ('east', 25)")
      sql("DELETE FROM mm_sales_11 WHERE region = 'west' AND amount = 250")
      sql("INSERT INTO mm_sales_11 VALUES ('west', 300)")
      refreshMv("mv_mm_11")
      assertMvCorrect(
        "mv_mm_11",
  ...
  describe("(12) MIN/MAX ignore NULLs per SQL semantics") {
    it("groups containing NULLs report the min/max of the non-NULL values") {
      sql("CREATE TABLE IF NOT EXISTS mm_sales_12(region STRING, amount INT) USING DELTA")
      sql(
      ...
      sql("INSERT INTO mm_sales_12 VALUES ('east', NULL), ('south', NULL), ('south', 7)")
      sql("DELETE FROM mm_sales_12 WHERE region = 'west' AND amount IS NULL")
      refreshMv("mv_mm_12")
      assertMvCorrect(
        "mv_mm_12",
  ...
  describe("(13) Composite group keys (region, channel) with MIN and MAX") {
    it("MIN/MAX maintained correctly under a multi-column GROUP BY") {
      ...
      sql("DELETE FROM mm_sales_13 WHERE region = 'east' AND channel = 'web' AND amount = 100")
      refreshMv("mv_mm_13")
      assertMvCorrect(
        "mv_mm_13",
        "SELECT region, channel, MIN(amount) AS lo, MAX(amount) AS hi " +
