...
/** End-to-end TPC-DI parity spec.
  ...
  *     current batch are skipped at the spec level (no spark.sql() call;
  *     not even the no-op fast path inside RefreshMaterializedViewCommand).
  */
class TpcDiSpec extends AnyFunSpec with Matchers with BeforeAndAfterAll {
  ...
  private val warehouseDir: String = {
    val d = new File(s"/tmp/test-warehouse-tpcdi-${UUID.randomUUID().toString.take(8)}")
    d.mkdirs()
    d.getAbsolutePath
  }
  ...
  private val mvBodies: mutable.Map[String, String]          = mutable.Map.empty
  private val compileExceptions: mutable.Map[String, String] = mutable.Map.empty
  ...
  // spark.sql() calls so Spark can interleave their stages on the 16 cores.
  private val poolSize                      = sys.env.get("OPENIVM_TPCDI_PARALLELISM").map(_.toInt).getOrElse(8)
  ...
  override def beforeAll(): Unit = {
    ...
    Seq("bronze", "silver", "gold").foreach(s => spark.sql(s"CREATE DATABASE IF NOT EXISTS $s"))
  ...
  describe("TPC-DI mini-bench parity (49 MVs over ≤100-row CSV fixtures)") {
    ...
    it("batch-2 AND batch-3 wall-clock stay within the regression budget") {
      val budget = TpcDiSpec.PerfMaxRatio
      withClue(
  ...
  /** CREATE every MV in dependency-respecting order. Intentionally SERIAL
    ...
    */
  private def createAllMvs(): Unit = {
    topology.foreach { mvName =>
      val body = mvBodies(mvName)
      try {
        spark.sql(s"CREATE MATERIALIZED VIEW $mvName AS $body").collect()
      } catch {
        case NonFatal(e) =>
  ...
  /** REFRESH MVs in dependency-respecting parallel waves. MVs whose source
    ...
    */
  private def refreshAllMvs(batchName: String): Unit = {
    val activeSources = TpcDiSpec.SourcesAppendedPerBatch(batchName)
    ...
    waves.zipWithIndex.foreach { case (wave, idx) =>
      ...
      if (toRefresh.nonEmpty) {
        ...
        runWaveParallel(toRefresh) { mvName =>
          spark.sql(s"REFRESH MATERIALIZED VIEW $mvName").collect()
        }
  ...
  /** Compute the set of MVs whose transitive source-table dependencies do NOT
    ...
    */
  private def skipMvsForBatch(activeSources: Set[String]): Set[String] = {
    ...
    def transitive(mvName: String): Set[String] = cache.getOrElseUpdate(
      mvName, {
        ...
        val direct = TpcDiSpec.SourceTableRe.findAllIn(body).toSet
  ...
  /** Compute dependency-respecting waves: each wave is a Seq of MV names with
    ...
    */
  private def computeWaves(topology: Seq[String], mvBodies: Map[String, String]): Seq[Seq[String]] = {
    val mvNameSet = topology.toSet
  ...
  /** Word-boundary substring match. `\b` in Java regex treats `_` as a word
    ...
    */
  private def containsWordBoundary(text: String, term: String): Boolean = {
  ...
  /** Submit each item in `wave` to the executor pool and wait for all to
    ...
    */
  private def runWaveParallel(wave: Seq[String])(work: String => Unit): Unit = {
  ...
  /** Bidirectional `EXCEPT ALL` between the MV and a fresh recompute of its
    ...
    * MV's REFRESH and a fresh `spark.sql(body)` recompute can legitimately
    ...
    *     away [[TpcDiSpec.NonDeterministicWindowColumns]] from BOTH sides
    *     and re-check. Remaining columns MUST match exactly.
    */
  private def assertMvCorrect(mvName: String, refreshType: String, phase: String): Unit = {
    val body     = mvBodies(mvName)
    val expected = spark.sql(body)
    ...
    val dropForNonDet = TpcDiSpec.NonDeterministicWindowColumns
  ...
  /** Snapshot pre-batch Delta version for every non-FullRefresh MV. Uses
    ...
    */
  private def snapshotVersionsForNonFullRefreshMvs(): Map[String, Long] = {
    MvCatalog
      .list(spark)
      .iterator
  ...
  /** For every non-FullRefresh MV: read its Delta history after the
    ...
    */
  private def assertNonFullRefreshMvsAreIncremental(
  ...
  /** "bronze.brokerage_trade" → "/tpcdi/models/bronze/brokerage_trade.sql"
    ...
    */
  private def modelSqlResource(mvName: String): String = {
    val parts  = mvName.split("\\.")
    val schema = parts(0)
    val short  = parts(1)
    val sub    = if (schema == "gold" && TpcDiSpec.AnalyticsModels.contains(short)) "gold/analytics" else schema
    s"/tpcdi/models/$sub/$short.sql"
  }

  private def serializeMvName(id: TableIdentifier): String =
    id.database.fold(id.identifier)(db => s"$db.${id.identifier}")

  private def loadResourceLines(resource: String): Seq[String] = {
    val s = loadResourceString(resource)
    s.linesIterator.filter(_.trim.nonEmpty).toIndexedSeq
  }

  private def loadResourceString(resource: String): String = {
...
object TpcDiSpec {
  ...
  /** SCD-2 / windowed columns that the dbt TPC-DI models emit via
    ...
    * execution) and a fresh `spark.sql(body)` recompute (a second
