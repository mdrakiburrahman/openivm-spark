...
/** P5.rt9 — Comprehensive coverage of SEMI/ANTI shapes (without aggregation).
  ...
  */
class SemiAntiSpec extends AnyFunSpec with Matchers with BeforeAndAfterAll {

  private val warehouseDir: String = {
    val d = new File(s"target/test-warehouse-sa-${UUID.randomUUID().toString.take(8)}")
    d.mkdirs()
    d.getAbsolutePath
  }
  ...
  override def beforeAll(): Unit = {
    ...
    spark = SparkSession
      .builder()
      .master("local[1]")
      .appName("openivm-spark-SemiAntiSpec")
      .config(
  ...
  describe("(1) WHERE EXISTS subquery → FULL_REFRESH fallback") {
    it("refreshes correctly after INSERTs on both sides") {
      spark.sql("CREATE TABLE IF NOT EXISTS sa_gods_1(uid INT, name STRING) USING DELTA")
      spark.sql("CREATE TABLE IF NOT EXISTS sa_pay_1(from_uid INT, amt INT) USING DELTA")
      spark.sql("INSERT INTO sa_gods_1 VALUES (1, 'Zeus'), (2, 'Hera'), (3, 'Ares')")
      spark.sql("INSERT INTO sa_pay_1 VALUES (1, 100), (3, 50)")

      val viewBody =
        "SELECT * FROM sa_gods_1 g WHERE EXISTS " +
          "(SELECT 1 FROM sa_pay_1 p WHERE p.from_uid = g.uid)"
      spark.sql(s"CREATE MATERIALIZED VIEW mv_sa_1 AS $viewBody")

      mvRefreshType("mv_sa_1") shouldBe RefreshTypeCode.FullRefresh
      assertMvCorrect("mv_sa_1", viewBody)

      spark.sql("INSERT INTO sa_gods_1 VALUES (4, 'Athena')")
      spark.sql("INSERT INTO sa_pay_1 VALUES (4, 25)")
      refreshMv("mv_sa_1")
      assertMvCorrect("mv_sa_1", viewBody)
    }
  }
  ...
  describe("(2) WHERE NOT EXISTS subquery → FULL_REFRESH fallback") {
    it("refreshes correctly after INSERTs on both sides") {
      spark.sql("CREATE TABLE IF NOT EXISTS sa_gods_2(uid INT, name STRING) USING DELTA")
      spark.sql("CREATE TABLE IF NOT EXISTS sa_pay_2(from_uid INT, amt INT) USING DELTA")
      spark.sql("INSERT INTO sa_gods_2 VALUES (1, 'Zeus'), (2, 'Hera'), (3, 'Ares')")
      spark.sql("INSERT INTO sa_pay_2 VALUES (1, 100)")

      val viewBody =
        "SELECT * FROM sa_gods_2 g WHERE NOT EXISTS " +
          "(SELECT 1 FROM sa_pay_2 p WHERE p.from_uid = g.uid)"
      spark.sql(s"CREATE MATERIALIZED VIEW mv_sa_2 AS $viewBody")
      ...
      spark.sql("INSERT INTO sa_gods_2 VALUES (4, 'Athena')")
      spark.sql("INSERT INTO sa_pay_2 VALUES (2, 5)")
      refreshMv("mv_sa_2")
      assertMvCorrect("mv_sa_2", viewBody)
    }
  }
  ...
  describe("(3) WHERE IN subquery → FULL_REFRESH fallback") {
    it("refreshes correctly after INSERTs on both sides") {
      spark.sql("CREATE TABLE IF NOT EXISTS sa_gods_3(uid INT, name STRING) USING DELTA")
      spark.sql("CREATE TABLE IF NOT EXISTS sa_pay_3(from_uid INT, amt INT) USING DELTA")
      spark.sql("INSERT INTO sa_gods_3 VALUES (1, 'Zeus'), (2, 'Hera'), (3, 'Ares')")
      spark.sql("INSERT INTO sa_pay_3 VALUES (1, 100), (2, 50)")

      val viewBody =
        "SELECT * FROM sa_gods_3 WHERE uid IN (SELECT from_uid FROM sa_pay_3)"
      spark.sql(s"CREATE MATERIALIZED VIEW mv_sa_3 AS $viewBody")

      mvRefreshType("mv_sa_3") shouldBe RefreshTypeCode.FullRefresh
      assertMvCorrect("mv_sa_3", viewBody)

      spark.sql("INSERT INTO sa_gods_3 VALUES (4, 'Athena')")
      spark.sql("INSERT INTO sa_pay_3 VALUES (4, 1), (3, 7)")
      refreshMv("mv_sa_3")
      assertMvCorrect("mv_sa_3", viewBody)
    }
  }
  ...
  describe("(4) WHERE NOT IN subquery (NULL-filtered right) → FULL_REFRESH fallback") {
    it("refreshes correctly after INSERTs on both sides") {
      spark.sql("CREATE TABLE IF NOT EXISTS sa_gods_4(uid INT, name STRING) USING DELTA")
      spark.sql("CREATE TABLE IF NOT EXISTS sa_pay_4(from_uid INT, amt INT) USING DELTA")
      spark.sql("INSERT INTO sa_gods_4 VALUES (1, 'Zeus'), (2, 'Hera'), (3, 'Ares')")
      spark.sql("INSERT INTO sa_pay_4 VALUES (1, 100)")

      val viewBody =
        "SELECT * FROM sa_gods_4 WHERE uid NOT IN " +
          "(SELECT from_uid FROM sa_pay_4 WHERE from_uid IS NOT NULL)"
      spark.sql(s"CREATE MATERIALIZED VIEW mv_sa_4 AS $viewBody")

      mvRefreshType("mv_sa_4") shouldBe RefreshTypeCode.FullRefresh
      assertMvCorrect("mv_sa_4", viewBody)

      spark.sql("INSERT INTO sa_gods_4 VALUES (4, 'Athena')")
      spark.sql("INSERT INTO sa_pay_4 VALUES (3, 25)")
      refreshMv("mv_sa_4")
      assertMvCorrect("mv_sa_4", viewBody)
    }
  }
  ...
  describe("(5) LEFT SEMI JOIN syntax → FULL_REFRESH fallback") {
    it("refreshes correctly after INSERTs on both sides") {
      spark.sql("CREATE TABLE IF NOT EXISTS sa_gods_5(uid INT, name STRING) USING DELTA")
      spark.sql("CREATE TABLE IF NOT EXISTS sa_pay_5(from_uid INT, amt INT) USING DELTA")
      spark.sql("INSERT INTO sa_gods_5 VALUES (1, 'Zeus'), (2, 'Hera'), (3, 'Ares')")
      spark.sql("INSERT INTO sa_pay_5 VALUES (1, 100), (1, 200), (3, 50)")

      val viewBody =
        "SELECT g.* FROM sa_gods_5 g LEFT SEMI JOIN sa_pay_5 p ON g.uid = p.from_uid"
      spark.sql(s"CREATE MATERIALIZED VIEW mv_sa_5 AS $viewBody")

      mvRefreshType("mv_sa_5") shouldBe RefreshTypeCode.FullRefresh
      assertMvCorrect("mv_sa_5", viewBody)

      spark.sql("INSERT INTO sa_gods_5 VALUES (4, 'Athena')")
      spark.sql("INSERT INTO sa_pay_5 VALUES (4, 7)")
      refreshMv("mv_sa_5")
      assertMvCorrect("mv_sa_5", viewBody)
    }
  }
  ...
  describe("(6) LEFT ANTI JOIN syntax → FULL_REFRESH fallback") {
    it("refreshes correctly after INSERTs on both sides") {
      spark.sql("CREATE TABLE IF NOT EXISTS sa_gods_6(uid INT, name STRING) USING DELTA")
      spark.sql("CREATE TABLE IF NOT EXISTS sa_pay_6(from_uid INT, amt INT) USING DELTA")
      spark.sql("INSERT INTO sa_gods_6 VALUES (1, 'Zeus'), (2, 'Hera'), (3, 'Ares')")
      spark.sql("INSERT INTO sa_pay_6 VALUES (1, 100)")

      val viewBody =
        "SELECT g.* FROM sa_gods_6 g LEFT ANTI JOIN sa_pay_6 p ON g.uid = p.from_uid"
      spark.sql(s"CREATE MATERIALIZED VIEW mv_sa_6 AS $viewBody")

      mvRefreshType("mv_sa_6") shouldBe RefreshTypeCode.FullRefresh
      assertMvCorrect("mv_sa_6", viewBody)

      spark.sql("INSERT INTO sa_gods_6 VALUES (4, 'Athena')")
      spark.sql("INSERT INTO sa_pay_6 VALUES (2, 5), (3, 10)")
      refreshMv("mv_sa_6")
      assertMvCorrect("mv_sa_6", viewBody)
    }
  }
  ...
  describe("(7) RIGHT-side INSERT flips a left tuple invisible → visible") {
    it("MV reflects the new visibility after refresh") {
      spark.sql("CREATE TABLE IF NOT EXISTS sa_gods_7(uid INT, name STRING) USING DELTA")
      spark.sql("CREATE TABLE IF NOT EXISTS sa_pay_7(from_uid INT, amt INT) USING DELTA")
      spark.sql("INSERT INTO sa_gods_7 VALUES (1, 'Zeus'), (2, 'Hera'), (3, 'Ares')")
      spark.sql("INSERT INTO sa_pay_7 VALUES (1, 100)")

      val viewBody =
        "SELECT g.* FROM sa_gods_7 g WHERE EXISTS " +
          "(SELECT 1 FROM sa_pay_7 p WHERE p.from_uid = g.uid)"
      spark.sql(s"CREATE MATERIALIZED VIEW mv_sa_7 AS $viewBody")
      ...
      spark.sql("INSERT INTO sa_pay_7 VALUES (2, 50)")
      refreshMv("mv_sa_7")
      assertMvCorrect("mv_sa_7", viewBody)
  ...
  describe("(8) RIGHT-side DELETE flips a left tuple visible → invisible (last match removed)") {
    it("MV drops the left tuple after the last matching right row is deleted") {
      spark.sql("CREATE TABLE IF NOT EXISTS sa_gods_8(uid INT, name STRING) USING DELTA")
      spark.sql("CREATE TABLE IF NOT EXISTS sa_pay_8(from_uid INT, amt INT) USING DELTA")
      spark.sql("INSERT INTO sa_gods_8 VALUES (1, 'Zeus'), (2, 'Hera')")
      spark.sql("INSERT INTO sa_pay_8 VALUES (1, 100), (2, 50)")

      val viewBody =
        "SELECT g.* FROM sa_gods_8 g LEFT SEMI JOIN sa_pay_8 p ON g.uid = p.from_uid"
      spark.sql(s"CREATE MATERIALIZED VIEW mv_sa_8 AS $viewBody")
      ...
      spark.sql("DELETE FROM sa_pay_8 WHERE from_uid = 2")
      refreshMv("mv_sa_8")
      assertMvCorrect("mv_sa_8", viewBody)
  ...
  describe("(9) Multiple matches per left tuple — partial vs full retraction") {
    it("partial RIGHT-side DELETE keeps the row; final DELETE drops it") {
      spark.sql("CREATE TABLE IF NOT EXISTS sa_gods_9(uid INT, name STRING) USING DELTA")
      spark.sql("CREATE TABLE IF NOT EXISTS sa_pay_9(pid INT, from_uid INT, amt INT) USING DELTA")
      spark.sql("INSERT INTO sa_gods_9 VALUES (1, 'Zeus'), (2, 'Hera')")
      ...
      spark.sql(s"CREATE MATERIALIZED VIEW mv_sa_9 AS $viewBody")
      ...
      spark.sql("DELETE FROM sa_pay_9 WHERE pid = 10")
      refreshMv("mv_sa_9")
      assertMvCorrect("mv_sa_9", viewBody)
      spark.table("mv_sa_9").count() shouldBe 2L

      // Remove the remaining two — uid=1 must disappear.
      spark.sql("DELETE FROM sa_pay_9 WHERE pid IN (11, 12)")
      refreshMv("mv_sa_9")
      assertMvCorrect("mv_sa_9", viewBody)
  ...
  describe("(10) Batched LEFT+RIGHT DML mix → single REFRESH") {
    it("one refresh reconciles inserts and deletes on both sides") {
      spark.sql("CREATE TABLE IF NOT EXISTS sa_gods_10(uid INT, name STRING) USING DELTA")
      spark.sql("CREATE TABLE IF NOT EXISTS sa_pay_10(pid INT, from_uid INT, amt INT) USING DELTA")
      spark.sql(
        "INSERT INTO sa_gods_10 VALUES (1, 'Zeus'), (2, 'Hera'), (3, 'Ares'), (4, 'Apollo')"
      )
      // uid=1, uid=4 have payments; uid=2, uid=3 don't.
      spark.sql("INSERT INTO sa_pay_10 VALUES (100, 1, 50), (101, 4, 75)")

      val viewBody =
        "SELECT g.* FROM sa_gods_10 g LEFT SEMI JOIN sa_pay_10 p ON g.uid = p.from_uid"
      spark.sql(s"CREATE MATERIALIZED VIEW mv_sa_10 AS $viewBody")
      ...
      spark.sql("INSERT INTO sa_pay_10 VALUES (102, 2, 10)")   // RIGHT insert: Hera flips ON
      spark.sql("DELETE FROM sa_pay_10 WHERE pid = 101")       // RIGHT delete: Apollo flips OFF
      spark.sql("INSERT INTO sa_gods_10 VALUES (5, 'Hermes')") // LEFT insert, no match
      spark.sql("INSERT INTO sa_pay_10 VALUES (103, 5, 1)")    // RIGHT insert: Hermes flips ON
      spark.sql("DELETE FROM sa_gods_10 WHERE uid = 1")        // LEFT delete: Zeus removed
      refreshMv("mv_sa_10")
      assertMvCorrect("mv_sa_10", viewBody)
    }
  }
  ...
  describe("(11) NULL in RIGHT poisons NOT IN — MV is empty") {
    it("inserting a NULL into the right side empties the MV after refresh") {
      spark.sql("CREATE TABLE IF NOT EXISTS sa_gods_11(uid INT, name STRING) USING DELTA")
      spark.sql("CREATE TABLE IF NOT EXISTS sa_pay_11(from_uid INT, amt INT) USING DELTA")
      spark.sql("INSERT INTO sa_gods_11 VALUES (1, 'Zeus'), (2, 'Hera'), (3, 'Ares')")
      spark.sql("INSERT INTO sa_pay_11 VALUES (1, 100)")

      val viewBody =
        "SELECT * FROM sa_gods_11 WHERE uid NOT IN (SELECT from_uid FROM sa_pay_11)"
      spark.sql(s"CREATE MATERIALIZED VIEW mv_sa_11 AS $viewBody")
      ...
      spark.sql("INSERT INTO sa_pay_11 VALUES (CAST(NULL AS INT), 0)")
      refreshMv("mv_sa_11")
      assertMvCorrect("mv_sa_11", viewBody)
