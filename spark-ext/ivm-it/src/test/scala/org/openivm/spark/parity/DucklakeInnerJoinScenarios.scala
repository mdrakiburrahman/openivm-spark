...
/** ScalaTest port of `openivm/test/sql/ducklake_inner_join.test`.
  ...
  */
abstract class DucklakeInnerJoinScenarios extends IvmParitySpecBase("ducklake-inner-join") {
  ...
  describe("Test 1: 2-table INNER JOIN — INSERT left, then right") {
    it("incrementally maintains a 2-way INNER JOIN projection across batched INSERTs on both sides") {
      sql("CREATE TABLE IF NOT EXISTS dlij_customers(id INT, name STRING) USING DELTA")
      sql(
        "CREATE TABLE IF NOT EXISTS dlij_purchases(cust_id INT, item STRING, price INT) USING DELTA"
      )
      sql("INSERT INTO dlij_customers VALUES (1, 'Alice'), (2, 'Bob'), (3, 'Carol')")
      sql("INSERT INTO dlij_purchases VALUES (1, 'Widget', 10), (2, 'Gadget', 25)")

      val viewSql =
        "SELECT c.name, p.item, p.price " +
          "FROM dlij_customers c INNER JOIN dlij_purchases p ON c.id = p.cust_id"
      sql(s"CREATE MATERIALIZED VIEW dlij_customer_purchases AS $viewSql")
      refreshMv("dlij_customer_purchases")
      assertMvCorrect("dlij_customer_purchases", viewSql)

      // INSERT on right side (purchases)
      sql("INSERT INTO dlij_purchases VALUES (1, 'Doohickey', 5), (3, 'Gizmo', 50)")
      refreshMv("dlij_customer_purchases")
      assertMvCorrect("dlij_customer_purchases", viewSql)

      // INSERT on left side (customers) — new customer with existing purchase match
      sql("INSERT INTO dlij_customers VALUES (4, 'Diana')")
      sql("INSERT INTO dlij_purchases VALUES (4, 'Thingamajig', 15)")
      refreshMv("dlij_customer_purchases")
      assertMvCorrect("dlij_customer_purchases", viewSql)
    }
  }
  ...
  describe("Test 2: Aggregate over join (SUM + COUNT + GROUP BY)") {
    it("incrementally maintains SUM/COUNT aggregate over 2-way INNER JOIN") {
      sql("CREATE TABLE IF NOT EXISTS dlij_products(pid INT, pname STRING) USING DELTA")
      sql("CREATE TABLE IF NOT EXISTS dlij_sales(pid INT, qty INT, revenue INT) USING DELTA")
      sql("INSERT INTO dlij_products VALUES (1, 'Alpha'), (2, 'Beta'), (3, 'Gamma')")
      sql("INSERT INTO dlij_sales VALUES (1, 10, 100), (1, 5, 50), (2, 20, 400)")

      val viewSql =
        "SELECT p.pname, SUM(s.revenue) AS total_rev, COUNT(*) AS sale_count " +
          "FROM dlij_products p INNER JOIN dlij_sales s ON p.pid = s.pid " +
          "GROUP BY p.pname"
      sql(s"CREATE MATERIALIZED VIEW dlij_product_summary AS $viewSql")
      ...
      sql("INSERT INTO dlij_sales VALUES (2, 3, 60), (3, 7, 140)")
      refreshMv("dlij_product_summary")
      assertMvCorrect("dlij_product_summary", viewSql)

      // Delete a sale row — group total should adjust
      sql("DELETE FROM dlij_sales WHERE pid = 1 AND qty = 10")
      refreshMv("dlij_product_summary")
      assertMvCorrect("dlij_product_summary", viewSql)
    }
  }
  ...
  describe("Test 3: Stress — batch INSERT + DELETE on both tables before single refresh") {
    it("delta consolidation is correct for a many-conflicting-op batch") {
      sql("CREATE TABLE IF NOT EXISTS dlij_departments(did INT, dname STRING) USING DELTA")
      sql(
      ...
      sql(s"CREATE MATERIALIZED VIEW dlij_dept_roster AS $viewSql")
      ...
      sql("INSERT INTO dlij_departments VALUES (4, 'Legal')")
      sql("INSERT INTO dlij_employees VALUES (6, 4, 250), (7, 4, 300)")
      sql("INSERT INTO dlij_employees VALUES (8, 1, 500), (9, 2, 175)")
      sql("DELETE FROM dlij_employees WHERE eid = 2")
      sql("DELETE FROM dlij_employees WHERE eid = 3")
      sql("DELETE FROM dlij_departments WHERE did = 3")
      sql("INSERT INTO dlij_employees VALUES (10, 3, 999)")
      sql("INSERT INTO dlij_employees VALUES (11, 1, 50)")
      sql("DELETE FROM dlij_employees WHERE eid = 11")
  ...
  describe("Test 4: Empty delta — refresh with no changes is a safe no-op") {
    it("repeated refreshes with empty deltas leave the MV unchanged") {
      sql("CREATE TABLE IF NOT EXISTS dlij_empty_d(did INT, dname STRING) USING DELTA")
      sql("CREATE TABLE IF NOT EXISTS dlij_empty_e(eid INT, did INT, salary INT) USING DELTA")
      sql("INSERT INTO dlij_empty_d VALUES (1, 'X'), (2, 'Y')")
      sql("INSERT INTO dlij_empty_e VALUES (1, 1, 100), (2, 2, 200)")

      val viewSql =
        "SELECT d.dname, e.eid, e.salary " +
          "FROM dlij_empty_d d INNER JOIN dlij_empty_e e ON d.did = e.did"
      sql(s"CREATE MATERIALIZED VIEW dlij_empty_roster AS $viewSql")
  ...
  describe("Test 6: Cost-model probe (DuckLake PRAGMA refresh_cost has no Spark analogue)") {
    it(
      ...
      sql("CREATE TABLE IF NOT EXISTS dlij_cost_p(pid INT, pname STRING) USING DELTA")
      sql("CREATE TABLE IF NOT EXISTS dlij_cost_s(pid INT, qty INT, revenue INT) USING DELTA")
      sql("INSERT INTO dlij_cost_p VALUES (1, 'Alpha'), (2, 'Beta'), (3, 'Gamma')")
      sql("INSERT INTO dlij_cost_s VALUES (1, 10, 100), (1, 5, 50), (2, 20, 400)")

      val viewSql =
        "SELECT p.pname, SUM(s.revenue) AS total_rev, COUNT(*) AS sale_count " +
          "FROM dlij_cost_p p INNER JOIN dlij_cost_s s ON p.pid = s.pid " +
          "GROUP BY p.pname"
      sql(s"CREATE MATERIALIZED VIEW dlij_cost_summary AS $viewSql")
      refreshMv("dlij_cost_summary")

      // Insert data into only one side of the join, then refresh.
      sql("INSERT INTO dlij_cost_s VALUES (3, 1, 999)")
      refreshMv("dlij_cost_summary")
  ...
  describe("Test 7: Projection-key refresh over a 4-table join with a self-join alias") {
    it("4-way join with one table appearing twice (via alias) maintains correctness across UPDATE/INSERT") {
      ...
      sql("INSERT INTO dlij_pk_trades VALUES (100, 10), (200, 20), (300, 30)")
      sql("INSERT INTO dlij_pk_accounts VALUES (1, 1000), (2, 2000)")
      sql("INSERT INTO dlij_pk_securities VALUES ('A', 500), ('B', 600)")
      sql("INSERT INTO dlij_pk_holdings VALUES (10, 20, 1, 'A', 5), (30, 20, 2, 'B', 9)")
      ...
      sql(s"CREATE MATERIALIZED VIEW dlij_pk_holdings_mv AS $viewSql")
      ...
      sql("UPDATE dlij_pk_trades SET sk_trade_id = 201 WHERE trade_id = 20")
      sql("UPDATE dlij_pk_accounts SET sk_customer_id = 1001 WHERE account_id = 1")
      sql("INSERT INTO dlij_pk_holdings VALUES (10, 20, 1, 'A', 6)")
      refreshMv("dlij_pk_holdings_mv")
      assertMvCorrect("dlij_pk_holdings_mv", viewSql)

      // Changing the output key itself must delete the old key partition and insert the new one.
      sql("UPDATE dlij_pk_trades SET sk_trade_id = 101 WHERE trade_id = 10")
      refreshMv("dlij_pk_holdings_mv")
      assertMvCorrect("dlij_pk_holdings_mv", viewSql)
    }
  }
