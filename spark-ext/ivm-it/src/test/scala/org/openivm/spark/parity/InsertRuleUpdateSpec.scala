...
/** Slice of `InsertRuleSpec` covering UPDATE: constant SET value (with
  ...
  */
class InsertRuleUpdateSpec extends AnyFunSpec with Matchers with BeforeAndAfterAll {

  private val warehouseDir: String = {
    val d = new File(s"target/test-warehouse-ins-update-${UUID.randomUUID().toString.take(8)}")
    d.mkdirs()
    d.getAbsolutePath
  }
  ...
  override def beforeAll(): Unit = {
    ...
    spark = SparkSession
      .builder()
      .master("local[1]")
      .appName("openivm-spark-InsertRuleUpdateSpec")
      .config(
  ...
  describe("(6) UPDATE — constant SET value") {
    ...
    it("UPDATE writes UPDATE_BEFORE + UPDATE_AFTER staging rows and propagates correctly") {
      setupStaff()
      spark.sql("UPDATE staff_ir SET name = 'Alice2' WHERE id = 1")
  ...
  describe("(10) Stress: UPDATE same row twice before REFRESH") {
    ...
    it("the second UPDATE's value wins; MV reflects the final state") {
      setupStaff()
      spark.sql("UPDATE staff_ir SET name = 'FirstUpdate' WHERE id = 1")
      spark.sql("UPDATE staff_ir SET name = 'SecondUpdate' WHERE id = 1")
      refreshMv("mv_staff_ir")
      assertMvCorrect("mv_staff_ir", "SELECT id, name, dept, salary FROM staff_ir")
      spark
  ...
  describe("(14) UPDATE multiple columns at once") {
    ...
    it("multi-column SET updates all three columns and propagates to the MV") {
      setupStaff()
      spark.sql("INSERT INTO staff_ir VALUES (130, 'Multi', 'eng', 500)")
      refreshMv("mv_staff_ir")
      spark.sql(
      ...
      val row = spark.sql("SELECT id, name, dept, salary FROM mv_staff_ir WHERE id = 130").collect().head
