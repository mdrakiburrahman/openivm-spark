...
/** Parity port of `openivm/test/sql/inner_join_fk.test`.
  ...
  */
class InnerJoinFkSpec extends AnyFunSpec with Matchers with BeforeAndAfterAll {

  private val warehouseDir: String = {
    val d = new File(s"target/test-warehouse-fkjoin-${UUID.randomUUID().toString.take(8)}")
    d.mkdirs()
    d.getAbsolutePath
  }
  ...
  override def beforeAll(): Unit = {
    ...
    spark = SparkSession
      .builder()
      .master("local[1]")
      .appName("openivm-spark-InnerJoinFkSpec")
      .config(
  ...
  /** Bidirectional EXCEPT ALL equivalence check.  Projects the MV to the same
    * columns as the expected query so any hidden bookkeeping columns are
  ...
  describe("(1) Star schema (fact_sales, dim_product, dim_region) — FK-pruning scenarios") {
    ...
    it("(1a) initial load: MV = base join") {
      spark.sql("CREATE TABLE IF NOT EXISTS dim_product(product_id INT, name STRING) USING DELTA")
      spark.sql("CREATE TABLE IF NOT EXISTS dim_region(region_id INT, region_name STRING) USING DELTA")
      spark.sql(
        "CREATE TABLE IF NOT EXISTS fact_sales(sale_id INT, product_id INT, region_id INT, amount DECIMAL(10,2)) USING DELTA"
      )

      spark.sql("INSERT INTO dim_product VALUES (1, 'Widget'), (2, 'Gadget'), (3, 'Doohickey')")
      spark.sql("INSERT INTO dim_region VALUES (10, 'North'), (20, 'South')")
      spark.sql("INSERT INTO fact_sales VALUES (1, 1, 10, 100.0), (2, 2, 20, 200.0), (3, 1, 20, 150.0)")
    ...
    it("(1b) INSERT into dim_product alone — no MV change") {
      spark.sql("INSERT INTO dim_product VALUES (4, 'Thingamajig')")
      refreshMv("fk_star")
      assertMvCorrect(
        "fk_star",
        "SELECT p.name, r.region_name, SUM(f.amount) AS total, COUNT(*) AS cnt " +
    ...
    it("(1c) INSERT into fact referencing the new dim_product — new group appears") {
      spark.sql("INSERT INTO fact_sales VALUES (4, 4, 10, 300.0)")
      refreshMv("fk_star")
      assertMvCorrect(
        "fk_star",
        "SELECT p.name, r.region_name, SUM(f.amount) AS total, COUNT(*) AS cnt " +
    ...
    it("(1d) UPDATE dim_product (delete + insert delta) — name change propagates") {
      spark.sql("UPDATE dim_product SET name = 'SuperWidget' WHERE product_id = 1")
      refreshMv("fk_star")
      assertMvCorrect(
        "fk_star",
        "SELECT p.name, r.region_name, SUM(f.amount) AS total, COUNT(*) AS cnt " +
    ...
    it("(1e) DELETE from dim_product + cascading delete on fact_sales") {
      spark.sql("DELETE FROM fact_sales WHERE product_id = 3")
      spark.sql("DELETE FROM dim_product WHERE product_id = 3")
      refreshMv("fk_star")
      assertMvCorrect(
        "fk_star",
        "SELECT p.name, r.region_name, SUM(f.amount) AS total, COUNT(*) AS cnt " +
    ...
    it("(1f) Simultaneous INSERT into fact_sales + dim_region — new region group") {
      spark.sql("INSERT INTO dim_region VALUES (30, 'East')")
      spark.sql("INSERT INTO fact_sales VALUES (5, 2, 30, 500.0)")
      refreshMv("fk_star")
      assertMvCorrect(
        "fk_star",
        "SELECT p.name, r.region_name, SUM(f.amount) AS total, COUNT(*) AS cnt " +
    ...
    it("(1g) Stress — batched INSERTs/DELETE/UPDATE on fact + insert-only dims, single REFRESH") {
      spark.sql("INSERT INTO dim_product VALUES (5, 'Gizmo'), (6, 'Contraption')")
      spark.sql("INSERT INTO dim_region VALUES (40, 'West')")
      spark.sql(
        "INSERT INTO fact_sales VALUES " +
          "(10, 5, 10, 50.0), (11, 5, 20, 75.0), (12, 6, 30, 200.0), " +
          "(13, 2, 40, 125.0), (14, 4, 40, 350.0)"
      )
      spark.sql("DELETE FROM fact_sales WHERE sale_id = 2")
      spark.sql("UPDATE fact_sales SET amount = 999.0 WHERE sale_id = 1")
      refreshMv("fk_star")
      assertMvCorrect(
        "fk_star",
        "SELECT p.name, r.region_name, SUM(f.amount) AS total, COUNT(*) AS cnt " +
  ...
  describe("(2) 2-table FK join (departments, employees)") {
    it("INSERT into departments alone → no MV change; subsequent employee INSERT → new group") {
      spark.sql("CREATE TABLE IF NOT EXISTS departments(dept_id INT, dept_name STRING) USING DELTA")
      spark.sql(
        "CREATE TABLE IF NOT EXISTS employees(emp_id INT, dept_id INT, salary INT) USING DELTA"
      )
      spark.sql("INSERT INTO departments VALUES (1, 'Engineering'), (2, 'Sales')")
      spark.sql("INSERT INTO employees VALUES (1, 1, 100000), (2, 1, 120000), (3, 2, 80000)")
      ...
      spark.sql("INSERT INTO departments VALUES (3, 'Marketing')")
      ...
      spark.sql("INSERT INTO employees VALUES (4, 3, 90000)")
      refreshMv("dept_costs")
      assertMvCorrect(
        "dept_costs",
        "SELECT d.dept_name, SUM(e.salary) AS total_salary, COUNT(*) AS headcount " +
  ...
  describe("(3) 2-table join WITHOUT FK metadata — pruning never applies, correctness must hold") {
    it("MV stays bag-equal under simultaneous inserts on both sides") {
      spark.sql("CREATE TABLE IF NOT EXISTS t_left_nofk(a INT, b INT) USING DELTA")
      spark.sql("CREATE TABLE IF NOT EXISTS t_right_nofk(b INT, c INT) USING DELTA")
      spark.sql("INSERT INTO t_left_nofk VALUES (1, 10), (2, 20)")
      spark.sql("INSERT INTO t_right_nofk VALUES (10, 100), (20, 200)")
      ...
      spark.sql("INSERT INTO t_right_nofk VALUES (30, 300)")
      spark.sql("INSERT INTO t_left_nofk VALUES (3, 30)")
      refreshMv("mv_no_fk")
      assertMvCorrect(
        "mv_no_fk",
        "SELECT t_left_nofk.a, t_right_nofk.c FROM t_left_nofk JOIN t_right_nofk ON t_left_nofk.b = t_right_nofk.b"
