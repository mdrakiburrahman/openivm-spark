...
class MvCatalogMigrationSpec extends AnyFunSpec with Matchers with BeforeAndAfterAll {

  private val warehouseRoot: File = {
    val d = new File(s"target/test-warehouse-mvc-migration-${UUID.randomUUID().toString.take(8)}")
    d.mkdirs()
    d
  }

  private val legacyMvSchema = StructType(
    Array(
  ...
  private val legacyStagingSchema = StructType(
    Array(
  ...
  private def withSpark(warehouseName: String)(f: (SparkSession, String) => Unit): Unit = {
  ...
  private def legacyMvMetaPath(warehouseDir: String): String =
    Paths.get(warehouseDir, "_ivm", "_meta", "mv_metadata").toString

  private def legacyStagingPath(warehouseDir: String): String =
    Paths.get(warehouseDir, "_ivm", "_meta", "staging").toString

  private def pathExists(path: String): Boolean = Files.exists(Paths.get(path))

  private def writeDeltaRows(spark: SparkSession, path: String, schema: StructType, rows: Seq[Row]): Unit =
    spark
  ...
  private def sampleMeta(warehouseDir: String, name: String): MvMetadata =
    MvMetadata(
