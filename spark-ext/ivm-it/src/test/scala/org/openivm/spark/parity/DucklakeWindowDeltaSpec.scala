...
/** P6e — ScalaTest port of `openivm/test/sql/ducklake_window_delta.test`.
  ...
  */
class DucklakeWindowDeltaSpec extends AnyFunSpec with Matchers with BeforeAndAfterAll {

  private val warehouseDir: String = {
    val d = new File(s"target/test-warehouse-dlwd-${UUID.randomUUID().toString.take(8)}")
    d.mkdirs()
    d.getAbsolutePath
  }
  ...
  override def beforeAll(): Unit = {
    ...
    spark = SparkSession
      .builder()
      .master("local[1]")
      .appName("openivm-spark-DucklakeWindowDeltaSpec")
      .config(
  ...
  describe("DuckLake window refresh preserves unchanged partition rows + downstream join chain") {
    it(
      ...
      spark.sql("INSERT INTO dlwd_security VALUES ('A', 'tech'), ('B', 'bank')")
      ...
      val mvMarketSql =
        "SELECT symbol, d, price, " +
          ...
          ") AS roll_max " +
          "FROM dlwd_market"
      spark.sql(s"CREATE MATERIALIZED VIEW dlwd_mv_market AS $mvMarketSql")
      ...
      spark.sql(s"CREATE MATERIALIZED VIEW dlwd_mv_market_join AS $mvJoinSql")
      ...
      spark.sql("INSERT INTO dlwd_market VALUES ('A', 4, 25)")
