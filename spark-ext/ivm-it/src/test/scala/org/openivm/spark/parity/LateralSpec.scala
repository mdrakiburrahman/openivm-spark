...
/** P6a — Port of `openivm/test/sql/lateral.test`.
  ...
  */
class LateralSpec extends AnyFunSpec with Matchers with BeforeAndAfterAll {

  private val warehouseDir: String = {
    val d = new File(s"target/test-warehouse-lat-${UUID.randomUUID().toString.take(8)}")
    d.mkdirs()
    d.getAbsolutePath
  }
  ...
  override def beforeAll(): Unit = {
    ...
    spark = SparkSession
      .builder()
      .master("local[1]")
      .appName("openivm-spark-LateralSpec")
      .config(
  ...
  describe("(1a) DELIM/DEPENDENT scalar aggregate — initial MV correctness (no DML)") {
    it("MV is bag-equal to view body immediately after CREATE") {
      spark.sql("CREATE TABLE IF NOT EXISTS lat_customer(c_id INT, c_w_id INT) USING DELTA")
      spark.sql("CREATE TABLE IF NOT EXISTS lat_history(h_c_id INT, h_c_w_id INT, h_amount INT) USING DELTA")
      spark.sql("INSERT INTO lat_customer VALUES (1, 1), (1, 1), (2, 1)")
      spark.sql("INSERT INTO lat_history  VALUES (1, 1, 10), (1, 1, 20), (2, 1, 5)")
      ...
      val viewBody =
        "SELECT c.c_id, c.c_w_id, " +
          ...
          "FROM lat_customer c"
      spark.sql(s"CREATE MATERIALIZED VIEW mv_lat AS $viewBody")
  ...
  describe("(1b) DELIM/DEPENDENT scalar aggregate — incremental refresh after DML") {
    ...
    ignore("MV stays bag-equal to live view body across INSERT/DELETE on both sides") {
      spark.sql("CREATE TABLE IF NOT EXISTS lat_customer_b(c_id INT, c_w_id INT) USING DELTA")
      spark.sql("CREATE TABLE IF NOT EXISTS lat_history_b(h_c_id INT, h_c_w_id INT, h_amount INT) USING DELTA")
      spark.sql("INSERT INTO lat_customer_b VALUES (1, 1), (2, 1)")
      spark.sql("INSERT INTO lat_history_b  VALUES (1, 1, 10), (2, 1, 5)")
      ...
      spark.sql(s"CREATE MATERIALIZED VIEW mv_lat_b AS $viewBody")

      spark.sql("INSERT INTO lat_history_b VALUES (1, 1, 7)")
      refreshMv("mv_lat_b")
  ...
  describe("(2) Row-local UNNEST projection: SKIPPED — Spark LATERAL VIEW not parseable by DuckDB CLI") {
    ignore("MV stays bag-equal across INSERT/DELETE/UPDATE on the source table") {
      ...
      spark.sql(s"CREATE MATERIALIZED VIEW mv_lat_unnest_projection AS $viewBody")
  ...
  describe("(3) UNNEST after JOIN with aggregate: SKIPPED — no portable lateral syntax") {
    ignore("aggregate over join + lateral-explode delta is incrementally maintained") {
      spark.sql("CREATE TABLE IF NOT EXISTS lat_unnest_stock_agg(s_w_id INT, s_i_id INT, qty INT) USING DELTA")
      spark.sql("CREATE TABLE IF NOT EXISTS lat_unnest_item_agg(i_id INT, price INT) USING DELTA")
      spark.sql("INSERT INTO lat_unnest_stock_agg VALUES (1, 10, 5), (1, 11, 25), (2, 20, 50)")
      spark.sql("INSERT INTO lat_unnest_item_agg  VALUES (10, 30), (11, 60), (20, 70)")
      val viewBody =
        "SELECT u.flag, COUNT(*) AS rows_seen, SUM(s.qty * i.price) AS value_seen " +
          ...
          "GROUP BY u.flag"
      spark.sql(s"CREATE MATERIALIZED VIEW mv_lat_unnest_join_agg AS $viewBody")
  ...
  describe("(4) COUNT(DISTINCT) over UNNEST: SKIPPED — no portable lateral syntax") {
    ignore("MV correctly tracks deltas; classifier ≈ GROUP_RECOMPUTE") {
      ...
      spark.sql("INSERT INTO lat_unnest_cd VALUES (1, 10, 'a', 'b', 'a', 'c')")
      val viewBody =
      ...
      spark.sql(s"CREATE MATERIALIZED VIEW mv_lat_unnest_count_distinct AS $viewBody")
  ...
  describe("(5a) Scalar correlated subquery — initial MV correctness (no DML)") {
    it("MV is bag-equal to view body immediately after CREATE") {
      spark.sql("CREATE TABLE IF NOT EXISTS lat_scalar_wh(w_id INT, w_name STRING) USING DELTA")
      spark.sql("CREATE TABLE IF NOT EXISTS lat_scalar_n(n INT) USING DELTA")
      spark.sql("INSERT INTO lat_scalar_wh VALUES (1, 'w1'), (2, 'w2'), (4, 'w4')")
      spark.sql("INSERT INTO lat_scalar_n  VALUES (1), (2), (3), (4), (5)")
      ...
      val viewBody =
        "SELECT n, " +
          ...
          "FROM lat_scalar_n"
      spark.sql(s"CREATE MATERIALIZED VIEW mv_lat_scalar AS $viewBody")
  ...
  describe("(5b) Scalar correlated subquery — incremental refresh after DML") {
    ...
    ignore("MV maintains correctness under UPDATE / DELETE / INSERT on the looked-up table") {
      spark.sql("CREATE TABLE IF NOT EXISTS lat_scalar_wh_b(w_id INT, w_name STRING) USING DELTA")
      spark.sql("CREATE TABLE IF NOT EXISTS lat_scalar_n_b(n INT) USING DELTA")
      spark.sql("INSERT INTO lat_scalar_wh_b VALUES (1, 'w1'), (2, 'w2'), (4, 'w4')")
      spark.sql("INSERT INTO lat_scalar_n_b  VALUES (1), (2), (3), (4), (5)")

      val viewBody =
        "SELECT n, " +
          "  (SELECT MAX(w_name) FROM lat_scalar_wh_b WHERE w_id = n) AS wh_name " +
          "FROM lat_scalar_n_b"
      spark.sql(s"CREATE MATERIALIZED VIEW mv_lat_scalar_b AS $viewBody")

      // Batched: UPDATE + DELETE + INSERT on the lookup table
      spark.sql("UPDATE lat_scalar_wh_b SET w_name = 'w1x' WHERE w_id = 1")
      spark.sql("DELETE FROM lat_scalar_wh_b WHERE w_id = 2")
      spark.sql("INSERT INTO lat_scalar_wh_b VALUES (3, 'w3')")
  ...
  describe("(6) Per-row generator via LATERAL VIEW: SKIPPED — Spark-only syntax") {
    ignore("MV grows when a new w_id is inserted; explode produces n rows per source row") {
      spark.sql("CREATE TABLE IF NOT EXISTS lat_wh(w_id INT, w_name STRING) USING DELTA")
      spark.sql("INSERT INTO lat_wh VALUES (1, 'w1'), (2, 'w2')")
      val viewBody =
        "SELECT w_id, slot FROM lat_wh w " +
          "LATERAL VIEW explode(sequence(1, w.w_id)) t AS slot"
      spark.sql(s"CREATE MATERIALIZED VIEW mv_lat_series AS $viewBody")
