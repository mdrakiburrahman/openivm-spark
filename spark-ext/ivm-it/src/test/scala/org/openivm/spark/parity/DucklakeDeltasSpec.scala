...
/** P6d — Port of `openivm/test/sql/ducklake_deltas.test`.
  ...
  */
class DucklakeDeltasSpec extends AnyFunSpec with Matchers with BeforeAndAfterAll {

  private val warehouseDir: String = {
    val d = new File(s"target/test-warehouse-dl-deltas-${UUID.randomUUID().toString.take(8)}")
    d.mkdirs()
    d.getAbsolutePath
  }
  ...
  override def beforeAll(): Unit = {
    ...
    spark = SparkSession
      .builder()
      .master("local[1]")
      .appName("openivm-spark-DucklakeDeltasSpec")
      .config(
  ...
  describe("(DDel-1) MV Delta version is set after CREATE and advances after refresh") {
    it("last_version >= 0 after CREATE and strictly increases on each new-data REFRESH") {
      spark.sql("CREATE TABLE IF NOT EXISTS dl_snap_test(id INT, grp STRING, val INT) USING DELTA")
      spark.sql("INSERT INTO dl_snap_test VALUES (1, 'A', 10), (2, 'A', 20)")
      spark.sql(
      ...
      spark.sql("INSERT INTO dl_snap_test VALUES (3, 'A', 30)")
  ...
  describe("(DDel-2) Stored MV SQL does not pin a Delta version") {
    it("querySql contains no VERSION AS OF / TIMESTAMP AS OF / AT (VERSION") {
      spark.sql("CREATE TABLE IF NOT EXISTS dl_snap_test2(id INT, grp STRING, val INT) USING DELTA")
      spark.sql("INSERT INTO dl_snap_test2 VALUES (1, 'A', 10)")
      spark.sql(
        "CREATE MATERIALIZED VIEW dl_mv_snap2 AS " +
          "SELECT grp, SUM(val) AS total, COUNT(*) AS cnt FROM dl_snap_test2 GROUP BY grp"
      )
      val sql = mvStoredSql("dl_mv_snap2").toUpperCase
      withClue(s"Stored SQL: $sql") {
        sql should not include "VERSION AS OF"
        sql should not include "TIMESTAMP AS OF"
        sql should not include "AT (VERSION"
      }
    }
  }
  ...
  describe("(DDel-3) Multi-batch deltas — single REFRESH picks up all batches") {
    it("three separate INSERTs are reconciled by a single refresh") {
      spark.sql("CREATE TABLE IF NOT EXISTS dl_multi(id INT, name STRING, amount INT) USING DELTA")
      spark.sql("INSERT INTO dl_multi VALUES (1, 'A', 100)")
      spark.sql(
      ...
      spark.sql("INSERT INTO dl_multi VALUES (2, 'A', 50)")
      spark.sql("INSERT INTO dl_multi VALUES (3, 'B', 200)")
      spark.sql("INSERT INTO dl_multi VALUES (4, 'A', 25)")
  ...
  describe("(DDel-4) DELETE propagation — single row + entire group") {
    it("DELETE of one row leaves the group; DELETE of last row in group removes it") {
      ...
      spark.sql("DELETE FROM dl_del_test WHERE category = 'Y'")
      refreshMv("dl_mv_del")
      assertMvCorrect(
        "dl_mv_del",
        "SELECT category, SUM(price) AS total, COUNT(*) AS cnt FROM dl_del_test GROUP BY category"
  ...
  describe("(DDel-5) Idempotent refresh — no replay of previously consumed deltas") {
    it("second back-to-back refresh keeps MV equal to view body (no double-counting)") {
      ...
      spark.sql("INSERT INTO dl_replay_sales VALUES (4, 'east', 50), (5, 'north', 300)")
      refreshMv("dl_mv_replay_sales")
      val afterInsertVer = mvLastVersion("dl_mv_replay_sales")

      spark.sql("DELETE FROM dl_replay_sales WHERE id = 1")
      refreshMv("dl_mv_replay_sales")
  ...
  describe("(DDel-6) Failed CREATE MV cleans up; the same name can be retried") {
    it("CREATE MV against a pre-existing user table fails AND leaves no orphans") {
      spark.sql("CREATE TABLE IF NOT EXISTS dl_publish_src(id INT, grp STRING, v INT) USING DELTA")
      spark.sql("INSERT INTO dl_publish_src VALUES (1, 'a', 10), (2, 'b', 20)")

      // Pre-existing target collides with the would-be MV name
      spark.sql("CREATE TABLE IF NOT EXISTS dl_blocked_publish_mv(x INT) USING DELTA")
      ...
      try spark.sql("DROP TABLE IF EXISTS dl_blocked_publish_mv").collect()
      catch { case _: Throwable => () }
      spark.sql("CREATE TABLE dl_blocked_publish_mv(x INT) USING DELTA")
      ...
      spark.sql("DROP TABLE IF EXISTS dl_blocked_publish_mv")
      spark.sql(
        "CREATE MATERIALIZED VIEW dl_blocked_publish_mv AS " +
          "SELECT grp, SUM(v) AS total FROM dl_publish_src GROUP BY grp"
      )
  ...
  describe("(DDel-7) Empty-delta skip — unrelated-table writes don't advance MV version") {
    it("INSERT into a non-source table leaves MV last_version unchanged after REFRESH") {
      spark.sql("CREATE TABLE IF NOT EXISTS dl_skip_base(id INT, v INT) USING DELTA")
      spark.sql("CREATE TABLE IF NOT EXISTS dl_skip_unrelated(id INT) USING DELTA")
      spark.sql("INSERT INTO dl_skip_base VALUES (1, 10)")
      spark.sql("CREATE MATERIALIZED VIEW dl_mv_skip_base AS SELECT id, v FROM dl_skip_base")

      val vBefore = mvLastVersion("dl_mv_skip_base")

      // INSERT into an unrelated table — must not register staging for our MV.
      spark.sql("INSERT INTO dl_skip_unrelated VALUES (1)")
    ...
    it("join MV: refresh after INSERT into ONLY one side stays correct") {
      spark.sql("CREATE TABLE IF NOT EXISTS dl_skip_left(id INT, v INT) USING DELTA")
      spark.sql("CREATE TABLE IF NOT EXISTS dl_skip_right(id INT, w INT) USING DELTA")
      spark.sql("INSERT INTO dl_skip_left VALUES (1, 10), (2, 20)")
      spark.sql("INSERT INTO dl_skip_right VALUES (1, 100), (2, 200)")
      spark.sql(
      ...
      spark.sql("INSERT INTO dl_skip_left VALUES (3, 30)")
    ...
    it("join MV: new left-side rows with no matching right-side keys produce no MV rows") {
      spark.sql("CREATE TABLE IF NOT EXISTS dl_skip_key_left(id INT, v INT) USING DELTA")
      spark.sql("CREATE TABLE IF NOT EXISTS dl_skip_key_right(id INT, w INT) USING DELTA")
      spark.sql("INSERT INTO dl_skip_key_left VALUES (1, 10)")
      spark.sql("INSERT INTO dl_skip_key_right VALUES (1, 100)")
      spark.sql(
      ...
      spark.sql("INSERT INTO dl_skip_key_left VALUES (999, 999)")
  ...
  describe("(DDel-8) MV data table is a Delta table with queryable version history") {
    it("readable via DeltaTable.history and option('versionAsOf', N)") {
      spark.sql("CREATE TABLE IF NOT EXISTS dl_ver_src(id INT, v INT) USING DELTA")
      spark.sql("INSERT INTO dl_ver_src VALUES (1, 10), (2, 20)")
      spark.sql(
        "CREATE MATERIALIZED VIEW dl_mv_ver AS SELECT id, v FROM dl_ver_src"
      )
      val v0 = mvLastVersion("dl_mv_ver")

      spark.sql("INSERT INTO dl_ver_src VALUES (3, 30)")
      refreshMv("dl_mv_ver")
      ...
      asOfV0
        .selectExpr("id", "v")
        .exceptAll(spark.sql("SELECT id, v FROM dl_ver_src WHERE id IN (1,2)"))
        .count() shouldBe 0L
    }
  }
