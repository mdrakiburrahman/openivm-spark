...
/** P6f — 1:1 ScalaTest port of openivm `test/sql/recompute.test` (748 lines).
  ...
  */
class RecomputeSpec extends AnyFunSpec with Matchers with BeforeAndAfterAll {

  private val warehouseDir: String = {
    val d = new File(s"target/test-warehouse-recompute-${UUID.randomUUID().toString.take(8)}")
    d.mkdirs()
    d.getAbsolutePath
  }
  ...
  override def beforeAll(): Unit = {
    ...
    spark = SparkSession
      .builder()
      .master("local[1]")
      .appName("openivm-spark-RecomputeSpec")
      .config(
  ...
  /** Like [[assertMvCorrect]] but rounds the named columns on both sides to
    ...
    * implementation.
    */
  private def assertMvCorrectRounded(
      mvName: String,
      expectedSql: String,
  ...
  describe("(B) Market rollup — self-join CTE with 12-month date range") {
    ...
    ignore("matches the base query after inserting a row that updates 52-week lows") {
      ...
      spark.sql("INSERT INTO rc_market VALUES (DATE '2019-12-31', 'A', 9, 21, 1, 50)")
  ...
  describe("(C) UNION ALL over per-branch aggregates with branch-tag column") {
    ...
    it("tracks deltas across both branches; empty-delta refresh is a no-op") {
      spark.sql("CREATE TABLE IF NOT EXISTS rc_orders_a(region STRING, amount INT) USING DELTA")
      spark.sql("CREATE TABLE IF NOT EXISTS rc_orders_b(region STRING, amount INT) USING DELTA")
      spark.sql("INSERT INTO rc_orders_a VALUES ('east', 100), ('west', 200), ('east', 50)")
      spark.sql("INSERT INTO rc_orders_b VALUES ('east', 300), ('south', 75)")
      ...
      spark.sql("INSERT INTO rc_orders_a VALUES ('north', 500)")
      spark.sql("INSERT INTO rc_orders_b VALUES ('west', 120)")
      refreshMv("rc_union_agg")
      assertMvCorrect("rc_union_agg", viewBody)

      // Batched mixed delta across both branches
      spark.sql("DELETE FROM rc_orders_a WHERE region = 'east'")
      spark.sql("INSERT INTO rc_orders_b VALUES ('east', 10), ('south', 25)")
      spark.sql("UPDATE rc_orders_a SET amount = 999 WHERE region = 'west'")
      refreshMv("rc_union_agg")
      assertMvCorrect("rc_union_agg", viewBody)
  ...
  describe("(D) GROUP BY ROLLUP — produces hierarchical totals incrementally") {
    ...
    it("ROLLUP rows for (region,dept), (region,NULL), (NULL,NULL) stay bag-equal after batched DML") {
      spark.sql("CREATE TABLE IF NOT EXISTS rc_sales(region STRING, dept STRING, amount INT) USING DELTA")
      spark.sql(
      ...
      spark.sql("INSERT INTO rc_sales VALUES ('east', 'A', 300), ('south', 'C', 75)")
      refreshMv("rc_sales_rollup")
      assertMvCorrect("rc_sales_rollup", viewBody)

      // Batched: delete a ROLLUP cell, update another, introduce a new (region,dept)
      spark.sql("DELETE FROM rc_sales WHERE region = 'west' AND dept = 'B'")
      spark.sql("UPDATE rc_sales SET amount = 500 WHERE region = 'east' AND dept = 'B'")
      spark.sql("INSERT INTO rc_sales VALUES ('north', 'A', 999)")
      refreshMv("rc_sales_rollup")
      assertMvCorrect("rc_sales_rollup", viewBody)
    }
  }
  ...
  describe("(E) AVG under JOIN — district averages times warehouse tax factor") {
    ...
    it("matches the base join-over-aggregate after INSERT and after batched DML stress") {
      spark.sql("CREATE TABLE IF NOT EXISTS rc_wh(w_id INT, w_name STRING, w_tax DOUBLE) USING DELTA")
      spark.sql("CREATE TABLE IF NOT EXISTS rc_ol(ol_w_id INT, ol_d_id INT, ol_amount DOUBLE) USING DELTA")
      spark.sql("INSERT INTO rc_wh VALUES (1, 'Alpha', 0.1), (2, 'Beta', 0.05)")
      spark.sql(
      ...
      spark.sql("INSERT INTO rc_ol VALUES (1, 1, 30.0), (2, 1, 50.0)")
      ...
      spark.sql("INSERT INTO rc_ol VALUES (1, 3, 100.0), (2, 2, 40.0)")
      spark.sql("DELETE FROM rc_ol WHERE ol_w_id = 1 AND ol_d_id = 2")
      spark.sql("UPDATE rc_ol SET ol_amount = 999.0 WHERE ol_w_id = 2 AND ol_d_id = 1 AND ol_amount = 50.0")
      spark.sql("INSERT INTO rc_wh VALUES (3, 'Gamma', 0.15)")
      refreshMv("rc_avg_under_join")
      assertMvCorrect("rc_avg_under_join", viewBody)
    }
  }
  ...
  describe("(F) STDDEV / VARIANCE under JOIN — variance-family decomposition under join") {
    ...
    it("matches the base join-over-aggregate (rounded to 10 dp) after a mixed delta batch") {
      spark.sql("CREATE TABLE IF NOT EXISTS rc_wh_f(w_id INT, w_name STRING, w_tax DOUBLE) USING DELTA")
      spark.sql("CREATE TABLE IF NOT EXISTS rc_ol_f(ol_w_id INT, ol_d_id INT, ol_amount DOUBLE) USING DELTA")
      ...
      spark.sql("INSERT INTO rc_wh_f VALUES (1, 'Alpha', 0.1), (2, 'Beta', 0.05), (3, 'Gamma', 0.15)")
      spark.sql(
      ...
      spark.sql("INSERT INTO rc_ol_f VALUES (1, 4, 7.0), (1, 4, 9.0), (1, 4, 11.0)")
      spark.sql("DELETE FROM rc_ol_f WHERE ol_w_id = 2 AND ol_d_id = 2")
      spark.sql("UPDATE rc_ol_f SET ol_amount = 12.5 WHERE ol_w_id = 1 AND ol_d_id = 1 AND ol_amount = 30.0")
      spark.sql("INSERT INTO rc_wh_f VALUES (4, 'Delta', 0.2)")
      spark.sql("INSERT INTO rc_ol_f VALUES (4, 1, 50.0), (4, 1, 60.0), (4, 2, 100.0)")
