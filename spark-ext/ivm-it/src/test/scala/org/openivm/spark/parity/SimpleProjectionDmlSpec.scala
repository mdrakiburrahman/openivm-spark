...
/** DML slice of the SIMPLE_PROJECTION parity coverage (INSERT/DELETE/UPDATE,
  ...
  */
class SimpleProjectionDmlSpec extends AnyFunSpec with Matchers with BeforeAndAfterAll {

  private val warehouseDir: String = {
    val d = new File(s"target/test-warehouse-proj-dml-${UUID.randomUUID().toString.take(8)}")
    d.mkdirs()
    d.getAbsolutePath
  }
  ...
  override def beforeAll(): Unit = {
    ...
    spark = SparkSession
      .builder()
      .master("local[1]")
      .appName("openivm-spark-SimpleProjectionDmlSpec")
      .config(
