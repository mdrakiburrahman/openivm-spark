...
/** Repro for the SIMPLE_PROJECTION INSERT multiplicity-loss bug
  ...
  */
abstract class SimpleProjectionMultiplicityFanoutScenarios
    extends IvmParitySpecBase("simple-projection-multiplicity-fanout") {
  ...
  describe("(1) SIMPLE_PROJECTION JOIN with K=2 dim fan-out") {
    ...
    it("preserves multiplicity on incremental INSERT (mult=+2 → 2 copies)") {
      sql("CREATE TABLE IF NOT EXISTS spmf_src1(k INT, label STRING) USING DELTA")
      sql("CREATE TABLE IF NOT EXISTS spmf_dim1(k INT, dropped INT, kept INT) USING DELTA")
      sql(
      ...
      sql("INSERT INTO spmf_src1 VALUES (1, 'a')")
      sql("INSERT INTO spmf_dim1 VALUES (1, 10, 100), (1, 20, 100)")
      sql("REFRESH MATERIALIZED VIEW spmf_mv1").collect()
      ...
      sql("INSERT INTO spmf_src1 VALUES (2, 'b')")
      sql("INSERT INTO spmf_dim1 VALUES (2, 10, 200), (2, 20, 200)")
      sql("REFRESH MATERIALIZED VIEW spmf_mv1").collect()
  ...
  describe("(2) SIMPLE_PROJECTION update of a K=2-fanned-out key") {
    ...
    it("preserves multiplicity on update (mult=-2 retract + mult=+2 insert)") {
      sql("CREATE TABLE IF NOT EXISTS spmf_src2(k INT, label STRING) USING DELTA")
      sql("CREATE TABLE IF NOT EXISTS spmf_dim2(k INT, dropped INT, kept INT) USING DELTA")
      sql(
      ...
      sql("INSERT INTO spmf_src2 VALUES (1, 'a')")
      sql("INSERT INTO spmf_dim2 VALUES (1, 10, 100), (1, 20, 100)")
      sql("REFRESH MATERIALIZED VIEW spmf_mv2").collect()
      ...
      sql("DELETE FROM spmf_dim2 WHERE k = 1")
      sql("INSERT INTO spmf_dim2 VALUES (1, 10, 999), (1, 20, 999)")
      sql("REFRESH MATERIALIZED VIEW spmf_mv2").collect()
