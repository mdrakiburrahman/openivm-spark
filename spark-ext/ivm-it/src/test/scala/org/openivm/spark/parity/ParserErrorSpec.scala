...
/** Slice of `ParserSpec` covering the IvmParser's error surface: malformed
  ...
  */
class ParserErrorSpec extends AnyFunSpec with Matchers with BeforeAndAfterAll {

  private val warehouseDir: String = {
    val d = new File(s"target/test-warehouse-parser-error-${UUID.randomUUID().toString.take(8)}")
    d.mkdirs()
    d.getAbsolutePath
  }
  ...
  override def beforeAll(): Unit = {
    ...
    spark = SparkSession
      .builder()
      .master("local[1]")
      .appName("openivm-spark-ParserErrorSpec")
      .config(
  ...
  describe("(5) Error surface — malformed DDL and analyzer-time failures") {
    ...
    it("Typo 'MATERIALIZD' falls through to Spark's parser and fails") {
      ...
      a[ParseException] should be thrownBy {
        spark.sql("CREATE MATERIALIZD VIEW mv_typo AS SELECT 1")
      }
    }

    it("Typo 'VEIW' is rejected by the IvmParser grammar") {
      a[ParseException] should be thrownBy {
        spark.sql("CREATE MATERIALIZED VEIW mv_typo2 AS SELECT 1")
      }
    }
  ...
  describe("(6) Duplicate MV name") {
    ...
    it("a second CREATE for the same MV name fails and leaves the existing MV intact") {
      spark.sql("CREATE TABLE IF NOT EXISTS dup_p6(id INT, val INT) USING DELTA")
      spark.sql("INSERT INTO dup_p6 VALUES (1,10),(2,20)")
      spark.sql("CREATE MATERIALIZED VIEW mv_dup_p6 AS SELECT id, val FROM dup_p6")
      val before = spark.table("mv_dup_p6").collect().toSet

      val ex = the[AnalysisException] thrownBy {
        spark.sql("CREATE MATERIALIZED VIEW mv_dup_p6 AS SELECT id, val + 1 FROM dup_p6")
      }
  ...
  describe("(14) DROP MATERIALIZED VIEW IF EXISTS") {
    ...
    it("dropping a non-existent MV with IF EXISTS does not throw") {
      ...
      noException should be thrownBy {
        spark.sql("DROP MATERIALIZED VIEW IF EXISTS mv_never_existed_p14")
      }
    }
    ...
    it("dropping a non-existent MV WITHOUT IF EXISTS raises an analysis error") {
      ...
      an[AnalysisException] should be thrownBy {
        spark.sql("DROP MATERIALIZED VIEW mv_definitely_not_there_p14")
      }
    }
  }
}
