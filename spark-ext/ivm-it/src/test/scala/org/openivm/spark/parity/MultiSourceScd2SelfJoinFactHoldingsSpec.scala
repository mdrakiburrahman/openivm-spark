...
/** Repro for the ivm-bench SF=10 OAT failure on `gold.fact_holdings`:
  ...
  */
class MultiSourceScd2SelfJoinFactHoldingsSpec extends AnyFunSpec with Matchers with BeforeAndAfterAll {

  private val warehouseDir: String = {
    val d = new File(s"target/test-warehouse-msj-${UUID.randomUUID().toString.take(8)}")
    d.mkdirs()
    d.getAbsolutePath
  }
  ...
  override def beforeAll(): Unit = {
    ...
    spark = SparkSession
      .builder()
      .master("local[1]")
      .appName("openivm-spark-MultiSourceScd2SelfJoinFactHoldingsSpec")
      .config(
  ...
  private val FactHoldingsBody: String =
    """SELECT
  ...
  describe("Multi-source SCD2 self-join SIMPLE_PROJECTION (fact_holdings shape)") {
    ...
    it("preserves bag-correctness across batch 2 multi-source DML (holdings + dim_trade SCD2 + dim_account SCD2)") {
      ...
      spark.sql(s"CREATE MATERIALIZED VIEW msj_mv AS $FactHoldingsBody")
      ...
      spark.sql("REFRESH MATERIALIZED VIEW msj_mv").collect()
      assertMvCorrect("msj_mv", FactHoldingsBody)
    }
  }
}
