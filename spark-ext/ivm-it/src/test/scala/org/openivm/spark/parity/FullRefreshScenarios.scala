...
/** P5.rt3 — Comprehensive coverage of RefreshType 3 (FULL_REFRESH).
  ...
  */
abstract class FullRefreshScenarios extends IvmParitySpecBase("full-refresh") {
  ...
  describe("(1) INTERSECT ALL set operation → FULL_REFRESH (classifier: unsupported set op)") {
    ...
    it("classifier assigns FULL_REFRESH; INSERT+DELETE then REFRESH rewrites the MV correctly") {
      sql("CREATE TABLE IF NOT EXISTS sales_fr1(region STRING, amount INT) USING DELTA")
      sql("INSERT INTO sales_fr1 VALUES ('east', 100), ('west', 50), ('north', 200)")
      ...
      sql("INSERT INTO sales_fr1 VALUES ('south', 500)")
      sql("DELETE FROM sales_fr1 WHERE region = 'west'")
      refreshMv("mv_fr1")
  ...
  describe("(2) SEMI join (WHERE EXISTS) + GROUP BY aggregation → FULL_REFRESH (classifier: SEMI+aggregate)") {
    ...
    it("classifier assigns FULL_REFRESH; new promotion entry is reflected after REFRESH") {
      ...
      sql("INSERT INTO promotions_fr2 VALUES ('east'), ('west')")
  ...
  describe("(3) EXCEPT ALL set operation → FULL_REFRESH (classifier: unsupported set op)") {
    ...
    it("classifier assigns FULL_REFRESH; INSERT then REFRESH rewrites the MV correctly") {
      sql("CREATE TABLE IF NOT EXISTS sales_fr3(region STRING, amount INT) USING DELTA")
      ...
      sql("INSERT INTO sales_fr3 VALUES ('east', 150), ('west', 60), ('north', 210)")
      sql(
      ...
      sql("INSERT INTO sales_fr3 VALUES ('south', 500), ('east', 80)")
      refreshMv("mv_fr3")
  ...
  describe("(5) Volatile function random() → FULL_REFRESH (classifier: HasVolatileExpression)") {
    ...
    it("classifier assigns FULL_REFRESH; MV row count matches base table after REFRESH") {
      sql("CREATE TABLE IF NOT EXISTS sales_fr5(region STRING, amount INT) USING DELTA")
      sql("INSERT INTO sales_fr5 VALUES ('east', 100), ('west', 200)")
      sql(
      ...
      sql("INSERT INTO sales_fr5 VALUES ('north', 300)")
      refreshMv("mv_fr5")
  ...
  describe("(6) Mixed inventory: two AGGREGATE_GROUP + one FULL_REFRESH MV in the same session") {
    ...
    it("each MV gets its own refresh path; all three are correct after REFRESH") {
      sql("CREATE TABLE IF NOT EXISTS sales_fr6(region STRING, amount INT) USING DELTA")
      sql("INSERT INTO sales_fr6 VALUES ('east', 100), ('west', 200), ('north', 50)")
      ...
      sql("INSERT INTO sales_fr6 VALUES ('south', 400), ('east', 75)")
      sql("DELETE FROM sales_fr6 WHERE region = 'north'")
  ...
  describe("(7) FULL_REFRESH performance baseline: 10,000-row base table") {
    ...
    it("REFRESH completes in under 60 seconds and the MV is correct") {
      sql("CREATE TABLE IF NOT EXISTS sales_fr7(region STRING, amount INT) USING DELTA")
      val rows = (1 to 10000).map(i => s"('region_${i % 20}', $i)").mkString(", ")
      sql(s"INSERT INTO sales_fr7 VALUES $rows")
      // FULL_REFRESH via EXCEPT ALL set operation (same path as test 3).
      sql(
      ...
      sql("INSERT INTO sales_fr7 VALUES ('region_0', 99999)")
  ...
  describe("(8) FULL_REFRESH after simulated partial staging failure") {
    ...
    itIntercept("corrupt staging entry does not affect correctness — FULL_REFRESH rewrites from base") {
      sql("CREATE TABLE IF NOT EXISTS sales_fr8(region STRING, amount INT) USING DELTA")
      sql("INSERT INTO sales_fr8 VALUES ('east', 100), ('west', 200)")
      // FULL_REFRESH via EXCEPT ALL set operation (same path as test 3).
      sql(
      ...
      sql("INSERT INTO sales_fr8 VALUES ('north', 50)")
