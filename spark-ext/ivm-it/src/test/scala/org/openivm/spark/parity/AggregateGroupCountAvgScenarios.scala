...
/** Split-off from `AggregateGroupSpec.scala` so that each chunk of ~8 tests
  ...
  */
abstract class AggregateGroupCountAvgScenarios extends IvmParitySpecBase("aggregate-group-count-avg") {
  self: org.openivm.spark.parity.base.IvmParityMode =>

  describe("(2a) GROUP BY k, COUNT(*) — INSERT only") {
    it("incremental refresh yields correct counts after INSERT") {
      sql("CREATE TABLE IF NOT EXISTS aggrpca_ag_cnt_2a(k STRING, x INT) USING DELTA")
      sql("INSERT INTO aggrpca_ag_cnt_2a VALUES ('a', 1), ('a', 2), ('b', 3)")
      sql(
        "CREATE MATERIALIZED VIEW aggrpca_mv_ag_cnt_2a AS " +
          "SELECT k, COUNT(*) AS cnt FROM aggrpca_ag_cnt_2a GROUP BY k"
      )
      sql("INSERT INTO aggrpca_ag_cnt_2a VALUES ('a', 10), ('c', 7)")
  ...
  describe("(2b) GROUP BY k, COUNT(*) — DELETE") {
    it("incremental refresh decrements count after row deletion") {
      sql("CREATE TABLE IF NOT EXISTS aggrpca_ag_cnt_2b(k STRING, x INT) USING DELTA")
      sql("INSERT INTO aggrpca_ag_cnt_2b VALUES ('a', 1), ('a', 2), ('b', 3), ('b', 4)")
      sql(
        "CREATE MATERIALIZED VIEW aggrpca_mv_ag_cnt_2b AS " +
          "SELECT k, COUNT(*) AS cnt FROM aggrpca_ag_cnt_2b GROUP BY k"
      )
      sql("DELETE FROM aggrpca_ag_cnt_2b WHERE x = 1")
  ...
  describe("(2c) GROUP BY k, COUNT(*) — UPDATE") {
    it("incremental refresh adjusts count correctly after UPDATE") {
      sql("CREATE TABLE IF NOT EXISTS aggrpca_ag_cnt_2c(k STRING, x INT) USING DELTA")
      sql("INSERT INTO aggrpca_ag_cnt_2c VALUES ('a', 1), ('a', 2), ('b', 3)")
      sql(
      ...
      sql("UPDATE aggrpca_ag_cnt_2c SET k = 'c' WHERE x = 2")
  ...
  describe("(2d) GROUP BY k, COUNT(*) — batched mixed DML") {
    it("incremental refresh reconciles mixed DML for COUNT(*)") {
      sql("CREATE TABLE IF NOT EXISTS aggrpca_ag_cnt_2d(k STRING, x INT) USING DELTA")
      sql("INSERT INTO aggrpca_ag_cnt_2d VALUES ('a', 1), ('b', 2), ('c', 3)")
      sql(
        "CREATE MATERIALIZED VIEW aggrpca_mv_ag_cnt_2d AS " +
          "SELECT k, COUNT(*) AS cnt FROM aggrpca_ag_cnt_2d GROUP BY k"
      )
      sql("INSERT INTO aggrpca_ag_cnt_2d VALUES ('a', 99), ('d', 77)")
      sql("DELETE FROM aggrpca_ag_cnt_2d WHERE k = 'c'")
      sql("UPDATE aggrpca_ag_cnt_2d SET k = 'b' WHERE x = 99")
  ...
  describe("(3) GROUP BY k, COUNT(x) — with nullable column") {
    it("COUNT(x) ignores NULLs; incremental refresh tracks correctly") {
      sql("CREATE TABLE IF NOT EXISTS aggrpca_ag_cntx_3(k STRING, x INT) USING DELTA")
      sql("INSERT INTO aggrpca_ag_cntx_3 VALUES ('a', 10), ('a', NULL), ('b', 20)")
      sql(
        "CREATE MATERIALIZED VIEW aggrpca_mv_ag_cntx_3 AS " +
          "SELECT k, COUNT(x) AS cnt_x FROM aggrpca_ag_cntx_3 GROUP BY k"
      )
      sql("INSERT INTO aggrpca_ag_cntx_3 VALUES ('a', NULL), ('b', 5), ('c', NULL)")
      sql("DELETE FROM aggrpca_ag_cntx_3 WHERE k = 'a' AND x IS NULL")
  ...
  describe("(4a) GROUP BY k, AVG(x) — INSERT") {
    it("AVG is maintained via hidden SUM/COUNT decomposition") {
      sql("CREATE TABLE IF NOT EXISTS aggrpca_ag_avg_4a(k STRING, x DOUBLE) USING DELTA")
      sql("INSERT INTO aggrpca_ag_avg_4a VALUES ('a', 10.0), ('a', 20.0), ('b', 5.0)")
      sql(
        "CREATE MATERIALIZED VIEW aggrpca_mv_ag_avg_4a AS " +
          "SELECT k, AVG(x) AS avg_x FROM aggrpca_ag_avg_4a GROUP BY k"
      )
      sql("INSERT INTO aggrpca_ag_avg_4a VALUES ('a', 30.0), ('c', 15.0)")
  ...
  describe("(4b) GROUP BY k, AVG(x) — DELETE") {
    it("AVG is correctly recalculated after DELETE") {
      sql("CREATE TABLE IF NOT EXISTS aggrpca_ag_avg_4b(k STRING, x DOUBLE) USING DELTA")
      sql("INSERT INTO aggrpca_ag_avg_4b VALUES ('a', 10.0), ('a', 20.0), ('a', 30.0), ('b', 5.0)")
      sql(
        "CREATE MATERIALIZED VIEW aggrpca_mv_ag_avg_4b AS " +
          "SELECT k, AVG(x) AS avg_x FROM aggrpca_ag_avg_4b GROUP BY k"
      )
      sql("DELETE FROM aggrpca_ag_avg_4b WHERE x = 10.0")
  ...
  describe("(4c) GROUP BY k, AVG(x) — DELETE + INSERT with unequal counts") {
    ...
    it("AVG is correctly recalculated after asymmetric DELETE + INSERT") {
      sql("CREATE TABLE IF NOT EXISTS aggrpca_ag_avg_4c(k STRING, x DOUBLE) USING DELTA")
      sql("INSERT INTO aggrpca_ag_avg_4c VALUES ('a', 10.0), ('a', 20.0), ('b', 5.0)")
      sql(
      ...
      sql("DELETE FROM aggrpca_ag_avg_4c WHERE k = 'a' AND x = 20.0")
      sql("INSERT INTO aggrpca_ag_avg_4c VALUES ('a', 30.0), ('a', 40.0)")
  ...
  describe("(12) NULL group key — NULL is a valid group") {
    it("INSERT (NULL, 5) creates a row with NULL key and correct aggregate") {
      sql("CREATE TABLE IF NOT EXISTS aggrpca_ag_nullk_12(k STRING, x INT) USING DELTA")
      sql("INSERT INTO aggrpca_ag_nullk_12 VALUES ('a', 10), (NULL, 5)")
      sql(
        "CREATE MATERIALIZED VIEW aggrpca_mv_ag_nullk_12 AS " +
          "SELECT k, SUM(x) AS total FROM aggrpca_ag_nullk_12 GROUP BY k"
      )
      sql("INSERT INTO aggrpca_ag_nullk_12 VALUES (NULL, 15), ('b', 7)")
