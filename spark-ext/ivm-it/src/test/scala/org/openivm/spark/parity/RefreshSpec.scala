...
/** Parity port of `openivm/test/sql/refresh.test`.
  ...
  */
class RefreshSpec extends AnyFunSpec with Matchers with BeforeAndAfterAll {

  private val warehouseDir: String = {
    val d = new File(s"target/test-warehouse-refresh-${UUID.randomUUID().toString.take(8)}")
    d.mkdirs()
    d.getAbsolutePath
  }
  ...
  override def beforeAll(): Unit = {
    ...
    spark = SparkSession
      .builder()
      .master("local[1]")
      .appName("openivm-spark-RefreshSpec")
      .config(
  ...
  describe("(1) Idempotency — REFRESH on a freshly-created MV is a no-op") {
    it("calling REFRESH twice in a row leaves MV bag-equal to the view body") {
      spark.sql("CREATE TABLE IF NOT EXISTS sales_r1(region STRING, amount INT) USING DELTA")
      spark.sql("INSERT INTO sales_r1 VALUES ('east', 100), ('west', 200)")
  ...
  describe("(2) Single REFRESH after INSERT applies the staged delta") {
    it("MV reflects the new row after one REFRESH") {
      spark.sql("CREATE TABLE IF NOT EXISTS sales_r2(region STRING, amount INT) USING DELTA")
      spark.sql("INSERT INTO sales_r2 VALUES ('east', 100), ('west', 200)")
      ...
      spark.sql("INSERT INTO sales_r2 VALUES ('east', 50)")
      refreshMv("mv_r2")
  ...
  describe("(3) Multi-call REFRESH safety — replaying does not double-apply") {
    it("ten consecutive REFRESH calls after a single INSERT leave the MV correct exactly once") {
      spark.sql("CREATE TABLE IF NOT EXISTS orders_r3(id INT, qty INT) USING DELTA")
      spark.sql("INSERT INTO orders_r3 VALUES (1, 10), (2, 20)")

      spark.sql(
        "CREATE MATERIALIZED VIEW mv_r3 AS SELECT SUM(qty) AS total, COUNT(*) AS cnt FROM orders_r3"
      )

      spark.sql("INSERT INTO orders_r3 VALUES (3, 30)")
  ...
  describe("(4) REFRESH with no pending staging entries is a no-op") {
    it("REFRESH right after CREATE does not raise and leaves the MV correct") {
      spark.sql("CREATE TABLE IF NOT EXISTS sales_r4(region STRING, amount INT) USING DELTA")
      spark.sql("INSERT INTO sales_r4 VALUES ('east', 100)")
  ...
  describe("(5) Multiple MVs side-by-side — refreshing one does not affect the other") {
    it("REFRESH on mv_fast does not change mv_slow when only orders_r5 changed") {
      spark.sql("CREATE TABLE IF NOT EXISTS orders_r5(id INT, qty INT) USING DELTA")
      spark.sql("INSERT INTO orders_r5 VALUES (1, 10), (2, 20)")

      spark.sql(
        "CREATE MATERIALIZED VIEW mv_r5_fast AS SELECT SUM(qty) AS total, COUNT(*) AS cnt FROM orders_r5"
      )
      spark.sql("CREATE MATERIALIZED VIEW mv_r5_slow AS SELECT id, qty FROM orders_r5")

      spark.sql("INSERT INTO orders_r5 VALUES (3, 30)")
  ...
  describe("(6) Crash-recovery analogue — many batched DML ops, then a single REFRESH") {
    it("multiple unrefreshed batches → single REFRESH → MV is correct (full delta consolidation)") {
      spark.sql("CREATE TABLE IF NOT EXISTS crash_r6(id INT, val INT) USING DELTA")
      spark.sql(
      ...
      // Batch 2 — DELETE
      spark.sql("DELETE FROM crash_r6 WHERE id = 1")
      // Batch 3 — UPDATE
      spark.sql("UPDATE crash_r6 SET val = 999 WHERE id = 2")
      // Batch 4 — more inserts
      spark.sql("INSERT INTO crash_r6 VALUES (16,160),(17,170),(18,180)")
      ...
      spark.sql("INSERT INTO crash_r6 VALUES (19,190),(20,200)")
      refreshMv("mv_r6")
      assertMvCorrect(
        "mv_r6",
        "SELECT val, COUNT(*) AS cnt FROM crash_r6 GROUP BY val"
  ...
  describe("(7) Out-of-scope: REFRESH EVERY / ALTER REFRESH MANUAL (PLAN §12)") {
    ...
    it("manual REFRESH on a non-scheduled MV always works (REFRESH EVERY analogue)") {
      ...
      spark.sql("CREATE TABLE IF NOT EXISTS sales_r7(region STRING, amount INT) USING DELTA")
      spark.sql("INSERT INTO sales_r7 VALUES ('east', 100), ('west', 200)")
      ...
      spark.sql("INSERT INTO sales_r7 VALUES ('east', 50)")
      ...
      spark.sql("INSERT INTO sales_r7 VALUES ('north', 500)")
      refreshMv("mv_r7_manual")
      assertMvCorrect(
        "mv_r7_manual",
        "SELECT region, SUM(amount) AS total, COUNT(*) AS cnt FROM sales_r7 GROUP BY region"
