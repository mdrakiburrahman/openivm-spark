...
/** P6f — second half of the 1:1 ScalaTest port of openivm `test/sql/cte.test`.
  ...
  * for §1–§8.
  */
class CteOpenIvmDmlSpec extends AnyFunSpec with Matchers with BeforeAndAfterAll {

  private val warehouseDir: String = {
    val d = new File(s"target/test-warehouse-cte-openivm-dml-${UUID.randomUUID().toString.take(8)}")
    d.mkdirs()
    d.getAbsolutePath
  }
  ...
  override def beforeAll(): Unit = {
    ...
    spark = SparkSession
      .builder()
      .master("local[1]")
      .appName("openivm-spark-CteOpenIvmDmlSpec")
      .config(
  ...
  describe(
    ...
    it("STDDEV+AVG aggregate over CTE remains equal to the recomputed query after mixed UPDATEs") {
      ...
      spark.sql("CREATE TABLE cte_joinagg_wh (w INT, tax DOUBLE) USING DELTA")
      spark.sql(
        "INSERT INTO cte_joinagg_customer VALUES " +
          "(1, 'GC', 10), (1, 'GC', 20), (1, 'BC', 50), " +
          "(2, 'GC', 100), (2, 'GC', 130)"
      )
      spark.sql("INSERT INTO cte_joinagg_wh VALUES (1, 0.05), (2, 0.10)")
      spark.sql(
      ...
      spark.sql("UPDATE cte_joinagg_wh SET tax = 0.15 WHERE w = 2")
  ...
  describe("openivm/cte §10: HAVING inside CTE body (cte_hav_item / cte_hav_ol / mv_cte_having)") {
    it("CTE-internal HAVING is preserved after batched mixed DML refresh") {
      ...
      spark.sql("INSERT INTO cte_hav_item VALUES (1, 'a', 10), (2, 'b', 20), (3, 'c', 30)")
      spark.sql(
      ...
      spark.sql("DELETE FROM cte_hav_ol WHERE ol_i_id = 1 AND ol_amount = 60")
      spark.sql("INSERT INTO cte_hav_ol VALUES (2, 80)")
      spark.sql("INSERT INTO cte_hav_item VALUES (4, 'd', 40)")
      spark.sql("INSERT INTO cte_hav_ol VALUES (4, 150)")
  ...
  describe("openivm/cte §12: CTE self-join regression (selfjoin_t / selfjoin_mv)") {
    it("self-join under a CTE produces correct cross-pair deltas on single-row INSERT and mixed DML") {
      spark.sql("CREATE TABLE selfjoin_t (id INT, k INT, v INT) USING DELTA")
      spark.sql("INSERT INTO selfjoin_t VALUES (1,1,10), (2,1,20), (3,2,30), (4,2,40)")
      spark.sql(
      ...
      spark.sql("INSERT INTO selfjoin_t VALUES (5, 1, 50)")
      refreshMv("selfjoin_mv")
      assertMvCorrect("selfjoin_mv", expected)

      // Mixed batch: new row in another group + delete an existing row.
      spark.sql("INSERT INTO selfjoin_t VALUES (6, 2, 60)")
      spark.sql("DELETE FROM selfjoin_t WHERE id = 1")
      refreshMv("selfjoin_mv")
      assertMvCorrect("selfjoin_mv", expected)
    }
  }
  ...
  describe("openivm/cte §13: JOIN with inner aggregate subquery (jia_customer / jia_history / mv_jia)") {
    it("join-key-grouped aggregate stays correct across two refresh cycles with mixed DML") {
      spark.sql("CREATE TABLE jia_customer (c_id INT, c_w_id INT, c_balance DOUBLE) USING DELTA")
      spark.sql("CREATE TABLE jia_history (h_c_id INT, h_amount DOUBLE) USING DELTA")
      spark.sql("INSERT INTO jia_customer VALUES (1, 1, 50.0), (2, 1, 100.0), (3, 2, 75.0)")
      spark.sql("INSERT INTO jia_history VALUES (1, 10.0), (1, 20.0), (2, 5.0), (3, 30.0)")
      spark.sql(
      ...
      spark.sql("INSERT INTO jia_history VALUES (2, 15.0)")
      spark.sql("DELETE FROM jia_customer WHERE c_id = 3")
      refreshMv("mv_jia")
      assertMvCorrect("mv_jia", expected)

      // Second refresh: insert history (affects avg) + insert new customer row
      spark.sql("INSERT INTO jia_history VALUES (1, 30.0)")
      spark.sql("INSERT INTO jia_customer VALUES (4, 2, 200.0)")
      refreshMv("mv_jia")
      assertMvCorrect("mv_jia", expected)
    }
  }
  ...
  describe("openivm/cte §14: re-aggregate over a CTE aggregate (cra_customer / mv_cra)") {
    it("outer GROUP BY over inner CTE SUM/COUNT stays equal under inserts and deletes") {
      spark.sql("CREATE TABLE cra_customer (c_w_id INT, c_d_id INT, c_balance DOUBLE) USING DELTA")
      spark.sql(
      ...
      spark.sql("INSERT INTO cra_customer VALUES (1, 3, 999.0), (2, 1, 25.0)")
      refreshMv("mv_cra")
      assertMvCorrect("mv_cra", expected)

      // Second refresh: DELETE + INSERT (lines 1045–1073 of cte.test)
      spark.sql("DELETE FROM cra_customer WHERE c_d_id = 3")
      spark.sql("INSERT INTO cra_customer VALUES (3, 1, 500.0)")
      refreshMv("mv_cra")
      assertMvCorrect("mv_cra", expected)
    }
  }
  ...
  describe("openivm/cte §15: nested CTE aggregate (fgc_customer / mv_fgc)") {
    ignore(
      ...
      spark.sql("CREATE TABLE fgc_customer (c_w_id INT, c_id INT, c_balance DOUBLE) USING DELTA")
      spark.sql("INSERT INTO fgc_customer VALUES (1, 1, 100.0), (2, 1, -50.0), (3, 1, 10.0)")
      spark.sql(
      ...
      spark.sql("INSERT INTO fgc_customer VALUES (1, 2, -200.0)")
      spark.sql("DELETE FROM fgc_customer WHERE c_w_id = 2 AND c_id = 1")
      spark.sql("UPDATE fgc_customer SET c_balance = -20.0 WHERE c_w_id = 3 AND c_id = 1")
      refreshMv("mv_fgc")
  ...
  describe(
    ...
    it("two stacked CTE aggregates joined to two dimension tables refresh correctly after UPDATE+INSERT") {
      spark.sql("CREATE TABLE cte_wh (w_id INT, w_name STRING, w_ytd DOUBLE) USING DELTA")
      spark.sql("CREATE TABLE cte_dist (d_w_id INT, d_id INT, d_name STRING) USING DELTA")
      spark.sql(
        "CREATE TABLE cte_ol (ol_w_id INT, ol_d_id INT, ol_o_id INT, ol_number INT, ol_amount DOUBLE) USING DELTA"
      )
      spark.sql("INSERT INTO cte_wh VALUES (1, 'w1', 100.0), (2, 'w2', 200.0)")
      spark.sql("INSERT INTO cte_dist VALUES (1, 1, 'd1'), (1, 2, 'd2'), (2, 1, 'd3')")
      spark.sql(
      ...
      spark.sql("INSERT INTO cte_ol VALUES (2, 1, 4, 1, 5.0)")
