...
abstract class WindowLastValueSnapshotCacheScenarios extends IvmParitySpecBase("window-last-value-cache") {
  ...
  describe("dim_customer-shaped LAST_VALUE forward-fill WINDOW_PARTITION") {
    it("uses incremental WINDOW_PARTITION with cached single-pass snapshot and stays bag-equal") {
      ...
      val rewrittenRows = sql("SHOW OPENIVM QUERY LOG")
        .collect()
        .toSeq
        .filter(_.getString(1).split("\\.").last == "dimc_mv")
        .filter(_.getString(6) == "rewritten_stmt")
        .sortBy(_.getInt(3))
      val cacheRow = rewrittenRows
        .find(_.getString(9).equalsIgnoreCase("CACHE TABLE `openivm_new_dimc_mv`"))
        .getOrElse(fail("missing window snapshot CACHE TABLE statement"))
      val targetRows = rewrittenRows.filter { row =>
        val upper = row.getString(9).toUpperCase(java.util.Locale.ROOT)
      ...
      targetRow.getString(9).toUpperCase(java.util.Locale.ROOT) should include("FROM `OPENIVM_NEW_DIMC_MV`")
      targetRow.getString(9) should not include "openivm_multiplicity"
      cacheRow.getInt(3) should be < targetRow.getInt(3)
    }
  }
}
