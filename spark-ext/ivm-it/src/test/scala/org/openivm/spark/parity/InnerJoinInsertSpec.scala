...
/** Insert-and-multi-table slice of the openivm `inner_join.test` parity port.
  ...
  */
class InnerJoinInsertSpec extends AnyFunSpec with Matchers with BeforeAndAfterAll {

  private val warehouseDir: String = {
    val d = new File(s"target/test-warehouse-ij-insert-${UUID.randomUUID().toString.take(8)}")
    d.mkdirs()
    d.getAbsolutePath
  }
  ...
  override def beforeAll(): Unit = {
    ...
    spark = SparkSession
      .builder()
      .master("local[1]")
      .appName("openivm-spark-InnerJoinInsertSpec")
      .config(
  ...
  describe("(2) god_locations — 3-table INNER JOIN (gods ⨝ god_cities ⨝ cities)") {
    it("maintains 3-way join across inserts to middle/rightmost tables and middle-table deletes") {
      ...
      spark.sql("CREATE TABLE IF NOT EXISTS gods (uid INT, user_name STRING) USING DELTA")
      spark.sql(
        "INSERT INTO gods VALUES (1, 'Apollo'), (2, 'Artemis'), (3, 'Dionysus'), (4, 'Poseidon'), (5, 'Zeus')"
      )

      spark.sql("CREATE TABLE IF NOT EXISTS cities (city_id INT, city_name STRING) USING DELTA")
      spark.sql("INSERT INTO cities VALUES (1, 'Athens'), (2, 'Sparta'), (3, 'Thebes')")

      spark.sql("CREATE TABLE IF NOT EXISTS god_cities (uid INT, city_id INT) USING DELTA")
      spark.sql("INSERT INTO god_cities VALUES (1, 1), (2, 2), (3, 1), (5, 3)")
      ...
      spark.sql("INSERT INTO god_cities VALUES (4, 2)")
      refreshMv("god_locations")
      assertMvCorrect("god_locations", viewBody)

      // Insert into rightmost table (cities) + link table — Apollo also in Corinth
      spark.sql("INSERT INTO cities VALUES (4, 'Corinth')")
      spark.sql("INSERT INTO god_cities VALUES (1, 4)")
      refreshMv("god_locations")
      assertMvCorrect("god_locations", viewBody)

      // Delete from middle table — Poseidon/Sparta link gone
      spark.sql("DELETE FROM god_cities WHERE uid = 4 AND city_id = 2")
      refreshMv("god_locations")
      assertMvCorrect("god_locations", viewBody)
    }
  }
  ...
  describe("(3) order_totals — multi-column INNER JOIN on (region, product) with DECIMAL prices") {
    it("incrementally maintains joins on composite keys with decimal-typed columns") {
      ...
      spark.sql("INSERT INTO prices VALUES ('US', 'A', 10.00), ('US', 'B', 20.00), ('EU', 'A', 12.00)")
      spark.sql("INSERT INTO orders_j VALUES ('US', 'A', 5), ('US', 'B', 3), ('EU', 'A', 10)")
      ...
      spark.sql("INSERT INTO orders_j VALUES ('US', 'A', 2), ('EU', 'A', 7)")
      refreshMv("order_totals")
      assertMvCorrect("order_totals", viewBody)

      // Delete from join
      spark.sql("DELETE FROM orders_j WHERE region = 'US' AND product = 'A' AND qty = 5")
      refreshMv("order_totals")
      assertMvCorrect("order_totals", viewBody)
    }
  }
  ...
  describe("(7) mv_4way — 4-table INNER JOIN (products × inventory × warehouses × suppliers)") {
    it("incrementally maintains a 4-way join (15 Möbius terms) across batched multi-table DML") {
      spark.sql("CREATE TABLE IF NOT EXISTS t4_products (pid INT, pname STRING) USING DELTA")
      spark.sql("CREATE TABLE IF NOT EXISTS t4_warehouses (wid INT, wname STRING) USING DELTA")
      spark.sql("CREATE TABLE IF NOT EXISTS t4_inventory (pid INT, wid INT, qty INT) USING DELTA")
      spark.sql("CREATE TABLE IF NOT EXISTS t4_suppliers (sid INT, pid INT, sname STRING) USING DELTA")

      spark.sql("INSERT INTO t4_products VALUES (1, 'Widget'), (2, 'Gadget')")
      spark.sql("INSERT INTO t4_warehouses VALUES (10, 'East'), (20, 'West')")
      spark.sql("INSERT INTO t4_inventory VALUES (1, 10, 100), (2, 20, 50)")
      spark.sql("INSERT INTO t4_suppliers VALUES (100, 1, 'Acme'), (200, 2, 'Globex')")
      ...
      spark.sql("INSERT INTO t4_inventory VALUES (1, 20, 30)")
      refreshMv("mv_4way")
      assertMvCorrect("mv_4way", viewBody)

      // Insert new product + supplier + inventory all at once (3-table batched insert)
      spark.sql("INSERT INTO t4_products VALUES (3, 'Doohickey')")
      spark.sql("INSERT INTO t4_suppliers VALUES (300, 3, 'Initech')")
      spark.sql("INSERT INTO t4_inventory VALUES (3, 10, 75)")
      refreshMv("mv_4way")
      assertMvCorrect("mv_4way", viewBody)

      // Delete from the middle (inventory) — should remove all related rows
      spark.sql("DELETE FROM t4_inventory WHERE pid = 1 AND wid = 10")
      refreshMv("mv_4way")
      assertMvCorrect("mv_4way", viewBody)
    }
  }
  ...
  describe("(10) mv_any_join — LOGICAL_ANY_JOIN: CROSS JOIN ... WHERE equi-predicate") {
    it("CROSS JOIN with a WHERE predicate refreshes incrementally across batched DML") {
      spark.sql("CREATE TABLE IF NOT EXISTS any_w (W_ID INT) USING DELTA")
      spark.sql("CREATE TABLE IF NOT EXISTS any_d (D_ID INT, D_W_ID INT) USING DELTA")
      spark.sql("INSERT INTO any_w VALUES (1), (2)")
      spark.sql("INSERT INTO any_d VALUES (10, 1), (20, 1), (30, 2)")
      ...
      spark.sql("INSERT INTO any_w VALUES (3)")
      spark.sql("INSERT INTO any_d VALUES (40, 3), (50, 2)")
      spark.sql("DELETE FROM any_d WHERE D_ID = 10")
  ...
  describe("(11) mv_cross_join — plain CROSS JOIN (no predicate)") {
    it("Cartesian product is bag-equal across one-sided inserts") {
      ...
      spark.sql("INSERT INTO any_w VALUES (4)")
  ...
  describe("(13) mv_skip_join_key — inserts on unmatched join keys leave MV bag-equal") {
    it("inserting rows whose join key matches nothing on the other side does not change the MV body") {
      spark.sql("CREATE TABLE IF NOT EXISTS skj_left (id INT, v INT) USING DELTA")
      spark.sql("CREATE TABLE IF NOT EXISTS skj_right (id INT, w INT) USING DELTA")
      spark.sql("INSERT INTO skj_left VALUES (1, 10)")
      spark.sql("INSERT INTO skj_right VALUES (1, 100)")
      ...
      spark.sql("INSERT INTO skj_left VALUES (999, 999)")
      refreshMv("mv_skip_join_key")
      assertMvCorrect("mv_skip_join_key", viewBody)

      // Both-sided inserts with no overlapping keys
      spark.sql("INSERT INTO skj_left VALUES (1000, 1000)")
      spark.sql("INSERT INTO skj_right VALUES (2000, 2000)")
      refreshMv("mv_skip_join_key")
      assertMvCorrect("mv_skip_join_key", viewBody)
    }
  }
