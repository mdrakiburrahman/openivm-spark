...
/** Split from the original parity spec.  Scope:
  ...
  */
class MetadataMvCatalogSpec extends AnyFunSpec with Matchers with BeforeAndAfterAll {

  private val warehouseDir: String = {
    val d = new File(s"target/test-warehouse-meta-mvc-${UUID.randomUUID().toString.take(8)}")
    d.mkdirs()
    d.getAbsolutePath
  }
  ...
  override def beforeAll(): Unit = {
    ...
    spark = SparkSession
      .builder()
      .master("local[1]")
      .appName("openivm-spark-MetadataMvCatalogSpec")
      .config(
  ...
  describe("(1) MV catalog state after CREATE MATERIALIZED VIEW") {
    ...
    it("populates mv_metadata with name, refreshType, sourceTables, schema fingerprint") {
      spark.sql("CREATE TABLE IF NOT EXISTS t1_mvc1(id INT, grp STRING, val INT) USING DELTA")
      spark.sql("INSERT INTO t1_mvc1 VALUES (1,'a',10), (2,'b',20)")
      spark.sql(
    ...
    it("two MVs over different sources yield two distinct rows in MvCatalog") {
      spark.sql("CREATE TABLE IF NOT EXISTS t2_mvc1b(id INT, name STRING) USING DELTA")
      spark.sql("INSERT INTO t2_mvc1b VALUES (1,'alice'),(2,'bob')")
      spark.sql("CREATE MATERIALIZED VIEW mv2_mvc1b AS SELECT id, name FROM t2_mvc1b")
    ...
    it("the MV is queryable through Spark's catalog (registered table)") {
      ...
      val rows = spark.sql("SELECT grp, total FROM mv1_mvc1 ORDER BY grp").collect()
  ...
  describe("(5) DROP MATERIALIZED VIEW cleanup") {
    ...
    it("removes the MvCatalog row, deletes the Delta storage, drops the catalog table") {
      spark.sql("CREATE TABLE IF NOT EXISTS t6_mvc5(id INT, val INT) USING DELTA")
      spark.sql("INSERT INTO t6_mvc5 VALUES (1, 10)")
      spark.sql("CREATE MATERIALIZED VIEW mv6_mvc5 AS SELECT id, val FROM t6_mvc5")

      val meta = lookup("mv6_mvc5").get
      val dir  = fileOf(meta.location)
      dir.exists() shouldBe true

      spark.sql("DROP MATERIALIZED VIEW mv6_mvc5")
    ...
    it("DROP without IF EXISTS on a missing MV raises an AnalysisException") {
      an[AnalysisException] should be thrownBy {
        spark.sql("DROP MATERIALIZED VIEW mv_does_not_exist_mvc5")
      }
    }

    it("DROP IF EXISTS on a missing MV is a no-op") {
      noException should be thrownBy {
        spark.sql("DROP MATERIALIZED VIEW IF EXISTS mv_does_not_exist_mvc5")
      }
    }
  }
  ...
  describe("(7) Re-create an MV with the same name after DROP") {
    ...
    it("a CREATE after DROP succeeds and registers a fresh catalog row") {
      spark.sql("CREATE TABLE IF NOT EXISTS t7_mvc7(id INT, val INT) USING DELTA")
      spark.sql("INSERT INTO t7_mvc7 VALUES (1, 100)")
      spark.sql("CREATE MATERIALIZED VIEW mv7_mvc7 AS SELECT id, val FROM t7_mvc7")
      lookup("mv7_mvc7").isDefined shouldBe true

      spark.sql("DROP MATERIALIZED VIEW mv7_mvc7")
      ...
      spark.sql("SELECT cnt FROM mv7_mvc7").collect().head.getAs[Long](0) shouldBe 1L
    }
  }
}
