...
/** P6d — Port of `openivm/test/sql/ducklake_chained_physical.test`.
  ...
  */
class DucklakeChainedPhysicalSpec extends AnyFunSpec with Matchers with BeforeAndAfterAll {

  private val warehouseDir: String = {
    val d = new File(s"target/test-warehouse-dl-cp-${UUID.randomUUID().toString.take(8)}")
    d.mkdirs()
    d.getAbsolutePath
  }
  ...
  override def beforeAll(): Unit = {
    ...
    spark = SparkSession
      .builder()
      .master("local[1]")
      .appName("openivm-spark-DucklakeChainedPhysicalSpec")
      .config(
  ...
  describe("(DChP-1) Depth-2 chain through a WINDOW MV") {
    it("l1 (projection) reflects src; l2 (window over l1) tracks via CTAS re-creation") {
      ...
      spark.sql(s"CREATE MATERIALIZED VIEW dl_silver_l2 AS $l2Body")
      ...
      spark.sql("INSERT INTO dl_bronze_src VALUES (DATE '2026-01-03', 'A', 7, 25)")
      ...
      spark.sql("DROP MATERIALIZED VIEW dl_silver_l2")
      spark.sql(s"CREATE MATERIALIZED VIEW dl_silver_l2 AS $l2Body")
      assertMvCorrect(
        "dl_silver_l2",
        """SELECT
