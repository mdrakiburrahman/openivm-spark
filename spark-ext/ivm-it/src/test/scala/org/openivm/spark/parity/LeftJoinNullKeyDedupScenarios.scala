...
/** Regression spec for the Cartesian-on-NULL MERGE explosion in
  ...
  */
abstract class LeftJoinNullKeyDedupScenarios extends IvmParitySpecBase("left-join-null-key-dedup") {
  ...
  describe("LEFT JOIN MV with NULL left-key and duplicate-key view-delta (Cartesian-dedup fix)") {
    it("retracts and re-inserts NULL-keyed and hot-keyed rows without Cartesian explosion") {
      ...
      sql(s"CREATE MATERIALIZED VIEW ljnkd_mv AS $viewBody")
      ...
      sql(s"INSERT INTO ljnkd_financials VALUES $dupRowsForKey10")
      sql(s"INSERT INTO ljnkd_financials VALUES $dupRowsForNull")

      refreshMv("ljnkd_mv")
      assertMvCorrect("ljnkd_mv", viewBody)

      // Delta 2: also bash with duplicate-non-NULL via UPDATE (delete+insert).
      sql("UPDATE ljnkd_financials SET revenue = 999 WHERE period = 'Q1' AND sk_company_id = 20")
      refreshMv("ljnkd_mv")
      assertMvCorrect("ljnkd_mv", viewBody)
      ...
      sql(s"INSERT INTO ljnkd_security VALUES $moreNullSec")
      sql(s"INSERT INTO ljnkd_financials VALUES $moreNullFin")
      refreshMv("ljnkd_mv")
      assertMvCorrect("ljnkd_mv", viewBody)

      // Delta 4: delete several null-keyed left-side rows.
      sql("DELETE FROM ljnkd_security WHERE sk_company_id IS NULL AND sk_security_id IN (4, 5, 7, 8, 9)")
      refreshMv("ljnkd_mv")
      assertMvCorrect("ljnkd_mv", viewBody)

      sql("DROP MATERIALIZED VIEW ljnkd_mv")
      sql("DROP TABLE ljnkd_security")
      sql("DROP TABLE ljnkd_financials")
    }
