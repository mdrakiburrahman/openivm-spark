...
/** P6d — Port of `openivm/test/sql/ducklake_aggregate.test`.
  ...
  */
class DucklakeAggregateSpec extends AnyFunSpec with Matchers with BeforeAndAfterAll {

  private val warehouseDir: String = {
    val d = new File(s"target/test-warehouse-dl-agg-${UUID.randomUUID().toString.take(8)}")
    d.mkdirs()
    d.getAbsolutePath
  }
  ...
  override def beforeAll(): Unit = {
    ...
    spark = SparkSession
      .builder()
      .master("local[1]")
      .appName("openivm-spark-DucklakeAggregateSpec")
      .config(
  ...
  describe("(DA1) Grouped SUM + COUNT — full DML matrix") {
    it("incremental refresh handles INSERT into existing/new group, DELETE, UPDATE") {
      spark.sql("CREATE TABLE IF NOT EXISTS dl_sales(id INT, region STRING, amount INT) USING DELTA")
      spark.sql("INSERT INTO dl_sales VALUES (1, 'east', 100), (2, 'west', 200), (3, 'east', 150)")
      spark.sql(
      ...
      spark.sql("INSERT INTO dl_sales VALUES (4, 'east', 50)")
      ...
      spark.sql("INSERT INTO dl_sales VALUES (5, 'north', 300)")
      refreshMv("dl_sales_summary")
      assertMvCorrect(
        "dl_sales_summary",
        "SELECT region, SUM(amount) AS total, COUNT(amount) AS cnt FROM dl_sales GROUP BY region"
      ...
      spark.sql("DELETE FROM dl_sales WHERE id = 1")
      refreshMv("dl_sales_summary")
      assertMvCorrect(
        "dl_sales_summary",
        "SELECT region, SUM(amount) AS total, COUNT(amount) AS cnt FROM dl_sales GROUP BY region"
      ...
      spark.sql("UPDATE dl_sales SET amount = 500 WHERE id = 2")
      refreshMv("dl_sales_summary")
      assertMvCorrect(
        "dl_sales_summary",
        "SELECT region, SUM(amount) AS total, COUNT(amount) AS cnt FROM dl_sales GROUP BY region"
  ...
  describe("(DA2) Ungrouped COUNT(*) — scalar aggregate") {
    it("incremental refresh tracks insert and delete row counts") {
      spark.sql("CREATE TABLE IF NOT EXISTS dl_counter(id INT, val INT) USING DELTA")
      spark.sql("INSERT INTO dl_counter VALUES (1, 10), (2, 20), (3, 30)")
      spark.sql("CREATE MATERIALIZED VIEW dl_mv_counter AS SELECT COUNT(*) AS cnt FROM dl_counter")
      assertMvCorrect("dl_mv_counter", "SELECT COUNT(*) AS cnt FROM dl_counter")

      spark.sql("INSERT INTO dl_counter VALUES (4, 40), (5, 50)")
      refreshMv("dl_mv_counter")
      assertMvCorrect("dl_mv_counter", "SELECT COUNT(*) AS cnt FROM dl_counter")

      spark.sql("DELETE FROM dl_counter WHERE id <= 2")
      refreshMv("dl_mv_counter")
      assertMvCorrect("dl_mv_counter", "SELECT COUNT(*) AS cnt FROM dl_counter")
    }
  }
  ...
  describe("(DA3) MIN/MAX + computed CASE — chained CTE shape") {
    it("MIN/MAX over a CTE-wrapped projection refreshes incrementally") {
      ...
      spark.sql(s"CREATE MATERIALIZED VIEW dl_watch_state AS $viewBody")
  ...
  describe("(DA4) Stress — batched conflicting INSERT + DELETE + UPDATE") {
    it("single REFRESH consolidates two rounds of overlapping DML") {
      spark.sql("CREATE TABLE IF NOT EXISTS dl_stress(id INT, grp STRING, val INT) USING DELTA")
      spark.sql(
      ...
      spark.sql("INSERT INTO dl_stress VALUES (7, 'a', 100), (8, 'd', 200)")
      spark.sql("DELETE FROM dl_stress WHERE id = 2")
      spark.sql("UPDATE dl_stress SET val = 999 WHERE id = 3")
      spark.sql("INSERT INTO dl_stress VALUES (9, 'b', 5), (10, 'b', 5)")
      spark.sql("DELETE FROM dl_stress WHERE id = 5")
      spark.sql("UPDATE dl_stress SET grp = 'a' WHERE id = 4")
      spark.sql("INSERT INTO dl_stress VALUES (11, 'c', 1)")
      spark.sql("DELETE FROM dl_stress WHERE id = 8")
      ...
      spark.sql("DELETE FROM dl_stress WHERE grp = 'b'")
      spark.sql("INSERT INTO dl_stress VALUES (12, 'e', 500), (13, 'e', 500)")
      spark.sql("UPDATE dl_stress SET val = 0 WHERE grp = 'a'")
      spark.sql("INSERT INTO dl_stress VALUES (14, 'a', 1)")
      spark.sql("DELETE FROM dl_stress WHERE id = 11")
      refreshMv("dl_stress_agg")
      assertMvCorrect(
        "dl_stress_agg",
        "SELECT grp, SUM(val) AS total, COUNT(*) AS cnt FROM dl_stress GROUP BY grp"
  ...
  describe("(DA5) Top-k aggregate — GROUP BY + ORDER BY + LIMIT") {
    it("refresh keeps top-k correct after INSERT and after a stress batch") {
      spark.sql("CREATE TABLE IF NOT EXISTS dl_topk_sales(id INT, product STRING, revenue INT) USING DELTA")
      spark.sql(
      ...
      spark.sql("INSERT INTO dl_topk_sales VALUES (6,'grape',600)")
      refreshMv("dl_topk_sales_mv")
      assertMvCorrect("dl_topk_sales_mv", topK)

      // Stress: conflicting INSERT + DELETE + UPDATE before a single refresh
      spark.sql("INSERT INTO dl_topk_sales VALUES (7,'hazel',700),(8,'iris',10)")
      spark.sql("DELETE FROM dl_topk_sales WHERE id = 6")
      spark.sql("UPDATE dl_topk_sales SET revenue = 600 WHERE id = 3")
      refreshMv("dl_topk_sales_mv")
      assertMvCorrect("dl_topk_sales_mv", topK)
    }
  }
  ...
  describe("(DA6) Scalar aggregate subquery in HAVING (3-way join)") {
    it("MV joining a HAVING-filtered CTE to two dimensions refreshes correctly") {
      spark.sql("CREATE TABLE IF NOT EXISTS dl_hav_line(w INT, o INT, d INT, amount DOUBLE) USING DELTA")
      spark.sql("CREATE TABLE IF NOT EXISTS dl_hav_order(w INT, o INT, d INT, c INT) USING DELTA")
      spark.sql("CREATE TABLE IF NOT EXISTS dl_hav_customer(w INT, d INT, c INT, last_name STRING) USING DELTA")
      spark.sql(
        "INSERT INTO dl_hav_line VALUES " +
          "(1, 1, 1, 10), (1, 1, 1, 12), (1, 2, 1, 200), (1, 2, 1, 220), (2, 1, 1, 5)"
      )
      spark.sql("INSERT INTO dl_hav_order VALUES (1, 1, 1, 10), (1, 2, 1, 11), (2, 1, 1, 12)")
      spark.sql(
      ...
      spark.sql(s"CREATE MATERIALIZED VIEW dl_mv_hav_scalar AS $viewBody")

      spark.sql("INSERT INTO dl_hav_line VALUES (1, 3, 1, 500)")
      spark.sql("INSERT INTO dl_hav_order VALUES (1, 3, 1, 13)")
      spark.sql("INSERT INTO dl_hav_customer VALUES (1, 1, 13, 'Delta')")
  ...
  describe("(DA7) COUNT(DISTINCT) over join — GROUP_RECOMPUTE classification") {
    it("INSERT + INSERT + DELETE before a single refresh keeps MV consistent") {
      spark.sql("CREATE TABLE IF NOT EXISTS dl_gr_items(id INT, category STRING) USING DELTA")
      spark.sql("CREATE TABLE IF NOT EXISTS dl_gr_sales(item_id INT, qty INT) USING DELTA")
      spark.sql("INSERT INTO dl_gr_items VALUES (1, 'hardware'), (2, 'hardware'), (3, 'office')")
      spark.sql("INSERT INTO dl_gr_sales VALUES (1, 5), (1, 7), (2, 10), (3, 4)")
      val viewBody =
        "SELECT i.category, COUNT(DISTINCT i.id) AS item_count, SUM(s.qty) AS total_qty " +
          "FROM dl_gr_items i " +
          "JOIN dl_gr_sales s ON i.id = s.item_id " +
          "GROUP BY i.category"
      spark.sql(s"CREATE MATERIALIZED VIEW dl_mv_gr_distinct AS $viewBody")

      spark.sql("INSERT INTO dl_gr_items VALUES (4, 'office')")
      spark.sql("INSERT INTO dl_gr_sales VALUES (4, 20), (2, 1)")
      spark.sql("DELETE FROM dl_gr_sales WHERE item_id = 1 AND qty = 5")
