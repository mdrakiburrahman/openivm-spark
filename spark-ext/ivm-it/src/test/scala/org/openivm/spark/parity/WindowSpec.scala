...
/** P6f — 1:1 ScalaTest port of `openivm/test/sql/window.test`.
  ...
  */
class WindowSpec extends AnyFunSpec with Matchers with BeforeAndAfterAll {

  private val warehouseDir: String = {
    val d = new File(s"target/test-warehouse-window-${UUID.randomUUID().toString.take(8)}")
    d.mkdirs()
    d.getAbsolutePath
  }
  ...
  override def beforeAll(): Unit = {
    ...
    spark = SparkSession
      .builder()
      .master("local[1]")
      .appName("openivm-spark-WindowSpec")
      .config(
  ...
  /** Bidirectional `EXCEPT ALL` equivalence between the MV (projected to the
    * user-visible columns of `expectedSql`) and the live recomputation.
    * Strips hidden `openivm_*` bookkeeping columns.
    */
  ...
  describe("(1)+(2)+(6) ROW_NUMBER OVER (PARTITION BY grp ORDER BY val)") {
    it("INSERT, DELETE, and empty-delta refresh all stay bag-equal to the base query") {
      spark.sql("CREATE TABLE IF NOT EXISTS wt(id INT, grp STRING, val INT) USING DELTA")
      spark.sql(
      ...
      spark.sql(s"CREATE MATERIALIZED VIEW mv_rn AS $viewSql")
      ...
      spark.sql("INSERT INTO wt VALUES (7,'a',25)")
      refreshMv("mv_rn")
      spark.table("mv_rn").count() shouldBe 7L
      assertMvCorrect("mv_rn", viewSql)

      // (2) DELETE
      spark.sql("DELETE FROM wt WHERE id = 2")
      refreshMv("mv_rn")
      assertMvCorrect("mv_rn", viewSql)
  ...
  describe("(3)+(4) SUM(salary) OVER (PARTITION BY dept) — INSERT then batched INSERT/DELETE/UPDATE") {
    it("partition totals stay bag-equal across INSERT, then across a single mixed-DML refresh") {
      spark.sql("CREATE TABLE IF NOT EXISTS wt2(id INT, dept STRING, salary INT) USING DELTA")
      spark.sql("INSERT INTO wt2 VALUES (1,'eng',100), (2,'eng',200), (3,'hr',150), (4,'hr',250)")

      val viewSql =
        "SELECT id, dept, salary, SUM(salary) OVER (PARTITION BY dept) AS dept_total FROM wt2"
      spark.sql(s"CREATE MATERIALIZED VIEW mv_wsum AS $viewSql")

      // Initial cross-check (window.test:99-105).
      assertMvCorrect("mv_wsum", viewSql)

      // (3) INSERT into 'eng' only.
      spark.sql("INSERT INTO wt2 VALUES (5,'eng',300)")
      refreshMv("mv_wsum")
      assertMvCorrect("mv_wsum", viewSql)
      ...
      spark.sql("INSERT INTO wt2 VALUES (6,'hr',50)")
      spark.sql("DELETE FROM wt2 WHERE id = 3")
      spark.sql("UPDATE wt2 SET salary = 999 WHERE id = 1")
      refreshMv("mv_wsum")
      assertMvCorrect("mv_wsum", viewSql)
    }
  }
  ...
  describe("(5) ROW_NUMBER OVER (ORDER BY val) — no PARTITION BY, full-recompute shape") {
    it("MV stays bag-equal to the base query after INSERT") {
      spark.sql("CREATE TABLE IF NOT EXISTS wt3(id INT, val INT) USING DELTA")
      spark.sql("INSERT INTO wt3 VALUES (1, 10), (2, 20), (3, 30)")

      val viewSql = "SELECT id, val, ROW_NUMBER() OVER (ORDER BY val) AS rn FROM wt3"
      spark.sql(s"CREATE MATERIALIZED VIEW mv_nopart AS $viewSql")

      spark.sql("INSERT INTO wt3 VALUES (4, 15)")
      refreshMv("mv_nopart")
  ...
  describe("(7) ROW_NUMBER OVER (PARTITION BY c.name ORDER BY o.amount DESC) over a JOIN") {
    it("MV stays bag-equal after INSERT into the orders side") {
      spark.sql("CREATE TABLE IF NOT EXISTS w_orders(id INT, cust_id INT, amount INT) USING DELTA")
      spark.sql("CREATE TABLE IF NOT EXISTS w_customers(id INT, name STRING) USING DELTA")
      spark.sql("INSERT INTO w_customers VALUES (1,'Alice'), (2,'Bob'), (3,'Carol')")
      spark.sql(
      ...
      spark.sql(s"CREATE MATERIALIZED VIEW mv_wjoin AS $viewSql")

      spark.sql("INSERT INTO w_orders VALUES (6,1,500), (7,3,10)")
      refreshMv("mv_wjoin")
  ...
  describe("(8) ROW_NUMBER OVER (PARTITION BY dept, team ORDER BY val) — composite partition") {
    it("MV stays bag-equal after INSERT into one (dept, team) cell") {
      ...
      spark.sql(s"CREATE MATERIALIZED VIEW mv_comp AS $viewSql")

      spark.sql("INSERT INTO w_comp VALUES (5,'eng','be',15)")
      refreshMv("mv_comp")
  ...
  describe("(9) chained — table → window MV → aggregate MV (depth-2)") {
    it("RANK window MV and downstream COUNT(*) aggregate MV both stay correct") {
      spark.sql("CREATE TABLE IF NOT EXISTS w_emp(id INT, dept STRING, salary INT) USING DELTA")
      spark.sql(
      ...
      val rankedSql =
        "SELECT id, dept, salary, RANK() OVER (PARTITION BY dept ORDER BY salary DESC) AS rnk " +
          "FROM w_emp"
      spark.sql(s"CREATE MATERIALIZED VIEW mv_ranked AS $rankedSql")
      spark.sql(
      ...
      val initial = spark.sql("SELECT dept, top_count FROM mv_top2 ORDER BY dept").collect().toSeq
      initial.map(r => (r.getString(0), r.getLong(1))) shouldBe Seq(("eng", 2L), ("hr", 2L))

      spark.sql("INSERT INTO w_emp VALUES (6,'eng',300), (7,'hr',200)")
      ...
      val after = spark.sql("SELECT dept, top_count FROM mv_top2 ORDER BY dept").collect().toSeq
    ...
    it("MV stays bag-equal after INSERT that touches both partition keys") {
      ...
      val viewSql =
        "SELECT id, dept, team, val, " +
          ...
          "FROM w_multi_part"
      spark.sql(s"CREATE MATERIALIZED VIEW mv_multi_part AS $viewSql")
      ...
      spark.sql("INSERT INTO w_multi_part VALUES (5,'eng','be',15)")
  ...
  describe("(11) COUNT(*) OVER (PARTITION BY h_w_id ORDER BY h_date RANGE INTERVAL '30' DAY PRECEDING)") {
    ...
    ignore("rolling 30-day window MV stays bag-equal to the live recompute after INSERT") {
      ...
      spark.sql(s"CREATE MATERIALIZED VIEW mv_window_range AS $viewSql")

      // INSERT a row inside the 30-day window of (1, 2026-01-10).
      spark.sql("INSERT INTO w_history_range VALUES (1, TIMESTAMP '2026-01-20 00:00:00', 50)")
  ...
  describe("(12) regression — 3-level chain: src → l1 → l2(window) → l3") {
    ignore("(depth-3, PLAN.md §12) downstream l3 stays bag-equal to l2 after window refresh") {
      ...
      spark.sql("INSERT INTO w_chain_src VALUES (DATE '2026-01-03', 'A', 7, 25)")
