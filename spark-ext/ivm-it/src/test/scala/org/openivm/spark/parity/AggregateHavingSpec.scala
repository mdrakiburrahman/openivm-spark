...
/** Comprehensive parity tests for RefreshType 4 — AGGREGATE_HAVING.
  ...
  */
class AggregateHavingSpec extends AnyFunSpec with Matchers with BeforeAndAfterAll {

  private val warehouseDir: String = {
    val d = new File(s"target/test-warehouse-having-${UUID.randomUUID().toString.take(8)}")
    d.mkdirs()
    d.getAbsolutePath
  }
  ...
  override def beforeAll(): Unit = {
    ...
    spark = SparkSession
      .builder()
      .master("local[1]")
      .appName("openivm-spark-AggregateHavingSpec")
      .config(
  ...
  describe("(1) HAVING SUM(amount) > X — basic threshold") {
    it("incremental refresh tracks SUM threshold correctly across INSERTs") {
      spark.sql("CREATE TABLE IF NOT EXISTS sales_h1(region STRING, amount INT) USING DELTA")
      spark.sql("INSERT INTO sales_h1 VALUES ('a', 100), ('a', 200), ('b', 50), ('c', 10)")
      spark.sql(
        "CREATE MATERIALIZED VIEW mv_h1 AS " +
          "SELECT region, SUM(amount) AS total FROM sales_h1 " +
          "GROUP BY region HAVING SUM(amount) > 60"
      )
      spark.sql("INSERT INTO sales_h1 VALUES ('b', 100), ('c', 200)")
      refreshMv("mv_h1")
      assertMvCorrect(
        "mv_h1",
        "SELECT region, SUM(amount) AS total FROM sales_h1 " +
  ...
  describe("(2) HAVING COUNT(*) >= K — count-based predicate") {
    it("incremental refresh tracks COUNT(*) threshold correctly") {
      spark.sql("CREATE TABLE IF NOT EXISTS sales_h2(region STRING, amount INT) USING DELTA")
      spark.sql(
      ...
      spark.sql("INSERT INTO sales_h2 VALUES ('c', 90), ('b', 60), ('b', 75)")
      refreshMv("mv_h2")
      assertMvCorrect(
        "mv_h2",
        "SELECT region, COUNT(*) AS cnt FROM sales_h2 GROUP BY region HAVING COUNT(*) >= 3"
  ...
  describe("(3) HAVING AVG(amount) BETWEEN A AND B — range predicate") {
    ...
    it("incremental refresh tracks AVG range predicate") {
      spark.sql("CREATE TABLE IF NOT EXISTS sales_h3(region STRING, amount DOUBLE) USING DELTA")
      spark.sql(
      ...
      spark.sql("INSERT INTO sales_h3 VALUES ('a', 50.0), ('a', 50.0), ('d', 25.0), ('d', 35.0)")
      refreshMv("mv_h3")
      assertMvCorrect(
        "mv_h3",
        "SELECT region, AVG(amount) AS avg_amt FROM sales_h3 " +
  ...
  describe("(4a) Composite GROUP BY (k1, k2) HAVING two predicates joined by AND") {
    it("incremental refresh tracks AND-combined HAVING predicates") {
      ...
      spark.sql("INSERT INTO sales_h4a VALUES ('a', 2, 30), ('a', 2, 40), ('c', 3, 50)")
      refreshMv("mv_h4a")
      assertMvCorrect(
        "mv_h4a",
        "SELECT region, sku, SUM(qty) AS total, COUNT(*) AS cnt " +
  ...
  describe("(4b) Composite GROUP BY (k1, k2) HAVING two predicates joined by OR") {
    it("incremental refresh tracks OR-combined HAVING predicates") {
      ...
      spark.sql("DELETE FROM sales_h4b WHERE region = 'b' AND qty = 5 AND sku = 2")
      spark.sql("INSERT INTO sales_h4b VALUES ('a', 1, 200)")
      refreshMv("mv_h4b")
      assertMvCorrect(
        "mv_h4b",
        "SELECT region, sku, SUM(qty) AS total, COUNT(*) AS cnt " +
  ...
  describe("(5) Multiple aggregates in SELECT, HAVING references one") {
    it("HAVING on SUM works while AVG/COUNT projections also stay in sync") {
      spark.sql("CREATE TABLE IF NOT EXISTS sales_h5(region STRING, amount INT) USING DELTA")
      spark.sql(
      ...
      spark.sql("INSERT INTO sales_h5 VALUES ('b', 50), ('b', 50), ('d', 200)")
      refreshMv("mv_h5")
      assertMvCorrect(
        "mv_h5",
        "SELECT region, SUM(amount) AS total, COUNT(*) AS cnt, AVG(amount) AS avg_amt " +
  ...
  describe("(6) Row transitions OUT after DELETE — group sum falls below threshold") {
    it("group disappears from MV when its sum falls below the HAVING threshold") {
      spark.sql("CREATE TABLE IF NOT EXISTS sales_h6(region STRING, amount INT) USING DELTA")
      spark.sql(
      ...
      spark.sql("DELETE FROM sales_h6 WHERE region = 'b' AND amount = 80")
      refreshMv("mv_h6")
      assertMvCorrect(
        "mv_h6",
        "SELECT region, SUM(amount) AS total FROM sales_h6 " +
  ...
  describe("(7) Row transitions IN after INSERT — group sum rises above threshold") {
    it("previously-failing group appears in MV after INSERT pushes it above threshold") {
      spark.sql("CREATE TABLE IF NOT EXISTS sales_h7(region STRING, amount INT) USING DELTA")
      ...
      spark.sql("INSERT INTO sales_h7 VALUES ('c', 50)")
      refreshMv("mv_h7")
      assertMvCorrect(
        "mv_h7",
        "SELECT region, SUM(amount) AS total FROM sales_h7 " +
  ...
  describe("(8) Net-zero INSERT+DELETE within one batch — MV unchanged") {
    it("MV equals base table when inserted rows are also deleted in same batch") {
      spark.sql("CREATE TABLE IF NOT EXISTS sales_h8(region STRING, amount INT) USING DELTA")
      spark.sql("INSERT INTO sales_h8 VALUES ('a', 100), ('a', 50), ('b', 200)")
      spark.sql(
      ...
      spark.sql("INSERT INTO sales_h8 VALUES ('z', 999)")
      spark.sql("DELETE FROM sales_h8 WHERE region = 'z'")
      refreshMv("mv_h8")
      assertMvCorrect(
        "mv_h8",
        "SELECT region, SUM(amount) AS total FROM sales_h8 " +
  ...
  describe("(9) Mixed batched DML — INSERT + DELETE + UPDATE on overlapping groups") {
    it("single REFRESH reconciles all conflicting deltas correctly") {
      spark.sql("CREATE TABLE IF NOT EXISTS sales_h9(region STRING, amount INT) USING DELTA")
      spark.sql(
      ...
      spark.sql("INSERT INTO sales_h9 VALUES ('d', 200), ('e', 500), ('a', 25)")
      // DELETE drops one 'b' row → sum=60 (still below 75).
      spark.sql("DELETE FROM sales_h9 WHERE region = 'b' AND amount = 20")
      // DELETE removes 'c' entirely (passes → disappears).
      spark.sql("DELETE FROM sales_h9 WHERE region = 'c'")
      refreshMv("mv_h9")
      assertMvCorrect(
        "mv_h9",
        "SELECT region, SUM(amount) AS total FROM sales_h9 " +
  ...
  describe("(10) NULL group keys and NULL aggregate inputs") {
    it("NULL key is a valid group; NULL amounts are ignored by SUM per SQL semantics") {
      spark.sql("CREATE TABLE IF NOT EXISTS sales_h10(region STRING, amount INT) USING DELTA")
      spark.sql(
      ...
      spark.sql("INSERT INTO sales_h10 VALUES (NULL, 40), ('c', 200), ('c', NULL)")
      refreshMv("mv_h10")
      assertMvCorrect(
        "mv_h10",
        "SELECT region, SUM(amount) AS total FROM sales_h10 " +
  ...
  describe("(11) All groups fail HAVING after refresh — MV is empty") {
    it("MV correctly reports empty when every group's aggregate falls below threshold") {
      spark.sql("CREATE TABLE IF NOT EXISTS sales_h11(region STRING, amount INT) USING DELTA")
      spark.sql("INSERT INTO sales_h11 VALUES ('a', 1000), ('b', 800)")
      spark.sql(
      ...
      spark.sql("DELETE FROM sales_h11 WHERE region = 'a'")
      spark.sql("DELETE FROM sales_h11 WHERE region = 'b'")
      // Add small groups so the underlying table is non-empty but no group passes.
      spark.sql("INSERT INTO sales_h11 VALUES ('a', 10), ('b', 20), ('c', 30)")
      refreshMv("mv_h11")
      assertMvCorrect(
        "mv_h11",
        "SELECT region, SUM(amount) AS total FROM sales_h11 " +
