...
/** Split-off from `AggregateGroupSpec.scala` so that each chunk of ~8 tests
  ...
  */
class AggregateGroupStddevCompositeSpec extends AnyFunSpec with Matchers with BeforeAndAfterAll {

  private val warehouseDir: String = {
    val d = new File(
      s"target/test-warehouse-aggregate-group-stddev-composite-spec-${UUID.randomUUID().toString.take(8)}"
    )
    d.mkdirs()
    d.getAbsolutePath
  }
  ...
  override def beforeAll(): Unit = {
    ...
    spark = SparkSession
      .builder()
      .master("local[1]")
      .appName("openivm-spark-AggregateGroupStddevCompositeSpec")
      .config(
  ...
  describe("(5a) GROUP BY k, STDDEV(x) — INSERT") {
    it("sample STDDEV is maintained via hidden SUM/COUNT/SUM_SQ decomposition") {
      spark.sql("CREATE TABLE IF NOT EXISTS aggrpsc_ag_std_5a(k STRING, x DOUBLE) USING DELTA")
      spark.sql(
      ...
      spark.sql("INSERT INTO aggrpsc_ag_std_5a VALUES ('a', 6.0), ('c', 2.0), ('c', 4.0)")
  ...
  describe("(5b) GROUP BY k, STDDEV(x) — DELETE") {
    it("sample STDDEV is correctly updated after DELETE") {
      spark.sql("CREATE TABLE IF NOT EXISTS aggrpsc_ag_std_5b(k STRING, x DOUBLE) USING DELTA")
      spark.sql(
      ...
      spark.sql("DELETE FROM aggrpsc_ag_std_5b WHERE k = 'a' AND x = 1.0")
  ...
  describe("(6) GROUP BY k, STDDEV_POP(x)") {
    it("population STDDEV is maintained incrementally") {
      spark.sql("CREATE TABLE IF NOT EXISTS aggrpsc_ag_stdp_6(k STRING, x DOUBLE) USING DELTA")
      spark.sql(
      ...
      spark.sql("INSERT INTO aggrpsc_ag_stdp_6 VALUES ('d', 4.0), ('d', 8.0)")
      spark.sql("DELETE FROM aggrpsc_ag_stdp_6 WHERE k = 'c' AND x = 3.0")
  ...
  describe("(7a) GROUP BY k, VARIANCE(x)") {
    it("sample VARIANCE is maintained incrementally") {
      spark.sql("CREATE TABLE IF NOT EXISTS aggrpsc_ag_var_7a(k STRING, x DOUBLE) USING DELTA")
      spark.sql(
      ...
      spark.sql("INSERT INTO aggrpsc_ag_var_7a VALUES ('a', 8.0), ('c', 3.0), ('c', 7.0)")
  ...
  describe("(7b) GROUP BY k, VAR_POP(x)") {
    it("population VARIANCE is maintained incrementally") {
      spark.sql("CREATE TABLE IF NOT EXISTS aggrpsc_ag_varp_7b(k STRING, x DOUBLE) USING DELTA")
      spark.sql("INSERT INTO aggrpsc_ag_varp_7b VALUES ('a', 2.0), ('a', 4.0), ('a', 6.0), ('b', 1.0), ('b', 3.0)")
      spark.sql(
        "CREATE MATERIALIZED VIEW aggrpsc_mv_ag_varp_7b AS " +
          "SELECT k, VAR_POP(x) AS vrp FROM aggrpsc_ag_varp_7b GROUP BY k"
      )
      spark.sql("INSERT INTO aggrpsc_ag_varp_7b VALUES ('b', 5.0), ('c', 10.0)")
      spark.sql("DELETE FROM aggrpsc_ag_varp_7b WHERE k = 'a' AND x = 2.0")
  ...
  describe("(9a) GROUP BY k, SUM(x) + COUNT(*) + AVG(x) — multiple aggregates INSERT") {
    it("multiple aggregates in one view are all maintained correctly") {
      spark.sql("CREATE TABLE IF NOT EXISTS aggrpsc_ag_multi_9a(k STRING, x DOUBLE) USING DELTA")
      spark.sql("INSERT INTO aggrpsc_ag_multi_9a VALUES ('a', 10.0), ('a', 20.0), ('b', 5.0), ('b', 15.0)")
      spark.sql(
      ...
      spark.sql("INSERT INTO aggrpsc_ag_multi_9a VALUES ('a', 30.0), ('c', 7.0)")
  ...
  describe("(9b) GROUP BY k, SUM(x) + COUNT(*) + AVG(x) — mixed DML") {
    ...
    it("all aggregates remain consistent after batched mixed DML") {
      spark.sql("CREATE TABLE IF NOT EXISTS aggrpsc_ag_multi_9b(k STRING, x DOUBLE) USING DELTA")
      spark.sql("INSERT INTO aggrpsc_ag_multi_9b VALUES ('a', 10.0), ('a', 20.0), ('b', 5.0), ('c', 100.0)")
      spark.sql(
        "CREATE MATERIALIZED VIEW aggrpsc_mv_ag_multi_9b AS " +
          "SELECT k, SUM(x) AS total, COUNT(*) AS cnt, AVG(x) AS avg_x " +
          "FROM aggrpsc_ag_multi_9b GROUP BY k"
      )
      spark.sql("DELETE FROM aggrpsc_ag_multi_9b WHERE k = 'c'")
      spark.sql("INSERT INTO aggrpsc_ag_multi_9b VALUES ('b', 25.0), ('d', 50.0)")
      // Remove one 'a' row (count_delta('a') = -1 ≠ 0) so AVG refreshes correctly.
      spark.sql("DELETE FROM aggrpsc_ag_multi_9b WHERE k = 'a' AND x = 10.0")
  ...
  describe("(13) NULL aggregate input — SUM/AVG handle NULLs per SQL semantics") {
    it("SUM ignores NULLs; a group of all-NULLs returns NULL sum") {
      spark.sql("CREATE TABLE IF NOT EXISTS aggrpsc_ag_nullv_13(k STRING, x INT) USING DELTA")
      spark.sql("INSERT INTO aggrpsc_ag_nullv_13 VALUES ('a', 10), ('a', NULL), ('b', NULL)")
      spark.sql(
      ...
      spark.sql("INSERT INTO aggrpsc_ag_nullv_13 VALUES ('b', NULL), ('c', 5)")
