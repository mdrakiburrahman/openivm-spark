...
/** P6a — Port of `openivm/test/sql/union.test`.
  ...
  */
class UnionSpec extends AnyFunSpec with Matchers with BeforeAndAfterAll {

  private val warehouseDir: String = {
    val d = new File(s"target/test-warehouse-union-${UUID.randomUUID().toString.take(8)}")
    d.mkdirs()
    d.getAbsolutePath
  }
  ...
  override def beforeAll(): Unit = {
    ...
    spark = SparkSession
      .builder()
      .master("local[1]")
      .appName("openivm-spark-UnionSpec")
      .config(
  ...
  describe("(1b) UNION ALL with fully-identical row INSERTs (duplicate-row bag preservation)") {
    ignore("inserting two byte-equal rows on a single branch yields 2 MV rows after refresh") {
      spark.sql("CREATE TABLE IF NOT EXISTS u_us_b(id INT, product STRING, amount INT) USING DELTA")
      spark.sql("CREATE TABLE IF NOT EXISTS u_eu_b(id INT, product STRING, amount INT) USING DELTA")
      spark.sql("INSERT INTO u_us_b VALUES (1, 'widget', 100)")
      spark.sql("INSERT INTO u_eu_b VALUES (3, 'widget', 150)")
      spark.sql(
      ...
      spark.sql("INSERT INTO u_eu_b VALUES (8, 'nail', 10), (8, 'nail', 10)")
  ...
  describe("(2) UNION ALL with aggregate on top: SUM(revenue) GROUP BY region") {
    it("incrementally maintains the aggregate over the union") {
      spark.sql("CREATE TABLE IF NOT EXISTS u_online(region STRING, revenue INT) USING DELTA")
      spark.sql("CREATE TABLE IF NOT EXISTS u_store(region STRING, revenue INT) USING DELTA")
      spark.sql("INSERT INTO u_online VALUES ('US', 100), ('EU', 200)")
      spark.sql("INSERT INTO u_store  VALUES ('US', 300), ('EU', 400)")
      ...
      spark.sql("INSERT INTO u_online VALUES ('US', 50), ('JP', 1000)")
      spark.sql("INSERT INTO u_store  VALUES ('EU', 100)")
  ...
  describe("(3) UNION ALL inside a JOIN: join over (orders_a UNION ALL orders_b)") {
    it("incrementally maintains the join+union projection") {
      spark.sql("CREATE TABLE IF NOT EXISTS u_products(id INT, name STRING) USING DELTA")
      spark.sql("CREATE TABLE IF NOT EXISTS u_orders_a(product_id INT, qty INT) USING DELTA")
      spark.sql("CREATE TABLE IF NOT EXISTS u_orders_b(product_id INT, qty INT) USING DELTA")
      spark.sql("INSERT INTO u_products VALUES (1, 'Alpha'), (2, 'Beta')")
      spark.sql("INSERT INTO u_orders_a VALUES (1, 10), (2, 5)")
      spark.sql("INSERT INTO u_orders_b VALUES (1, 20), (2, 15)")
      ...
      spark.sql("INSERT INTO u_orders_a VALUES (1, 30)")
      spark.sql("INSERT INTO u_orders_b VALUES (2, 25)")
      refreshMv("mv_all_product_orders")
      assertMvCorrect("mv_all_product_orders", expected)

      // DELETE from one branch
      spark.sql("DELETE FROM u_orders_a WHERE qty = 10")
      refreshMv("mv_all_product_orders")
      assertMvCorrect("mv_all_product_orders", expected)
    }
  }
  ...
  describe("(4) UNION ALL: drain both sides then repopulate") {
    it("MV correctly tracks delete-all → empty → re-populate cycle on both branches") {
      spark.sql("CREATE TABLE IF NOT EXISTS u_a(id INT) USING DELTA")
      spark.sql("CREATE TABLE IF NOT EXISTS u_b(id INT) USING DELTA")
      spark.sql("INSERT INTO u_a VALUES (1), (2), (3)")
      spark.sql("INSERT INTO u_b VALUES (4), (5)")
      ...
      spark.sql("DELETE FROM u_a")
      spark.sql("DELETE FROM u_b")
      refreshMv("mv_union_empty")
      assertMvCorrect("mv_union_empty", expected)
      spark.table("mv_union_empty").count() shouldBe 0L

      // Repopulate
      spark.sql("INSERT INTO u_a VALUES (10), (20)")
      spark.sql("INSERT INTO u_b VALUES (30), (40), (50)")
      refreshMv("mv_union_empty")
      assertMvCorrect("mv_union_empty", expected)
    }
  }
  ...
  describe("(5a) UNION ALL with heavy duplicates: initial CREATE-time population") {
    it("MV is bag-equal to view body immediately after CREATE (8 rows of id=1)") {
      spark.sql("CREATE TABLE IF NOT EXISTS u_dup_l(id INT) USING DELTA")
      spark.sql("CREATE TABLE IF NOT EXISTS u_dup_r(id INT) USING DELTA")
      spark.sql("INSERT INTO u_dup_l VALUES (1), (1), (1), (1), (1)")
      spark.sql("INSERT INTO u_dup_r VALUES (1), (1), (1)")
  ...
  describe("(5b) UNION ALL: drain one side + re-insert (duplicate-row bag preservation)") {
    ignore("draining one side and re-inserting byte-equal rows preserves bag count") {
      spark.sql("CREATE TABLE IF NOT EXISTS u_dup_l_b(id INT) USING DELTA")
      spark.sql("CREATE TABLE IF NOT EXISTS u_dup_r_b(id INT) USING DELTA")
      spark.sql("INSERT INTO u_dup_r_b VALUES (1), (1), (1)")
      spark.sql(
      ...
      spark.sql(s"INSERT INTO u_dup_l_b VALUES $tenOnes")
  ...
  describe("(6) UNION ALL: drain side A entirely, then repopulate with different rows") {
    it("MV correctly forgets and re-learns side A's rows") {
      spark.sql("CREATE TABLE IF NOT EXISTS u_side_a(id INT, name STRING) USING DELTA")
      spark.sql("CREATE TABLE IF NOT EXISTS u_side_b(id INT, name STRING) USING DELTA")
      spark.sql("INSERT INTO u_side_a VALUES (1, 'alpha'), (2, 'beta')")
      spark.sql("INSERT INTO u_side_b VALUES (3, 'gamma'), (4, 'delta')")
      ...
      spark.sql("DELETE FROM u_side_a")
      refreshMv("mv_union_side")
      assertMvCorrect("mv_union_side", expected)

      spark.sql("INSERT INTO u_side_a VALUES (5, 'epsilon'), (6, 'zeta')")
      refreshMv("mv_union_side")
      assertMvCorrect("mv_union_side", expected)
    }
  }
  ...
  describe("(7) Stress: two JOINs unioned, INSERT + DELETE across branches → single REFRESH") {
    it("incremental refresh correctly maintains both joined branches with batched conflicting DML") {
      spark.sql("CREATE TABLE IF NOT EXISTS u_emp(id INT, name STRING, dept_id INT) USING DELTA")
      spark.sql("CREATE TABLE IF NOT EXISTS u_dept(id INT, dname STRING) USING DELTA")
      spark.sql("CREATE TABLE IF NOT EXISTS u_contractor(id INT, name STRING, dept_id INT) USING DELTA")
      spark.sql("INSERT INTO u_dept VALUES (1, 'Eng'), (2, 'Sales')")
      spark.sql("INSERT INTO u_emp VALUES (1, 'Alice', 1), (2, 'Bob', 2)")
      spark.sql("INSERT INTO u_contractor VALUES (10, 'Xander', 1)")
      ...
      spark.sql("INSERT INTO u_contractor VALUES (11, 'Yara', 2)")
      spark.sql("DELETE FROM u_emp WHERE id = 2")
  ...
  describe("(8) UNION ALL over per-branch aggregates → GROUP_RECOMPUTE classification") {
    it("classifier routes to GROUP_RECOMPUTE; refresh stays bag-equal to live view body") {
      spark.sql("CREATE TABLE IF NOT EXISTS u_cust(w_id INT, balance DOUBLE) USING DELTA")
      spark.sql("CREATE TABLE IF NOT EXISTS u_hist(w_id INT, amount DOUBLE) USING DELTA")
      spark.sql("INSERT INTO u_cust VALUES (1, 100.0), (1, 200.0), (2, 50.0)")
      spark.sql("INSERT INTO u_hist VALUES (1, 10.0), (2, 20.0), (2, 30.0)")
      ...
      spark.sql("INSERT INTO u_cust VALUES (1, 300.0)")
      spark.sql("DELETE FROM u_hist WHERE w_id = 2 AND amount = 20.0")
      refreshMv("mv_uoa")
      assertMvCorrect("mv_uoa", expected)

      // Second refresh with more mixed deltas
      spark.sql("INSERT INTO u_hist VALUES (3, 99.0)")
      spark.sql("DELETE FROM u_cust WHERE w_id = 2")
      refreshMv("mv_uoa")
      assertMvCorrect("mv_uoa", expected)
    }
  }
  ...
  describe("(9) UNION DISTINCT over per-branch aggregates → GROUP_RECOMPUTE classification") {
    it("UNION DISTINCT folds duplicate (w_id,sum) rows across branches") {
      spark.sql("CREATE TABLE IF NOT EXISTS u_cust_d(w_id INT, balance DOUBLE) USING DELTA")
      spark.sql("CREATE TABLE IF NOT EXISTS u_hist_d(w_id INT, amount DOUBLE) USING DELTA")
      spark.sql("INSERT INTO u_cust_d VALUES (1, 100.0), (1, 200.0), (2, 50.0)")
      spark.sql("INSERT INTO u_hist_d VALUES (1, 10.0), (2, 20.0), (2, 30.0)")
      ...
      spark.sql("INSERT INTO u_cust_d VALUES (1, 300.0)")
      spark.sql("DELETE FROM u_hist_d WHERE w_id = 2 AND amount = 20.0")
      refreshMv("mv_uod")
      assertMvCorrect("mv_uod", expected)
    }
  }
