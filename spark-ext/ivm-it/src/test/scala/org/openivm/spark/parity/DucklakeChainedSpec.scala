...
/** P6d — Port of `openivm/test/sql/ducklake_chained.test`.
  ...
  */
class DucklakeChainedSpec extends AnyFunSpec with Matchers with BeforeAndAfterAll {

  private val warehouseDir: String = {
    val d = new File(s"target/test-warehouse-dl-chain-${UUID.randomUUID().toString.take(8)}")
    d.mkdirs()
    d.getAbsolutePath
  }
  ...
  override def beforeAll(): Unit = {
    ...
    spark = SparkSession
      .builder()
      .master("local[1]")
      .appName("openivm-spark-DucklakeChainedSpec")
      .config(
  ...
  describe("(DCh-1) Initial depth-2 chain — CTAS at CREATE time") {
    it("mv2 created over mv1 reflects mv1's current data") {
      spark.sql("CREATE TABLE IF NOT EXISTS dl_ch1_sales(id INT, region STRING, amount INT) USING DELTA")
      spark.sql(
  ...
  describe("(DCh-2) Depth-2 chain — incremental REFRESH of mv1 keeps mv1 correct") {
    it("after INSERT into base, REFRESH mv_region — mv_region matches grouped SUM") {
      spark.sql("CREATE TABLE IF NOT EXISTS dl_ch2_sales(id INT, region STRING, amount INT) USING DELTA")
      spark.sql(
      ...
      spark.sql("INSERT INTO dl_ch2_sales VALUES (5, 'south', 200)")
      ...
      spark.sql("DELETE FROM dl_ch2_sales WHERE id = 1")
      refreshMv("dl_ch2_mv_region")
      assertMvCorrect(
        "dl_ch2_mv_region",
        "SELECT region, SUM(amount) AS total, COUNT(*) AS cnt FROM dl_ch2_sales GROUP BY region"
      ...
      spark.sql("INSERT INTO dl_ch2_sales VALUES (6, 'north', 500)")
      spark.sql("DELETE FROM dl_ch2_sales WHERE id = 4")
      spark.sql("INSERT INTO dl_ch2_sales VALUES (7, 'west', 100)")
      refreshMv("dl_ch2_mv_region")
      assertMvCorrect(
        "dl_ch2_mv_region",
        "SELECT region, SUM(amount) AS total, COUNT(*) AS cnt FROM dl_ch2_sales GROUP BY region"
  ...
  describe("(DCh-3) Depth-2 chain — mv2 = SELECT…FROM mv1 reflects each mv1 refresh") {
    it("each DROP+CREATE of mv2 picks up mv1's most recent state") {
      spark.sql("CREATE TABLE IF NOT EXISTS dl_ch3_sales(id INT, region STRING, amount INT) USING DELTA")
      spark.sql(
      ...
      rounds.foreach { sqlBatch =>
        sqlBatch.split(";\\s*").foreach(stmt => spark.sql(stmt.trim))
        ...
        spark.sql("DROP MATERIALIZED VIEW dl_ch3_mv_top")
        spark.sql(
          "CREATE MATERIALIZED VIEW dl_ch3_mv_top AS " +
            "SELECT region, total, cnt FROM dl_ch3_mv_region WHERE total > 100"
        )
  ...
  describe("(DCh-4) Two-namespace depth-2 chain — verifies CTAS through mv1 to mv2") {
    it("pipeline_l2 chains correctly through pipeline_l1") {
      spark.sql("CREATE TABLE IF NOT EXISTS chain_a_pipeline_src(id INT, amount INT) USING DELTA")
      spark.sql("INSERT INTO chain_a_pipeline_src VALUES (1, 10), (2, 20)")
      spark.sql(
      ...
      spark.sql("INSERT INTO chain_a_pipeline_src VALUES (3, 30)")
      refreshMv("chain_b_pipeline_l1")
      spark.sql("DROP MATERIALIZED VIEW chain_a_pipeline_l2")
      spark.sql(
        "CREATE MATERIALIZED VIEW chain_a_pipeline_l2 AS " +
          "SELECT id, amount, amount * 2 AS doubled FROM chain_b_pipeline_l1"
      )
  ...
  describe("(DCh-5) Projection-only depth-2 chain") {
    it("catalog_chain_l2 (projection over catalog_chain_l1) tracks base via CTAS re-creation") {
      spark.sql("CREATE TABLE IF NOT EXISTS dl_catalog_chain_base(id INT, amount INT) USING DELTA")
      spark.sql("INSERT INTO dl_catalog_chain_base VALUES (1, 10), (2, 20)")
      spark.sql(
      ...
      spark.sql("INSERT INTO dl_catalog_chain_base VALUES (3, 30)")
      refreshMv("dl_catalog_chain_l1")
      spark.sql("DROP MATERIALIZED VIEW dl_catalog_chain_l2")
      spark.sql(
        "CREATE MATERIALIZED VIEW dl_catalog_chain_l2 AS " +
          "SELECT id, amount FROM dl_catalog_chain_l1"
      )
