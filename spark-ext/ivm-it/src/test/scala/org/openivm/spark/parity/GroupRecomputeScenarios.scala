...
/** P5.rt6 — Coverage of RefreshType 6 (GROUP_RECOMPUTE).
  ...
  */
abstract class GroupRecomputeScenarios extends IvmParitySpecBase("group-recompute") {
  ...
  describe("(1) Inner DISTINCT under SUM — SELECT region, SUM(DISTINCT amount) … GROUP BY region") {
    ...
    it("classifies as GROUP_RECOMPUTE and incrementally maintains SUM(DISTINCT) across DML") {
      sql("CREATE TABLE IF NOT EXISTS gr_sales_1(region STRING, amount INT) USING DELTA")
      sql(
      ...
      sql("INSERT INTO gr_sales_1 VALUES ('east', 40)")
      // Add a duplicate that should NOT change SUM(DISTINCT) for 'west' (30 already counted)
      sql("INSERT INTO gr_sales_1 VALUES ('west', 30)")
      refreshMv("mv_gr_1")
  ...
  describe("(2) Inner DISTINCT under COUNT — SELECT region, COUNT(DISTINCT customer_id) … GROUP BY region") {
    ...
    it("classifies as GROUP_RECOMPUTE and incrementally maintains COUNT(DISTINCT)") {
      sql("CREATE TABLE IF NOT EXISTS gr_sales_2(region STRING, customer_id INT) USING DELTA")
      sql(
      ...
      sql("INSERT INTO gr_sales_2 VALUES ('east', 3)")
      // Add duplicate customer in 'west' (5 already counted → uniq stays at 2)
      sql("INSERT INTO gr_sales_2 VALUES ('west', 5)")
      refreshMv("mv_gr_2")
  ...
  describe("(3) JOIN over CTE-aggregate (DELIM-join key recovery) → GROUP_RECOMPUTE") {
    ...
    it("classifies as GROUP_RECOMPUTE and refreshes after aggregate-source DML") {
      sql("CREATE TABLE IF NOT EXISTS gr_wh_3(w_id INT, w_name STRING) USING DELTA")
      sql("CREATE TABLE IF NOT EXISTS gr_ol_3(ol_w_id INT, ol_d_id INT, ol_amount DOUBLE) USING DELTA")
      sql("INSERT INTO gr_wh_3 VALUES (1, 'Alpha'), (2, 'Beta')")
      sql(
      ...
      sql("INSERT INTO gr_ol_3 VALUES (1, 1, 30.0), (2, 1, 50.0)")
      refreshMv("mv_gr_3")
  ...
  describe("(4) MIN/MAX with GROUP BY — observed refreshType = 0 (AGGREGATE_GROUP, MERGE with GREATEST/LEAST)") {
    ...
    it("INSERTs into existing and new groups; MIN/MAX correctly maintained via MERGE fast path") {
      sql("CREATE TABLE IF NOT EXISTS gr_sales_4(region STRING, amount INT) USING DELTA")
      sql(
      ...
      sql("INSERT INTO gr_sales_4 VALUES ('east', 40), ('west', 1), ('north', 100), ('north', 7)")
      refreshMv("mv_gr_4")
  ...
  describe("(5) Group-key change via UPDATE — old key recomputes, new key recomputes") {
    ...
    it("UPDATE that moves a row between groups → both source and destination groups recomputed") {
      sql("CREATE TABLE IF NOT EXISTS gr_sales_5(region STRING, amount INT) USING DELTA")
      sql(
      ...
      sql("UPDATE gr_sales_5 SET region = 'west' WHERE region = 'east' AND amount = 20")
      refreshMv("mv_gr_5")
  ...
  describe("(6) Insert into existing group → only the affected group is recomputed") {
    ...
    it("incremental refresh updates only the group that received new rows") {
      sql("CREATE TABLE IF NOT EXISTS gr_sales_6(region STRING, customer_id INT) USING DELTA")
      sql(
      ...
      sql("INSERT INTO gr_sales_6 VALUES ('east', 3), ('east', 4)")
      refreshMv("mv_gr_6")
  ...
  describe("(7) Delete the last row of a group — affected group is removed from MV") {
    ...
    it("DELETE of every row in a group → MV no longer contains that group") {
      sql("CREATE TABLE IF NOT EXISTS gr_sales_7(region STRING, customer_id INT) USING DELTA")
      sql(
      ...
      sql("DELETE FROM gr_sales_7 WHERE region IN ('east', 'west')")
      refreshMv("mv_gr_7")
  ...
  describe("(8) New group key appears for the first time — INSERT-only into a brand-new group") {
    ...
    it("incremental refresh inserts the new group without disturbing pre-existing ones") {
      sql("CREATE TABLE IF NOT EXISTS gr_sales_8(region STRING, amount INT) USING DELTA")
      sql("INSERT INTO gr_sales_8 VALUES ('east', 10), ('east', 20), ('west', 5)")
      sql(
      ...
      sql("INSERT INTO gr_sales_8 VALUES ('south', 100), ('south', 100), ('south', 200)")
      refreshMv("mv_gr_8")
  ...
  describe("(9) Batched conflicting DML across multiple groups before a single REFRESH") {
    ...
    it("INSERT + DELETE + UPDATE on overlapping groups are reconciled in one refresh") {
      sql("CREATE TABLE IF NOT EXISTS gr_sales_9(region STRING, customer_id INT) USING DELTA")
      sql(
      ...
      sql("DELETE FROM gr_sales_9 WHERE region = 'east' AND customer_id = 2")
      sql("INSERT INTO gr_sales_9 VALUES ('east', 4)")
      sql("INSERT INTO gr_sales_9 VALUES ('west', 12)")
      sql("DELETE FROM gr_sales_9 WHERE region = 'north' AND customer_id = 21")
      sql("INSERT INTO gr_sales_9 VALUES ('north', 20)")
      sql("INSERT INTO gr_sales_9 VALUES ('south', 100), ('south', 101)")
  ...
  describe("(10) NULL group keys are handled correctly (NULL-safe IS NOT DISTINCT FROM matching)") {
    ...
    it("rows with NULL region form a single group; DML targeting NULL group works") {
      sql("CREATE TABLE IF NOT EXISTS gr_sales_10(region STRING, customer_id INT) USING DELTA")
      sql(
      ...
      sql("INSERT INTO gr_sales_10 VALUES (NULL, 7)")
      // Insert a duplicate distinct value into NULL group → uniq unchanged
      sql("INSERT INTO gr_sales_10 VALUES (NULL, 5)")
      refreshMv("mv_gr_10")
