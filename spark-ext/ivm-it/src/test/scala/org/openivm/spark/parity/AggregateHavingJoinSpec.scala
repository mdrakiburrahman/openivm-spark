...
/** Split-off from `AggregateSpec.scala` so that each chunk of ~10 tests runs
  ...
  */
class AggregateHavingJoinSpec extends AnyFunSpec with Matchers with BeforeAndAfterAll {

  private val warehouseDir: String = {
    val d = new File(s"target/test-warehouse-aggregate-having-join-spec-${UUID.randomUUID().toString.take(8)}")
    d.mkdirs()
    d.getAbsolutePath
  }
  ...
  override def beforeAll(): Unit = {
    ...
    spark = SparkSession
      .builder()
      .master("local[1]")
      .appName("openivm-spark-AggregateHavingJoinSpec")
      .config(
  ...
  describe("HAVING clause — agghj_having_test") {
    it("HAVING-filtered MV stays bag-equal after INSERTs cross/uncross threshold") {
      spark.sql("CREATE TABLE agghj_having_test (region STRING, amount INT) USING DELTA")
      spark.sql("INSERT INTO agghj_having_test VALUES ('a', 100), ('a', 200), ('b', 50), ('c', 10)")
      val viewBody =
        "SELECT region, SUM(amount) AS total FROM agghj_having_test GROUP BY region " +
          "HAVING SUM(amount) > 60"
      spark.sql(s"CREATE MATERIALIZED VIEW agghj_mv_having AS $viewBody")
      assertMvCorrect("agghj_mv_having", viewBody)

      spark.sql("INSERT INTO agghj_having_test VALUES ('b', 100), ('c', 200)")
  ...
  describe("CASE WHEN inside aggregate — agghj_case_test") {
    it("SUM(CASE WHEN value > 0 THEN value ELSE 0 END) stays correct") {
      spark.sql("CREATE TABLE agghj_case_test (category STRING, value INT) USING DELTA")
      spark.sql("INSERT INTO agghj_case_test VALUES ('a', 10), ('a', -5), ('b', 20), ('b', -3)")
      val viewBody =
        "SELECT category, SUM(CASE WHEN value > 0 THEN value ELSE 0 END) AS pos_sum " +
          "FROM agghj_case_test GROUP BY category"
      spark.sql(s"CREATE MATERIALIZED VIEW agghj_mv_case AS $viewBody")
      assertMvCorrect("agghj_mv_case", viewBody)

      spark.sql("INSERT INTO agghj_case_test VALUES ('a', 30), ('b', -100), ('c', 50)")
  ...
  describe("NULL aggregate inputs with edge-case DELETE/INSERT — agghj_agg_null_edge") {
    it("SUM/COUNT(*)/COUNT(val) all stay consistent through NULL-row churn") {
      spark.sql("CREATE TABLE agghj_agg_null_edge (grp STRING, val INT) USING DELTA")
      spark.sql(
      ...
      spark.sql(s"CREATE MATERIALIZED VIEW agghj_mv_agg_null_edge AS $viewBody")
      ...
      spark.sql("DELETE FROM agghj_agg_null_edge WHERE val IS NULL")
      refreshMv("agghj_mv_agg_null_edge")
      assertMvCorrect("agghj_mv_agg_null_edge", viewBody)

      // Insert only NULLs into 'a'
      spark.sql("INSERT INTO agghj_agg_null_edge VALUES ('a', NULL), ('a', NULL)")
      refreshMv("agghj_mv_agg_null_edge")
      assertMvCorrect("agghj_mv_agg_null_edge", viewBody)
    }
  }
  ...
  describe("JOIN + GROUP BY + HAVING — agghj_ajh_orders/agghj_ajh_customers") {
    it("Region totals above threshold stay correct as rows are added / dropped") {
      spark.sql("CREATE TABLE agghj_ajh_orders (id INT, cust_id INT, amount INT) USING DELTA")
      spark.sql("CREATE TABLE agghj_ajh_customers (id INT, region STRING) USING DELTA")
      spark.sql("INSERT INTO agghj_ajh_customers VALUES (1, 'US'), (2, 'EU'), (3, 'US')")
      spark.sql("INSERT INTO agghj_ajh_orders VALUES (1, 1, 100), (2, 1, 200), (3, 2, 150), (4, 3, 50)")
      ...
      spark.sql(s"CREATE MATERIALIZED VIEW agghj_mv_big_regions AS $viewBody")
      assertMvCorrect("agghj_mv_big_regions", viewBody)

      spark.sql("INSERT INTO agghj_ajh_orders VALUES (5, 2, 200)")
      refreshMv("agghj_mv_big_regions")
      assertMvCorrect("agghj_mv_big_regions", viewBody)

      spark.sql("DELETE FROM agghj_ajh_orders WHERE cust_id = 1 AND amount = 200")
      refreshMv("agghj_mv_big_regions")
      assertMvCorrect("agghj_mv_big_regions", viewBody)

      spark.sql("INSERT INTO agghj_ajh_customers VALUES (4, 'APAC')")
      spark.sql("INSERT INTO agghj_ajh_orders VALUES (6, 4, 500), (7, 4, 100)")
      refreshMv("agghj_mv_big_regions")
      assertMvCorrect("agghj_mv_big_regions", viewBody)
    }
  }
  ...
  describe("UPDATE moves rows between groups (SUM + COUNT) — agghj_agg_key_move") {
    it("After GROUP BY key changes, SUM and COUNT both reflect the new groupings") {
      spark.sql("CREATE TABLE agghj_agg_key_move (grp STRING, val INT) USING DELTA")
      spark.sql("INSERT INTO agghj_agg_key_move VALUES ('m', 10), ('m', 20), ('n', 30), ('n', 40)")
      val viewBody =
        "SELECT grp, SUM(val) AS total, COUNT(*) AS cnt FROM agghj_agg_key_move GROUP BY grp"
      spark.sql(s"CREATE MATERIALIZED VIEW agghj_mv_agg_key_move AS $viewBody")
      refreshMv("agghj_mv_agg_key_move")
      assertMvCorrect("agghj_mv_agg_key_move", viewBody)

      spark.sql("UPDATE agghj_agg_key_move SET grp = 'n' WHERE val = 20")
      refreshMv("agghj_mv_agg_key_move")
      assertMvCorrect("agghj_mv_agg_key_move", viewBody)

      spark.sql("UPDATE agghj_agg_key_move SET grp = 'o' WHERE grp = 'm'")
      refreshMv("agghj_mv_agg_key_move")
      assertMvCorrect("agghj_mv_agg_key_move", viewBody)
    }
  }
  ...
  describe("LEFT JOIN + AGGREGATE — agghj_lja_customers LEFT JOIN agghj_lja_orders") {
    it("SUM/COUNT over NULL-extended rows remain correct across DML") {
      spark.sql("CREATE TABLE agghj_lja_customers (id INT, name STRING) USING DELTA")
      spark.sql("CREATE TABLE agghj_lja_orders (cust_id INT, amount INT) USING DELTA")
      spark.sql("INSERT INTO agghj_lja_customers VALUES (1, 'Alice'), (2, 'Bob'), (3, 'Carol')")
      spark.sql("INSERT INTO agghj_lja_orders VALUES (1, 100), (1, 200), (2, 50)")
      ...
      val viewBody =
        "SELECT c.name, SUM(o.amount) AS total, COUNT(o.amount) AS cnt " +
          ...
          "GROUP BY c.name"
      spark.sql(s"CREATE MATERIALIZED VIEW agghj_mv_cust_totals AS $viewBody")
      assertMvCorrect("agghj_mv_cust_totals", viewBody)

      // Add order for Carol + delete all Alice's orders
      spark.sql("INSERT INTO agghj_lja_orders VALUES (3, 75)")
      spark.sql("DELETE FROM agghj_lja_orders WHERE cust_id = 1")
      refreshMv("agghj_mv_cust_totals")
      assertMvCorrect("agghj_mv_cust_totals", viewBody)

      // Delete all orders — everyone NULL-extended
      spark.sql("DELETE FROM agghj_lja_orders")
      refreshMv("agghj_mv_cust_totals")
      assertMvCorrect("agghj_mv_cust_totals", viewBody)
    }
  }
  ...
  describe("HAVING AVG(x) > k decomposes AVG (q0929 regression) — agghj_hva_t") {
    it("AVG inside HAVING is correctly decomposed; delta refresh stays accurate") {
      spark.sql("CREATE TABLE agghj_hva_t(grp INT, v INT) USING DELTA")
      spark.sql("INSERT INTO agghj_hva_t VALUES (1,10), (1,20), (2,-5), (2,-15), (3,100)")

      val viewBody =
        "SELECT grp, AVG(v) AS a FROM agghj_hva_t GROUP BY grp HAVING AVG(v) > 0"
      spark.sql(s"CREATE MATERIALIZED VIEW agghj_mv_hva AS $viewBody")
      ...
      spark.sql("INSERT INTO agghj_hva_t VALUES (2, 100), (4, 50)")
  ...
  describe("HAVING referencing aggregates not in SELECT — agghj_hav_hidden_count") {
    ignore("Hidden COUNT(*) > 3 predicate is correctly evaluated through batched DML") {
      spark.sql("CREATE TABLE agghj_hav_hidden_count (w INT, i INT, amt INT) USING DELTA")
      spark.sql(
      ...
      spark.sql(s"CREATE MATERIALIZED VIEW agghj_mv_hav_hidden_count AS $viewBody")
      assertMvCorrect("agghj_mv_hav_hidden_count", viewBody)

      // Batched DML
      spark.sql("DELETE FROM agghj_hav_hidden_count WHERE w = 1 AND i = 2 AND amt = 50")
      spark.sql(
        "INSERT INTO agghj_hav_hidden_count VALUES " +
          "(1,1,300),(1,1,50),(1,3,500),(1,3,500),(1,3,500),(1,3,500)"
      )
      spark.sql("UPDATE agghj_hav_hidden_count SET amt = amt + 10 WHERE w = 1 AND i = 1")
  ...
  describe("HAVING SUM(COALESCE(...)) over LEFT JOIN — hav_coalesce") {
    it("Two distinct SUM aggregates in HAVING (one wrapped in COALESCE) stay consistent") {
      spark.sql("CREATE TABLE agghj_hav_coalesce_c (id INT, w INT, bal INT) USING DELTA")
      spark.sql("CREATE TABLE agghj_hav_coalesce_h (c_id INT, c_w INT, amt INT) USING DELTA")
      spark.sql("INSERT INTO agghj_hav_coalesce_c VALUES (1,1,100),(2,1,200),(3,1,50)")
      spark.sql("INSERT INTO agghj_hav_coalesce_h VALUES (1,1,30),(2,1,250)")
      ...
      spark.sql(s"CREATE MATERIALIZED VIEW agghj_mv_hav_coalesce AS $viewBody")
  ...
  describe("Fix B CTE AVG/STDDEV — agghj_fixb_customer") {
    it("CTE with AVG/STDDEV plus derived ROUND projections stays correct after batched DML") {
      spark.sql("CREATE TABLE agghj_fixb_customer (c_w_id INT, c_balance DECIMAL(12,2)) USING DELTA")
      spark.sql("INSERT INTO agghj_fixb_customer VALUES (1, 10.00), (1, 20.00), (2, 30.00)")
      ...
      val viewBody =
        "WITH per_warehouse AS (" +
          ...
          "FROM per_warehouse"
      spark.sql(s"CREATE MATERIALIZED VIEW agghj_fixb_mv AS $viewBody")

      spark.sql("INSERT INTO agghj_fixb_customer VALUES (1, 40.00), (2, 50.00), (3, 100.00)")
      spark.sql(
        "UPDATE agghj_fixb_customer SET c_balance = 15.00 WHERE c_w_id = 1 AND c_balance = 20.00"
      )
      spark.sql("DELETE FROM agghj_fixb_customer WHERE c_w_id = 1 AND c_balance = 10.00")
