...
/** P5.rt5 — Coverage of RefreshType 5 (WINDOW_PARTITION).
  ...
  */
class WindowPartitionSpec extends AnyFunSpec with Matchers with BeforeAndAfterAll {

  private val warehouseDir: String = {
    val d = new File(s"target/test-warehouse-wp-${UUID.randomUUID().toString.take(8)}")
    d.mkdirs()
    d.getAbsolutePath
  }
  ...
  override def beforeAll(): Unit = {
    ...
    spark = SparkSession
      .builder()
      .master("local[1]")
      .appName("openivm-spark-WindowPartitionSpec")
      .config(
  ...
  /** Bidirectional `EXCEPT ALL` equivalence check. Projects the MV to the same
    * columns as `expectedSql` before comparing so any internal openivm
    * bookkeeping columns (`openivm_*`) are stripped.
    */
  ...
  describe("(1) ROW_NUMBER PARTITION BY region ORDER BY amount → WINDOW_PARTITION") {
    it("classifies as WindowPartition and refreshes only affected partitions after INSERT") {
      ...
      spark.sql(s"CREATE MATERIALIZED VIEW mv_wp1 AS $viewSql")
      ...
      spark.sql("INSERT INTO sales_wp1 VALUES (7,'east',25)")
      refreshMv("mv_wp1")
  ...
  describe("(2) RANK PARTITION BY region ORDER BY amount DESC → WINDOW_PARTITION") {
    it("classifies as WindowPartition and refreshes correctly after INSERT") {
      ...
      spark.sql(s"CREATE MATERIALIZED VIEW mv_wp2 AS $viewSql")
      ...
      spark.sql("INSERT INTO sales_wp2 VALUES (6,'east',150)")
      refreshMv("mv_wp2")
  ...
  describe("(3) DENSE_RANK PARTITION BY region ORDER BY amount → WINDOW_PARTITION") {
    it("classifies as WindowPartition and DENSE_RANK ties are handled correctly") {
      ...
      spark.sql(s"CREATE MATERIALIZED VIEW mv_wp3 AS $viewSql")
      ...
      spark.sql("INSERT INTO sales_wp3 VALUES (6,'east',10), (7,'east',30)")
      refreshMv("mv_wp3")
  ...
  describe("(4) SUM() OVER (PARTITION BY region) — partition-only window aggregate") {
    it("classifies as WindowPartition; partition totals refresh after INSERT") {
      ...
      spark.sql(s"CREATE MATERIALIZED VIEW mv_wp4 AS $viewSql")
      ...
      spark.sql("INSERT INTO sales_wp4 VALUES (5,'east',300)")
      refreshMv("mv_wp4")
  ...
  describe("(5) LAG PARTITION BY region ORDER BY id → WINDOW_PARTITION") {
    it("classifies as WindowPartition; LAG values refresh after INSERT") {
      ...
      spark.sql(s"CREATE MATERIALIZED VIEW mv_wp5 AS $viewSql")
      ...
      spark.sql("INSERT INTO sales_wp5 VALUES (6,'east',40)")
      refreshMv("mv_wp5")
  ...
  describe("(6) LEAD PARTITION BY region ORDER BY id → WINDOW_PARTITION") {
    it("classifies as WindowPartition; LEAD values refresh after INSERT") {
      ...
      spark.sql(s"CREATE MATERIALIZED VIEW mv_wp6 AS $viewSql")
      ...
      spark.sql("INSERT INTO sales_wp6 VALUES (6,'west',300)")
      refreshMv("mv_wp6")
  ...
  describe("(7) INSERT into a single existing partition only affects that partition") {
    it("partitions not touched by the INSERT keep their original window values") {
      ...
      spark.sql(s"CREATE MATERIALIZED VIEW mv_wp7 AS $viewSql")
      ...
      spark.sql("INSERT INTO sales_wp7 VALUES (6,'east',15)")
      refreshMv("mv_wp7")
  ...
  describe("(8) DELETE within a partition re-windows the survivors") {
    it("DELETE shifts the ROW_NUMBER for the remaining rows of the affected partition") {
      ...
      spark.sql(s"CREATE MATERIALIZED VIEW mv_wp8 AS $viewSql")
      ...
      spark.sql("DELETE FROM sales_wp8 WHERE region='east' AND amount=20")
      refreshMv("mv_wp8")
  ...
  describe("(9) UPDATE within a partition re-windows the affected partition") {
    it("UPDATE that changes the ORDER BY value shifts the ROW_NUMBER assignments") {
      ...
      spark.sql(s"CREATE MATERIALIZED VIEW mv_wp9 AS $viewSql")
      ...
      spark.sql("UPDATE sales_wp9 SET amount=5 WHERE id=3")
      refreshMv("mv_wp9")
  ...
  describe("(10) INSERT into a previously-empty partition creates that partition in the MV") {
    it("a new partition key appears with the correct window values") {
      ...
      spark.sql(s"CREATE MATERIALIZED VIEW mv_wp10 AS $viewSql")

      mvRefreshType("mv_wp10") shouldBe RefreshTypeCode.WindowPartition

      // Brand-new 'north' partition.
      spark.sql("INSERT INTO sales_wp10 VALUES (5,'north',50), (6,'north',100)")
      refreshMv("mv_wp10")
    ...
    it("classifies as WindowPartition; ROW_NUMBER scoped to (region, product) tuples") {
      ...
      val viewSql =
        "SELECT id, region, product, amount, " +
          ...
          "FROM sales_wp11"
      spark.sql(s"CREATE MATERIALIZED VIEW mv_wp11 AS $viewSql")
      ...
      spark.sql("INSERT INTO sales_wp11 VALUES (7,'east','A',5)")
      refreshMv("mv_wp11")
  ...
  describe("(12) Batched mixed DML across multiple partitions consolidates into one REFRESH") {
    it("INSERT + DELETE + UPDATE spread over several partitions yield a correct single-refresh result") {
      ...
      spark.sql(s"CREATE MATERIALIZED VIEW mv_wp12 AS $viewSql")
      ...
      spark.sql("INSERT INTO sales_wp12 VALUES (9,'east',5), (10,'west',50)") // east, west
      spark.sql("DELETE FROM sales_wp12 WHERE region='east' AND amount=20")   // east
      spark.sql("UPDATE sales_wp12 SET amount=999 WHERE id=7")                // north
      // 'south' is a brand-new partition.
      spark.sql("INSERT INTO sales_wp12 VALUES (11,'south',1), (12,'south',2)")
      refreshMv("mv_wp12")
