...
abstract class RefreshShortCircuitScenarios extends IvmParitySpecBase("refresh-short-circuit") {
  ...
  protected def timedRefresh(name: String): Long = {
  ...
  describe("incremental REFRESH no-op short-circuit") {
    it("skips analyzed refresh SQL when no staging rows are pending") {
      sql("CREATE TABLE IF NOT EXISTS rsc_users(id INT, name STRING) USING DELTA")
      sql("INSERT INTO rsc_users VALUES (1, 'Alice')")
      sql("CREATE MATERIALIZED VIEW rsc_mv AS SELECT id, name FROM rsc_users")
      sql("INSERT INTO rsc_users VALUES (2, 'Bob')")
      ...
      sql("INSERT INTO rsc_users VALUES (3, 'Carol')")
