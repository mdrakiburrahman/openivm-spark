...
import org.openivm.spark.common.{MvCatalog, RefreshProfileCatalog, StagingCatalog}
...
/** End-to-end coverage of `RefreshProfile` instrumentation in
  ...
  * parity specs.
  */
class RefreshProfileSpec extends AnyFunSpec with Matchers with BeforeAndAfterAll {
  ...
  private def buildSpark(profileEnabled: Boolean): SparkSession =
    SparkSession
      .builder()
      .master("local[1]")
      .appName(s"openivm-spark-RefreshProfileSpec-${profileEnabled}")
      .config(
      ...
      .config("spark.openivm.profile.refresh", profileEnabled.toString)
  ...
  override def beforeAll(): Unit = {
    super.beforeAll()
    spark = buildSpark(profileEnabled = true)
  ...
  private def clearProfile(): Unit = RefreshProfileCatalog.removeAll(spark)

  private def showProfile(): Seq[Row] =
    spark.sql("SHOW OPENIVM REFRESH PROFILE").collect().toSeq

  private def stepsByRefresh(rows: Seq[Row]): Map[String, Seq[String]] =
    rows.groupBy(_.getString(0)).map { case (rid, rs) =>
      rid -> rs.sortBy(_.getInt(3)).map(_.getString(4))
    }

  private def detailsForStep(rows: Seq[Row], stepName: String): Seq[String] =
  ...
  describe("RefreshProfile — CREATE path") {
    it("emits create_compile_classification, create_mv_initial_load, create_mv_total with _create_mv_ refresh_id") {
      clearProfile()
      spark.sql("CREATE TABLE profile_t1 (k INT, v INT) USING DELTA")
      spark.sql("INSERT INTO profile_t1 VALUES (1, 10), (2, 20)")
      spark.sql("CREATE MATERIALIZED VIEW profile_mv1 AS SELECT k, SUM(v) AS s FROM profile_t1 GROUP BY k")
  ...
  describe("RefreshProfile — REFRESH no deltas") {
    it("emits acquire_locks + total_refresh with no execute_refresh_sql_stmt rows") {
      clearProfile()
      spark.sql("CREATE TABLE profile_t2 (k INT, v INT) USING DELTA")
      spark.sql("INSERT INTO profile_t2 VALUES (1, 10)")
      spark.sql("CREATE MATERIALIZED VIEW profile_mv2 AS SELECT k, SUM(v) AS s FROM profile_t2 GROUP BY k")
      clearProfile() // drop CREATE rows so we only assert on REFRESH

      spark.sql("REFRESH MATERIALIZED VIEW profile_mv2").collect()
  ...
  describe("RefreshProfile — REFRESH with deltas") {
    it("emits execute_refresh_sql_stmt rows with stmt_kind in detail") {
      clearProfile()
      spark.sql("CREATE TABLE profile_t3 (k INT, v INT) USING DELTA")
      spark.sql("INSERT INTO profile_t3 VALUES (1, 10)")
      spark.sql("CREATE MATERIALIZED VIEW profile_mv3 AS SELECT k, SUM(v) AS s FROM profile_t3 GROUP BY k")
      spark.sql("INSERT INTO profile_t3 VALUES (2, 20), (1, 30)")
      clearProfile()

      spark.sql("REFRESH MATERIALIZED VIEW profile_mv3").collect()
  ...
  describe("RefreshProfile — ordering invariant") {
    it("scanAll returns rows ordered by (profile_timestamp, refresh_id, step_order)") {
      clearProfile()
      spark.sql("CREATE TABLE profile_t4 (k INT, v INT) USING DELTA")
      spark.sql("INSERT INTO profile_t4 VALUES (1, 10)")
      spark.sql("CREATE MATERIALIZED VIEW profile_mv4 AS SELECT k, SUM(v) AS s FROM profile_t4 GROUP BY k")
      spark.sql("INSERT INTO profile_t4 VALUES (2, 20)")
      spark.sql("REFRESH MATERIALIZED VIEW profile_mv4").collect()
      spark.sql("INSERT INTO profile_t4 VALUES (3, 30)")
      spark.sql("REFRESH MATERIALIZED VIEW profile_mv4").collect()
  ...
  describe("RefreshProfile — profile gate OFF") {
    it("SHOW OPENIVM REFRESH PROFILE returns 0 rows after CREATE/REFRESH when gate is off") {
      // Tear down active session and rebuild with profile gate off.
      spark.stop()
      spark = buildSpark(profileEnabled = false)
      MvCatalog.ensureTables(spark)
      StagingCatalog.ensureTables(spark)
      RefreshProfileCatalog.ensureTables(spark)
      clearProfile()

      spark.sql("CREATE TABLE profile_t5 (k INT, v INT) USING DELTA")
      spark.sql("INSERT INTO profile_t5 VALUES (1, 10)")
      spark.sql("CREATE MATERIALIZED VIEW profile_mv5 AS SELECT k, SUM(v) AS s FROM profile_t5 GROUP BY k")
      spark.sql("INSERT INTO profile_t5 VALUES (2, 20)")
      spark.sql("REFRESH MATERIALIZED VIEW profile_mv5").collect()
