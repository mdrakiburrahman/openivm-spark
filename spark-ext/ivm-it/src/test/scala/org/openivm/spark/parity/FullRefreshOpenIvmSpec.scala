...
/** P6f — 1:1 ScalaTest port of `openivm/test/sql/full_refresh.test`.
  ...
  */
class FullRefreshOpenIvmSpec extends AnyFunSpec with Matchers with BeforeAndAfterAll {

  private val warehouseDir: String = {
    val d = new File(s"target/test-warehouse-fr-openivm-${UUID.randomUUID().toString.take(8)}")
    d.mkdirs()
    d.getAbsolutePath
  }
  ...
  override def beforeAll(): Unit = {
    ...
    spark = SparkSession
      .builder()
      .master("local[1]")
      .appName("openivm-spark-FullRefreshOpenIvmSpec")
      .config(
  ...
  /** Bidirectional `EXCEPT ALL` equivalence check.
    * Projects the MV onto the column set produced by `expectedSql` so that
    * hidden bookkeeping columns (e.g. `openivm_count_star`) cannot cause a
    * mismatch.
    */
  ...
  /** Marks every currently-pending staging delta for the new MV's actual
    ...
    */
  private def snapshotStagingForMv(mvName: String): Unit = {
  ...
  describe("setup — t1(a, b), t2(a, c) with initial rows") {
    it("creates base tables and loads the initial bag of rows") {
      spark.sql("CREATE TABLE IF NOT EXISTS t1 (a INT, b INT) USING DELTA")
      spark.sql("CREATE TABLE IF NOT EXISTS t2 (a INT, c INT) USING DELTA")
      spark.sql("INSERT INTO t1 VALUES (1, 10), (2, 20), (3, 30)")
      spark.sql("INSERT INTO t2 VALUES (1, 100), (2, 200)")
  ...
  describe("(1) LEFT JOIN materialized view") {
    it("after INSERT into both sides and REFRESH, mv_left equals the live LEFT JOIN") {
      ...
      spark.sql("INSERT INTO t1 VALUES (4, 40)")
      spark.sql("INSERT INTO t2 VALUES (3, 300)")
      refreshMv("mv_left")
  ...
  describe("(3) EXCEPT ALL materialized view (with embedded LEFT JOIN)") {
    it("after INSERT into t1 and REFRESH, mv_except_all equals the live EXCEPT ALL") {
      ...
      spark.sql("INSERT INTO t1 VALUES (99, 100)")
  ...
  describe("(4) STDDEV grouped aggregate") {
    it("after INSERT and REFRESH, mv_stddev equals the live grouped STDDEV (rounded, NaN-normalised)") {
      ...
      spark.sql("INSERT INTO t1 VALUES (1, 15)")
      refreshMv("mv_stddev")
  ...
  describe("(5) STRING_AGG over t1") {
    ...
    it("preserves the openivm-script side-effect INSERT INTO t1 VALUES (5, 50)") {
      spark.sql("INSERT INTO t1 VALUES (5, 50)")
  ...
  describe("(6) CAST projection (incremental in openivm, classifier may differ in Spark)") {
    it("mv_cast equals the live CAST projection in both directions after REFRESH") {
      spark.sql("CREATE TABLE IF NOT EXISTS t3 (x INT, y DECIMAL(10, 2)) USING DELTA")
      spark.sql("INSERT INTO t3 VALUES (1, 10.5), (2, 20.3)")
      spark.sql(
      ...
      spark.sql("INSERT INTO t3 VALUES (3, 30.7)")
      refreshMv("mv_cast")
  ...
  describe("(7) DELETE followed by REFRESH on mv_left") {
    it("after DELETE FROM t1 WHERE a = 1 and REFRESH, mv_left still equals the live LEFT JOIN") {
      spark.sql("DELETE FROM t1 WHERE a = 1")
      refreshMv("mv_left")
