...
run_sbt() {
    compose run --rm -T -e SBT_OPTS="$(sbt_opts)" build sbt "$@"
}
...
cmd_fmt()         { compose run --rm fmt; }
cmd_build()       { compose run --rm build "$@"; }
cmd_assembly()    { compose run --rm assembly; }
cmd_shell()       { compose run --rm shell; }
cmd_image_build() { compose build "$@"; }
cmd_openivm_test(){ compose run --rm openivm-test; }
