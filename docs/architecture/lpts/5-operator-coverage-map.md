...
- `spark-ext/dev/pins.env`: `OPENIVM_COMMIT=aab817f...`.
- `spark-ext/dev/pins.env`: `LPTS_COMMIT=21e7e31...`.
...
2. LPTS converts `LogicalOperator` to an AST (`LogicalPlanToAst`).
3. LPTS flattens the AST to a CTE list (`AstToCteList`).
4. LPTS renders dialect-specific SQL (`duckdb`, `postgres`, or `spark`).
5. `openivm-spark` applies `LptsSparkDialect.translate` to close remaining Spark gaps.
...
- Rewrite the query as conditional aggregates, e.g. `SUM(CASE WHEN k='x' THEN v
  ELSE 0 END) AS x`.
...
2. Selected functions are renamed for Postgres and Spark dialects.
3. Operator-like functions with two children can render in infix form.
...
1. During `CREATE MATERIALIZED VIEW`, OpenIVM tries to run LPTS on a rewritten
   select plan.
2. If LPTS throws `Exception`, `parser.cpp` catches it, sets
   `lpts_fallback=true`, and stores the original user query as `view_query`.
3. If LPTS throws a non-DuckDB exception, the generic catch does the same.
4. Later classification marks unsupported incremental constructs or
   non-incremental-compatible plans as `RefreshType::FULL_REFRESH`.
5. Some constructs use recompute-style refresh types instead of full refresh,
...
| concat | `concat` / `||` | `concat` / `||` | `concat` / `||` | Infix `||` is emitted for operator-like bound functions. |
...
2. Add `ToString`, `NodeType`, and `OutputColumnNames` behavior.
3. Add the `AstBuilder::BuildNode` switch case.
4. Register every output `ColumnBinding` in `column_map`.
