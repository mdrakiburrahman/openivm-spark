...
1. Plan and optimize the original query with DuckDB.
1. Convert that optimized plan to the LPTS AST.
1. Flatten the AST to a CTE list.
1. Render a SQL string with `CteList::ToQuery(true)`.
1. Normalize the original SQL to DuckDB's first parsed statement.
1. Compare `(original EXCEPT ALL generated)` and `(generated EXCEPT ALL original)`.
1. Return one boolean column named `match`.
...
## This matters because LPTS emits SQL for DuckDB's **optimized** plan. `PlanQuery` explicitly parses, plans, and then runs DuckDB's optimizer before LPTS sees the tree. That path is in `.temp/lpts/src/lpts_extension.cpp:57-80`. The source comments name optimizer effects such as CTE inlining, materialized CTEs, statistics propagation, compressed materialization, column lifetime pruning, filter reordering, and join filter pushdown. So the harness is not asking whether LPTS preserves the user's original spelling. It is asking whether LPTS preserves the optimized logical computation. That is the right level for an optimizer-oriented plan-to-SQL tool.
...
## LPTS is not a source-to-source formatter. It often replaces user aliases with internal stable names such as `t0_name` or `t1_aggregate_0`. The final read node restores visible column names when the query result is returned. But alias spelling inside intermediate CTEs is not preserved. The README states this limitation directly: LPTS does not preserve formatting, alias spelling, or original CTE structure. See `.temp/lpts/README.md:78-85`.
...
## The benchmark README says the runner executes the SQLStorm TPC-H query set through `PRAGMA lpts_check` and reports success, DuckDB errors, LPTS errors, unsupported cases, timeouts, and incorrect results. Source: `.temp/lpts/benchmark/README.md:15-36`. In this checkout that SQLStorm query directory was not populated, but the benchmark code and README are present.
...
## `lpts_check` runs in DuckDB. Setting `lpts_dialect='spark'` changes how LPTS renders SQL. It does not move execution into Spark. So a Spark-dialect `lpts_check` can catch many rendering regressions, but it cannot prove every emitted statement is accepted by Spark 3.5. For Spark acceptance, run openivm-spark parity tests.
...
1. Extract the DuckDB logical operator fields in `LogicalPlanToAst`.
1. Register every output `ColumnBinding` that parents may reference.
1. Add or reuse a CTE node in `src/include/cte_nodes.hpp`.
1. Render the CTE body in `src/cte_nodes.cpp`.
1. Add a SQLLogicTest with `PRAGMA lpts_check('<query>')` returning `true`.
1. Run the focused test file.
1. Run the broader regression corpus before pushing.
...
1. Take the MV body.
1. Reduce it to the smallest DuckDB schema and data that exercises the operator.
1. Run `PRAGMA lpts_check` in DuckDB with the default dialect.
1. Run again with `SET lpts_dialect='spark'`.
1. Use OpenIVM `openivm_compile_with_facts` to see the refresh program.
1. Run the actual openivm-spark Scala parity spec against Spark/Delta.
...
## All three are bag-equality checks. They sit at different layers.
...
## This reports aggregate counts for success, DuckDB errors, LPTS errors, unsupported queries, timeouts, nondeterminism, and incorrect results. It can also write a CSV with `--out <csv>`.
...
1. `PRAGMA lpts_exec` for concrete result inspection.
1. `PRAGMA lpts_check` for bag equality.
1. SQLLogicTests for regression coverage.
1. SQLStorm for broad stress coverage.
1. OpenIVM refresh tests for materialized-view maintenance.
1. openivm-spark parity tests for Spark/Delta execution.
...
1. Did `PRAGMA lpts('<query>')` produce parseable SQL?
1. Did `PRAGMA lpts_exec('<query>')` fail or return wrong rows?
1. Is the query nondeterministic?
1. Does the query use unordered order-sensitive aggregates?
1. Does it use `ORDER BY ... LIMIT` with tied keys?
1. Does it use floating aggregates where order changes rounding?
1. Did a new operator path forget to register output column bindings?
1. Did a projection/filter/order node expose a different number of columns than its CTE header?
1. Did a join rewrite preserve row multiplicities?
1. Did a recursive CTE self-reference resolve to the recursive CTE's output names?
1. Did dialect rendering introduce syntax that DuckDB cannot parse?
1. Is this an unsupported operator that should throw `NotImplementedException`?
1. Is the test missing deterministic data or deterministic order keys?
