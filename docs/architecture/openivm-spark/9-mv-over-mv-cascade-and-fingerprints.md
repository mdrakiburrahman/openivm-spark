...
> `MV-over-MV chains are supported at depth <= 2 (ChainedSpec); depth > 2 is out of scope.`
...
1. upstream refresh consumes base staging;
1. upstream refresh records one downstream-consumable `MV_VIEW_DELTA`;
1. downstream refresh consumes that staging;
1. refresh end marks the consumed paths.
...
1. refresh-time per-MV consumed markers.
...
1. create MV;
1. store original fingerprint;
1. `ALTER TABLE ... ADD COLUMNS`;
1. insert a row so refresh attempts work;
1. refresh raises a schema/fingerprint error;
1. drop and recreate the MV;
1. the new fingerprint differs from the original.
...
1. Preserve upstream `openivm_multiplicity` for `MV_VIEW_DELTA`.
1. Match downstream source keys by trailing short name when discovering dependents.
1. Use the exact source-key string stored in downstream `sourceTables` when recording.
1. Do not silently support depth 3+ without graph ordering and cycle checks.
1. Treat fingerprint mismatch as fail-loud drift, not as a transparent cascade replay.
1. Use DROP + CREATE to recover from schema or upstream-identity drift.
1. Keep trigger-only rows separate from signed cascade deltas.
1. Remember `_openivm/tables/.../rocksdb` is catalog state, not the Delta payload.
1. Remember `_ivm/view_deltas/...` is the signed cascade payload.
1. SIMPLE_PROJECTION must preserve the raw signed view-delta as the downstream
   cascade feed, even when the local MV consolidates opposing signs into net
   multiplicities. See chapter 8 §2.9.
