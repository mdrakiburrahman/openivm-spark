...
1. upstream OpenIVM DuckDB compiles it to an incremental refresh type;
1. openivm-spark either records `FULL_REFRESH`, emits no executable refresh SQL,
   or executes refresh SQL that fails or produces the wrong bag;
1. the failure is not an intentional limitation already documented in this
   repository.
...
1. register a DuckDB macro with a type-correct body in
   `OpenIvmCompiler.sparkFunctionShimsPrologue`;
1. post-rewrite the inlined DuckDB macro body back to the original Spark spelling
...
1. Confirm that DuckDB does IVM.
...
1. Add the shim, rewrite, classifier, or assembler case.
...
1. Add or update a parity spec under `spark-ext/ivm-it/src/test/scala/org/openivm/spark/parity/`.
...
1. Did I prove Spark now records the same refresh type?
1. Did I exercise at least one INSERT and, where relevant, DELETE or UPDATE?
1. Did I compare bags in both directions?
1. Did I inspect staging when the failure looked like stale or missing rows?
1. Did I choose the lowest responsible layer?
1. Did I avoid widening demotion?
1. Did I avoid verbose test logging?
1. Did I document the compiled SQL cache in the PR?
1. Did full verification pass?
