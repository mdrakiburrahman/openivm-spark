...
This section covers the **CREATE-time** demotion that persists
`refreshTypeName = FULL_REFRESH` in `MvMetadata` for every subsequent
REFRESH. It does not apply to mixed signed rows. SIMPLE_PROJECTION refreshes
consolidate those rows and apply their net multiplicities incrementally. See
chapter 8 §2.9.
