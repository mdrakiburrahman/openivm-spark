...
## 0.14 Reading order for the rest of the architecture guide

### Start here

1. `1.BOOTSTRAP-AND-FEATURE-GATE.md` — How Spark loads the extension, what is injected, and where the current
   feature-gate discrepancy lives.
2. `2.SQL-SURFACE-AND-PARSER.md` — The exact DDL grammar, parser delegation rules, and command logical plans.
3. `3.MODULES-AND-BUILD.md` — The sbt module graph, dependency pins, assembly shading, and development entry points.

### Deep dives

4. `4.STATE-SYSTEMS.md` — RocksDB metadata layout, Delta data paths, watermarks, consumed markers, and migration
   from legacy Delta metadata tables.
5. `5.DML-INTERCEPTION-AND-STAGING.md` — How INSERT/DELETE/UPDATE/MERGE/OVERWRITE are captured without Delta CDF.
6. `6.CREATE-MATERIALIZED-VIEW.md` — Source discovery, compiler invocation, refresh-type demotion, initial load,
   and metadata persistence.
7. `7.REFRESH-MATERIALIZED-VIEW.md` — Refresh locking, staging collection, schema validation, execution, and cleanup.
8. `8.COMPILE-BRIDGE.md` — DuckDB CLI subprocess construction, OpenIVM pragmas, JSON-lines parsing, and compile cache.
9. `9.LPTS-SPARK-DIALECT.md` — The pure string translations that convert LPTS/DuckDB spellings into Spark SQL.
10. `10.SPARK-REFRESH-REWRITER.md` — Statement classification, view-delta CTAS, MERGE/DELETE/INSERT rewrites, and
    Spark-specific execution markers.
11. `11.REFRESH-TYPES.md` — The ten ordinals, supported Spark strategies, demotions, and operator-family caveats.
12. `12.MV-OVER-MV-CASCADE.md` — Depth-2 cascade semantics, `MV_VIEW_DELTA`, trigger rows, and non-cascade upstreams.

### Debugging

13. `13.CONCURRENCY-RETRY-AND-FAILURES.md` — Per-MV mutexes, Delta conflict retry, at-least-once gaps, and failure
    cleanup paths.

### Demos

14. `14.END-TO-END-DEMO.md` — A small runnable walkthrough from CREATE through DML staging to REFRESH and DROP.

## 0.15 One-screen summary
