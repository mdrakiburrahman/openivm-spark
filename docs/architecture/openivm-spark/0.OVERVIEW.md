...
openivm-spark is a shared Spark SQL extension for Spark 3.5 / Delta Lake 3.2
and Spark 4.1 / Delta Lake 4.2. It brings OpenIVM-style incremental
materialized-view maintenance to Spark without requiring Delta Change Data
Feed.
...
1. `IvmDmlInterceptorRule`, through `injectResolutionRule`.
1. `IvmStrategy`, through `injectPlannerStrategy`.
...
The runtime matrix is pinned in `spark-ext/dev/targets/`: Spark 3.5.1 / Delta
3.2.0 / Scala 2.12.17 / Java 17 and Spark 4.1.0 / Delta 4.2.0 / Scala 2.13.17 /
Java 21. DuckDB JDBC remains 1.5.2.1 for ABI alignment
(`spark-ext/project/RuntimeTarget.scala`, `spark-ext/project/Dependencies.scala`).
...
```mermaid
graph TB
    subgraph Spark["Selected Spark / Delta runtime"]
        SQL[User SQL]
...
view, invokes `SELECT * FROM openivm_compile_with_facts('<view>', '<facts_json>')`, and parses the emitted SQL program
(`spark-ext/ivm-compiler/src/main/scala/org/openivm/spark/compiler/OpenIvmCompiler.scala:58-87`).
The facts JSON is exactly `{"target_dialect":"spark","compile_only":true,"force_view_delta_cascade":true}`.
It is passed as a single argument to `openivm_compile_with_facts`; no global session state is touched for dialect,
compile-only mode, or cascade behavior, so concurrent compiles cannot interfere via leaked flags.
...
- `SELECT * FROM openivm_compile_with_facts('<view>', '{"target_dialect":"spark","compile_only":true,"force_view_delta_cascade":true}')`
...
|       7 | `TOP_K`                | Dead direct enum; top-k rides on its inner type using a backing table plus Spark VIEW.     |
...
The Spark CREATE path extracts top-level `ORDER BY`/`LIMIT`/`OFFSET`, keeps the
inner compiler type, stores the unlimited state in `<mv>__ivm_data`, and exposes
the named MV as a Spark VIEW. Unsupported `TAIL` wrappers remain full refresh.
...
sequenceDiagram
    ...
    Compiler->>DuckDB: duckdb :memory: -jsonlines, LOAD extension, openivm_compile_with_facts
    ...
    Compiler->>DuckDB: openivm_compile_with_facts if cache miss
...
1. Resolves source tables and full source schemas from Spark's analyzed plan
   (`spark-ext/ivm-extension/src/main/scala/org/openivm/spark/commands/MaterializedViewCommands.scala:112-173`).
1. Invokes `OpenIvmCompiler.compile`
   (`spark-ext/ivm-extension/src/main/scala/org/openivm/spark/commands/MaterializedViewCommands.scala:359-393`).
1. Applies Spark-specific demotions and capability checks, including unsupported top-k tails, no-real-delta, unsafe HAVING, and
...
1. Persists cached compiled SQL and metadata properties when the view stays incremental
   (`spark-ext/ivm-extension/src/main/scala/org/openivm/spark/commands/MaterializedViewCommands.scala:653-674`).
1. Creates the initial Delta table using the OpenIVM initial-load SQL when appropriate, otherwise the original
   user query
   (`spark-ext/ivm-extension/src/main/scala/org/openivm/spark/commands/MaterializedViewCommands.scala:696-718`).
1. Writes `MvCatalog` metadata and records the initial Delta version
