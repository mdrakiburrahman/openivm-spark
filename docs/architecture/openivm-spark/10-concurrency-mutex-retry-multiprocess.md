...
2. the Delta-conflict `RetryPolicy`, and
3. the optional RocksDB multi-process file lock.
...
1. `MvCatalog.lookup`,
2. `StagingCatalog.hasPendingDeltas`,
3. `StagingCatalog.collectFor`,
4. schema fingerprint validation,
5. compile-cache lookup or compile-cache back-fill,
6. temp delta view registration,
7. refresh SQL rewrite,
8. Delta SQL execution,
9. count-monoid zero-row cleanup,
10. cascade view-delta recording, and
11. `postRefreshCleanup`.
...
2. some exception in the cause chain matches a configured regex.
...
2. acquire and release a POSIX file lock,
3. open RocksDB cold, and
4. flush and close RocksDB.
...
2. coordinate `REFRESH MATERIALIZED VIEW` calls externally so only one process refreshes a given MV at a time.
...
2. optionally delete zero-count rows for count-monoid paths,
3. optionally record an upstream MV view-delta for cascades,
4. read the latest Delta table version,
5. write `MvCatalog.advance`,
6. write `StagingCatalog.markConsumed`,
7. list MVs, and
8. prune fully consumed staging.
...
2. compare the MV against its full query result,
3. rebuild the MV if it diverged, and
4. resume serialized refreshes.
