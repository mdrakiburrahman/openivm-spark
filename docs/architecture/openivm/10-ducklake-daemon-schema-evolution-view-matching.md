...
2. open a DuckDB connection;
3. read cascade and backoff settings;
4. query scheduled views;
5. skip views not yet due;
6. skip views already locked;
7. call `PRAGMA refresh('view')`;
8. run optional refresh hooks;
9. apply adaptive backoff if refresh duration exceeds the configured interval.
Citations:
...
2. looks up eligible registered MVs in `MvCatalog`;
3. proves query/MV equivalence or containment;
4. reasons about freshness and pending staging deltas;
5. rewrites the Spark plan to scan the MV or MV plus residual delta compensation;
6. preserves Spark semantics, permissions, statistics, and explainability.
...
2. allow unreferenced column drops when refresh SQL never references them;
3. update shadow DuckDB DDL generation after safe schema changes;
4. rewrite stored MV SQL on column rename using Spark analyzed plans;
5. update downstream fingerprints and pending staging rows transactionally.
