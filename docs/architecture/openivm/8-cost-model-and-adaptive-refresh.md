...
`D` is the base data.
$\Delta D$ is the pending delta.
`|D|` is the size of the base.
$|\Delta D|$ is the size of the change set.
...
So $|\Delta D| / |D|$ is the starting point.
...
| ---------------------------------------------------------------------------- | ---------------------------------------------------------- | ---------------------------------------- |
| `FULL_REFRESH`                                                               | \`O(                                                       | D                                        |
| `SIMPLE_PROJECTION`                                                          | $O(\|\Delta D\|)$                                          | $\|\Delta D\| << \|D\|$                  |
| `SIMPLE_AGGREGATE`                                                           | $O(\|\Delta D\|)$ plus a small MV update                   | usually, unless the full plan changes    |
| `AGGREGATE_GROUP`                                                            | $O(\|\Delta D\|)$ plus touched-group merge                 | $\|\Delta D groups\| << \|all groups\|$  |
| `JOIN_INCREMENTAL (n=2)`                                                     | $O(\|\Delta R\| \times \|S\| + \|R\| \times \|\Delta S\|)$ | one side is small or one delta is sparse |
| `JOIN_INCREMENTAL (n=k)`                                                     | up to `O(2^k - 1)` terms                                   | small `k`, sparse deltas, pruning works  |
| `WINDOW_PARTITION` single source                                             | $O(\|\Delta D\| + \|touched partitions\|)$                 | touched partitions are few               |
| `WINDOW_PARTITION` multi source                                              | approximately \`O(                                         | D                                        |
