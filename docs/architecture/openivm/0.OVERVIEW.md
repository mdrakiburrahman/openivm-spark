...
2. What PRAGMA/settings contract does the extension expose?
3. What modules own parsing, classification, algebraic rewrite rules, and SQL emission?
4. What are the 10 `RefreshType` enum ordinals?
5. What math ideas should you remember before reading the details?
6. What DuckDB catalog tables and physical OpenIVM tables are created?
7. Which chapter should you read next for each subsystem?
...
Insertions are positive weights, deletions are negative weights, and refresh is the problem of applying `ΔQ` to a stored `Q`.
...
The derivative operator says: given a query `Q` and a source delta `ΔR`, produce `ΔQ`.
The integral operator says: accumulate those deltas into the stored materialized view.
In code, rewrite rules derive `ΔQ`, and upsert compilation integrates it into `openivm_data_<view>`.
...
DML has already changed the base table by the time refresh SQL runs, so a base scan sees `R_now = R_old + ΔR`.
