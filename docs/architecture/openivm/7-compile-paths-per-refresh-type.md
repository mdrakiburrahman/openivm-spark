...
2. The script loads the OpenIVM extension.
3. It sets `openivm_target_dialect='spark'`.
4. It sets `openivm_compile_only=true`.
5. It registers empty DuckDB source tables matching Spark schemas.
6. It creates a DuckDB materialized view from the Spark MV body.
7. It runs `PRAGMA compile_refresh('<view>')`.
8. `CompileRefreshQuery` calls `GenerateRefreshSQL`.
9. `GenerateRefreshSQL` looks up MV metadata and the CREATE-time `RefreshType`.
10. The dispatcher chooses the compile helper for that type.
11. Non-recompute paths first run `ComputeDelta` through optimizer rules.
12. LPTS serializes the rewritten logical delta plan to SQL.
13. The compile helper emits the upsert program for the MV data table.
14. The assembler surrounds that with metadata and cleanup statements.
15. The pragma returns one JSON row with the full refresh program in `sql`.
16. The CREATE side file `openivm_compiled_queries_<view>.sql` supplies `initialLoadSql`.
...
2. Read the classified `RefreshType` from `openivm_views` metadata.
3. Temporarily force `openivm_compile_only=true`.
4. Call `GenerateRefreshSQL`.
5. Restore the user's prior compile-only setting.
6. Return a single-row result with:
   - `refresh_type`;
   - `refresh_type_name`;
   - `sql`.
...
2. Fetch `view_query_sql` from metadata.
3. Fetch `view_query_type` from metadata.
4. Check recovery and forced full-refresh modes.
5. Load the delta view column names and types.
6. Compute fast-path flags such as insert-only.
7. Switch on `RefreshType`.
8. Call the type-specific compiler.
9. Optionally compute the view delta through LPTS.
10. Add cascade companion statements when requested.
11. Add OpenIVM metadata updates and cleanup statements.
12. Return the assembled program.
...
2. Multi-source DuckLake lineage partition refresh.
3. Multi-source DuckLake view-diff fallback to identify partitions.
4. Standard delta lineage partition refresh.
5. Full recompute fallback when lineage is incomplete.
6. `CompileWindowRecompute` for the normal standard path.
...
2. Optional cascade companion append.
3. Delta MERGE into the MV or backing table.
4. Optional delete-empty-groups statement.
...
2. Split the `sql` field on semicolons.
3. Classify each statement using chapter 6's `StatementKind` table.
4. Verify that every data-bearing statement is recognized.
5. Check whether an optional path added unrecognized TEMP-table SQL.
6. Confirm `LptsSparkDialect.translate` handles remaining DuckDB syntax.
7. For HAVING, verify Spark writes the backing table, not the public view.
8. For WINDOW, check whether the emitted SQL is partition-scoped or full recompute.
9. For GROUP_RECOMPUTE, check whether cascade snapshots are enabled.
10. For joins, debug the LPTS view-delta CTE first, then the upsert MERGE.
