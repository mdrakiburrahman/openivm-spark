...
2. A transaction is also a function from rows to integer weights.
3. Insertions are positive weights.
4. Deletions are negative weights.
5. Updates are a deletion plus an insertion.
6. A materialized view is a function over those weighted tables.
7. Incremental refresh computes the view's **change**, not the view from scratch.
8. That change is added to the materialized view state.
...
$$
Where `ΔS_t(x)` is the change in tuple `x` at time `t`.
...
2. A general algorithm for IVM over DBSP programs.
3. Models for full relational queries, grouping, aggregation, recursion, and streaming aggregation.
Reference:
...
- `ΔD` is the next input signal.
- `D \oplus ΔD` is the next state.
- `V(D \oplus ΔD) \ominus V(D)` is the output signal.
...
- `ΔD` be a source delta, represented as a Z-set.
- `V^Δ` be the delta version of `V`.
- `⊕` be Z-set addition.
...
$$
I will use `V^Δ` for the delta function to avoid overloading the prime on both sides.
...
2. Read the base-table delta `ΔD`.
3. Compute `V^Δ(D, ΔD)`.
4. Add it into the MV table.
5. Delete rows whose multiplicity/count reaches zero.
...
- The current scan reads `R_now = R_old + ΔR`.
...
With OpenIVM's current-base scans, the contribution for a non-empty subset `S ⊆ {1..n}` is:
$$
sign(S) = (-1)^{|S|-1}
$$
and:
...
\Delta R \Join \Delta S
$$
One duplicate `ΔR ⋈ ΔS` remains, which is exactly the textbook delta term.
...
$$
Same result.
Different implementation basis.
---
...
2. A user-facing Spark VIEW that applies the HAVING predicate.
...
| tuple | `R` multiplicity | `ΔR` multiplicity | `R ⊕ ΔR` multiplicity |
|---|---:|---:|---:|
| `(1,A)` | 1 | -1 | 0 |
| `(2,B)` | 2 | 0 | 2 |
| `(3,C)` | 0 | 1 | 1 |
...
| key | `V(R)` | `V^Δ(ΔR)` | `V(R) ⊕ V^Δ(ΔR)` | `V(R ⊕ ΔR)` |
|---:|---:|---:|---:|---:|
| 1 | 1 | -1 | 0 | 0 |
| 2 | 2 | 0 | 2 | 2 |
| 3 | 0 | 1 | 1 | 1 |
...
2. Calls OpenIVM's compiler.
3. Receives a `RefreshType` and SQL program.
4. Creates the initial MV table.
5. Stores metadata and compiled SQL.
The REFRESH path:
1. Collects staged source deltas.
2. Creates temp views over those deltas.
3. Rewrites the compiled SQL for Spark.
4. Executes the SQL statements.
5. Marks staged deltas consumed.
6. Applies count cleanup for count-monoid paths.
...
2. **OpenIVM arXiv version.** The project guide links *OpenIVM: a SQL-to-SQL Compiler for Incremental Computations* at <https://arxiv.org/abs/2404.16486> and says it is the theoretical reference for DBSP, Z-sets, and delta rules. Citation in repo: `.temp/openivm/CLAUDE.md:23-25`.
3. **Differential Dataflow.** Frank McSherry, Derek G. Murray, and Daniel J. Abadi. CIDR 2013. PDF: <https://www.cidrdb.org/cidr2013/Papers/CIDR13_Paper117.pdf>. This is the classic reference for data-parallel differential computation over changing collections.
4. **DBSP: Automatic Incremental View Maintenance for Rich Query Languages.** Mihai Budiu, Tej Chajed, Frank McSherry, Leonid Ryzhyk, and Val Tannen. PVLDB 2023 / arXiv:2203.16684. Links: <https://arxiv.org/abs/2203.16684>, <https://www.vldb.org/pvldb/vol16/p1601-budiu.pdf>. This is the clearest academic reference for Z-sets plus streams as a general IVM model.
5. **F-IVM / factorized IVM / Frances-IVM.** The OpenIVM paper positions OpenIVM in the SQL-to-SQL IVM line and is the best local citation for how these ideas connect to OpenIVM. Start with the OpenIVM paper above, then follow its related-work section.
6. **LPTS documentation.** LPTS is the Logical Plan To SQL bridge used by OpenIVM's SQL compilation flow. The README describes optimized-plan inspection and cross-system SQL compilation: `.temp/lpts/README.md:1-5`. A standalone LPTS paper is not present in the checkout; the README has a TODO for `LPTS_Research_Project_Report.pdf`: `.temp/lpts/README.md:93-98`.
7. **OpenIVM join implementation.** For the concrete Möbius inclusion-exclusion sign used by this codebase, read `.temp/openivm/src/rules/join.cpp:873-891`.
8. **OpenIVM classifier implementation.** For the actual decision tree, read `.temp/openivm/src/core/incremental_checker.cpp:103-400` and `.temp/openivm/src/core/parser.cpp:610-797`.
---
