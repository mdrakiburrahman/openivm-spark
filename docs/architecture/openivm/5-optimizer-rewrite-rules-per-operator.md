...
- **Input → output example:**
  ...
  Output plan: `Term0(Δa ⋈ b_old) UNION ALL Term1(a_now ⋈ Δb)` with snapshot annotations on later leaves.
...
- **Math:** Selection is linear over Z-sets (`Δσ_p(R)=σ_p(ΔR)`), so ordinary filters are safe. HAVING is non-local over aggregate state; chapter 1 treats it as a predicate over materialized group state, so OpenIVM stores all groups and applies HAVING at the user-view boundary.
...
- **Emitted SQL shape:** The logical SQL shape is `SELECT cols, +w FROM ΔR JOIN S UNION ALL SELECT cols, +w FROM R JOIN ΔS UNION ALL SELECT cols, -w1*w2 FROM ΔR JOIN ΔS ...`. `join_output.cpp` assembles the union.
- **Input → output example:**

  Input SQL: `SELECT * FROM r JOIN s ON r.k=s.k`.

  Output refresh subplan: `Project(cols, w_r) over Δr⋈s_now UNION ALL Project(cols, w_s) over r_now⋈Δs UNION ALL Project(cols, -w_r*w_s) over Δr⋈Δs`.
...
- **Math:** Projection is linear: `Δπ_f(R)=π_f(ΔR)` as long as the multiplicity is carried along. This is chapter 1's homomorphism for maps over Z-sets.
...
- **Math:** It materializes the chapter 1 input delta `ΔR`: base-table writes become signed Z-set rows with timestamp. Atomic UPDATE preserves the invariant that `-old` and `+new` are observed together.
...
- **Math:** This is the base case of all chapter 1 delta rules: `ΔR` is read from the tracked delta relation rather than the base relation.
...
- **Math:** Top-k and limit are non-linear over Z-sets: `ΔLIMIT(R)` is not `LIMIT(ΔR)`. Chapter 1 treats this as requiring recompute or boundary-state reasoning, so this rule avoids pretending it is linear.
...
- **Math:** UNION ALL is addition in the Z-set semiring: `Δ(R ⊎ S)=ΔR ⊎ ΔS`. The multiplicity columns carry the signed weights from both sides.
...
The join rule has to account for OpenIVM's physical timing: source base tables have already been updated when refresh runs, and delta tables record the signed changes. Therefore a base scan reads `R_now = R_old + ΔR`, not `R_old`.
...
|---|---|---|
| `aggregate.cpp` | `SELECT k, SUM(v) FROM t GROUP BY k` | `Aggregate(k,w; SUM(v)) over Δt`. |
...
| `join.cpp` | `r JOIN s` | `Δr⋈s UNION ALL r⋈Δs UNION ALL -Δr⋈Δs`. |
| `join_output.cpp` | Term vector | Left-deep `UNION ALL` plus clean projection. |
| `projection.cpp` | `SELECT f(a) FROM t` | `Project(f(a),w) over Δt`. |
...
| `union.cpp` | `q1 UNION ALL q2` | `Δq1 UNION ALL Δq2`. |
...
| Join delta union | `join.cpp`, `join_output.cpp` | `SELECT ..., +w FROM ΔR JOIN S UNION ALL SELECT ..., +w FROM R JOIN ΔS UNION ALL SELECT ..., -w1*w2 FROM ΔR JOIN ΔS` |
...
2. Projection and filter rules are child-first so their expressions continue to refer to rewritten child bindings.
3. Aggregates add multiplicity as a group key after their child has produced it.
4. Joins do not simply rewrite children once; they own the whole join subtree so they can enumerate delta masks and attach signs.
5. Distinct replaces itself with an aggregate and must repair parent bindings immediately, otherwise ancestors still point at the removed distinct node.
6. Top-k is intentionally late in the switch but local in effect: when encountered, it is stripped and the child is rewritten.
7. CTEs are special because references must learn about the added multiplicity column between rewriting the definition and rewriting the consumer.
8. No fixed-point iteration means every rule must leave a fully valid logical subtree on its first return.
