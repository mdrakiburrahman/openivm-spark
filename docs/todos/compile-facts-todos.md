...
- [ ] **⭐ W‑FULL — Cost‑based FULL‑REFRESH routing for high‑affected WINDOW MVs (the telemetry‑discovered beat‑vanilla lever).** _Target: the window MVs read 6–10× vanilla; this closes the gap by making them MATCH vanilla._ **Why (per‑execution telemetry, SF3):** openivm's WINDOW recompute is a Delta **MERGE that scans the one‑file MV ~5×** (`daily_market` reads **7.72M** to write 52K, 34 s) whereas **vanilla recomputes it in one clean CTAS — 1.28M, 4.1 s**. Because a TPC‑DI batch touches _most_ symbols, partition‑pruning (clustering) and suffix‑extend (P5.2) can't help. **So: when a `WINDOW_PARTITION` MV's affected‑fraction is high (use P1.0 delta stats), route that batch to a single‑pass full recompute (clean CTAS / `INSERT OVERWRITE`) instead of the 5‑pass partition‑scoped DELETE+INSERT MERGE.** Windows then match vanilla (~4 s) while `fact_market_history` (38×) etc. beat it → openivm total **beats vanilla**. **Cascade caveat (the design question):** a full‑refreshed window MV that feeds a downstream MV must either also full‑refresh that (small, gold) downstream, or still emit a cheap view‑delta — needs a depth‑≤2 cascade check. Files: `MaterializedViewCommands` window dispatch (add the overwrite path + the affected‑fraction gate), the cascade `postRefreshCleanup`. Gate: EXCEPT‑ALL green on all window + downstream specs; SF3/SF10 `daily_market`/`dim_customer` openivm `records_read` ≈ vanilla.
- [ ] **P5.2 — Running‑aggregate auxiliary state (openivm‑C++).** _Target: the ~99 s WINDOW full‑partition recompute — the #1 residual cost._ **STATUS 2026‑06‑30 (telemetry‑validated): P5.2 NOW FIRES on the real `daily_market` window‑over‑window shape and is CORRECT, but it READS the full MV → no net win yet; the fix is a persisted aux‑state table.** Progress this session: openivm‑side suffix‑extend emission + flag wiring + `SparkRefreshRewriter` translation (openivm‑spark `a209878`/`e85ade7`, openivm `wt/p52-running-agg` `3c2f942`), all validated end‑to‑end — `WindowRunningIncrementalSpec`+`CdfSpec` 4/4 (incl. the real `daily_market` window‑over‑window: cumulative `min/max` → CASE flags → cumulative `max`), bidirectional `EXCEPT ALL`, MV stays WINDOW_PARTITION, no regressions; **SF3 all 3 batches EXCEPT‑ALL green.** **Per‑MV telemetry verdict (the key result):** `daily_market` openivm B2 **writes 52K (= O(delta) ⇒ the suffix‑append fast path fired and the assumption holds)** but **reads 7.72M (6× vanilla)** because the emission's `bounds` (`MAX(order) … GROUP BY` all partitions) + `state` (`ROW_NUMBER … FROM openivm_data`) **scan the FULL MV** → 34.4 s vs vanilla 4.1 s. _Per‑execution telemetry later showed the dominant cost is actually the recompute **MERGE** scanning the full MV ~5× — see **W‑FULL** above, which supersedes this for the high‑affected TPC‑DI window workload._ **If pursued anyway (for strict‑suffix workloads): persisted per‑partition aux‑state table** (1 row/partition: last running output values + max order), read in O(partitions) — building‑block bounds‑filter already in `3c2f942`. Other windows each need their own increment: `dim_customer` (52 s, **worst**, `last_value`), `trades_history` (33 s @ SF10, `lag/row_number`); `trades` (unordered) not cheaply suffix‑extendable. **Tooling:** validate openivm changes fast via `GEN=ninja make` + stage `duckdb`/`extension` under `spark-ext/target/` + `docker compose run -e OPENIVM_CLI_PATH=… -e OPENIVM_EXTENSION_PATH=…`; measure with ivm‑bench `metrics_by_model.parquet` (`records_read`/`records_written` per MV — deterministic, decisive).
