...
- [ ] **P5.2 — Running‑aggregate auxiliary state (openivm‑C++).** _Target: the ~99 s WINDOW full‑partition recompute — the #1 residual cost, and the ONLY un‑disproven lever against it._ **STATUS 2026‑06‑30: first increment LANDED + independently validated openivm‑side** (fork branch `wt/p52-running-agg`, commit `ada5ecb`). A new gated compile flag (`openivm_running_window_incremental`, default OFF) detects pure cumulative `SUM/MIN/MAX/COUNT/AVG OVER (PARTITION BY k ORDER BY d …UNBOUNDED PRECEDING…)` windows and, for an append‑only batch, splits affected partitions into a **suffix‑append fast path** (`delta_min_order > old_max_order`) and a **per‑partition full‑recompute fallback** (`delta_min_order <= old_max_order`, i.e. backdated rows) — so the affected partitions' suffix is extended from the persisted running value instead of recomputed from base. Two native sqllogictests pass (baseline 12 + incremental 21 assertions, incl. asserting BOTH fast+fallback fire and bidirectional `EXCEPT ALL` after suffix‑append AND backdated batches); existing window/cascade tests unregressed. **This matches the SF10 `daily_market` 52‑week MIN/MAX shape** (`min/max OVER (PARTITION BY symbol ORDER BY date)`). **Why this is the only window lever left (2026‑06‑30 probe):** a clean `.explain()` proved Spark _already_ pushes the partition‑key `IN (Δ)` filter **below** the `Window` — so the WINDOW cost is the **full source scan** + DELETE/INSERT rewrite, which only physical clustering (W7.2, write‑overhead‑negated) or this aux‑state path can avoid. **Build is UNBLOCKED:** `apt install ninja-build && rm -rf build && GEN=ninja make -j$(nproc)` in `.temp/openivm` (the prior "broken build" was a missing `ninja` + stale Unix‑Makefiles cache, _not_ DuckLake/roaring). **NEXT increment (to get the SF10 win):** (1) wire the flag through openivm‑spark `CompileFacts`; (2) teach `SparkRefreshRewriter` the new emission shape (`openivm_bounds_*`/`openivm_run_fast_*`/`openivm_run_fallback_*` temp tables → Spark temp views + MERGE); (3) bump `OPENIVM_COMMIT`, rebuild the bench image, SF10. **Scope caveat:** only fires on the cumulative‑aggregate shape — `dim_customer` (`last_value`), `trades` (unordered `min/max`), `trades_history` (`lag/row_number`) each need their own increment; and the win is conditional on the batch being suffix‑append (later dates) per the P5.1 lesson.
