...
- [ ] **P5.2 — Running‑aggregate auxiliary state (openivm‑C++).** _Target: the ~99 s WINDOW full‑partition recompute — the #1 residual cost._ **STATUS 2026‑06‑30 (telemetry‑validated): P5.2 NOW FIRES on the real `daily_market` window‑over‑window shape and is CORRECT, but it READS the full MV → no net win yet; the fix is a persisted aux‑state table.** Progress this session: openivm‑side suffix‑extend emission + flag wiring + `SparkRefreshRewriter` translation (openivm‑spark `a209878`/`e85ade7`, openivm `wt/p52-running-agg` `3c2f942`), all validated end‑to‑end — `WindowRunningIncrementalSpec`+`CdfSpec` 4/4 (incl. the real `daily_market` window‑over‑window: cumulative `min/max` → CASE flags → cumulative `max`), bidirectional `EXCEPT ALL`, MV stays WINDOW_PARTITION, no regressions; **SF3 all 3 batches EXCEPT‑ALL green.** **Per‑MV telemetry verdict (the key result):** `daily_market` openivm B2 **writes 52K (= O(delta) ⇒ the suffix‑append fast path fired and the assumption holds)** but **reads 7.72M (6× vanilla)** because the emission's `bounds` (`MAX(order) … GROUP BY` all partitions) + `state` (`ROW_NUMBER … FROM openivm_data`) **scan the FULL MV** → 34.4 s vs vanilla 4.1 s. **THE FIX (next increment): persisted per‑partition aux‑state table** (1 row/partition: last running output values + max order), maintained incrementally and read in O(partitions) — eliminates the full‑MV bounds/state scans (a building‑block bounds‑filter is already in `3c2f942`). _Clustering won't substitute_: a TPC‑DI batch touches most symbols, so `WHERE part IN affected` prunes little. **Then the other windows** (each its own increment): `dim_customer` (52 s, **worst**, `last_value`), `trades_history` (33 s @ SF10, `lag/row_number`); `trades` (unordered) is not cheaply suffix‑extendable. Beat‑vanilla needs ~most of the ~92 s window cost (then B2 ≈ 202 < vanilla 222) — multi‑increment. **Tooling:** validate openivm changes fast via `GEN=ninja make` + stage `duckdb`/`extension` under `spark-ext/target/` + `docker compose run -e OPENIVM_CLI_PATH=… -e OPENIVM_EXTENSION_PATH=…`; measure with ivm‑bench `metrics_by_model.parquet` (`records_read`/`records_written` per MV — deterministic, decisive).
