...
> **P1.1‑PRE — openivm insert‑only fast‑path Spark‑emission correctness (UNBLOCK; prerequisite for P1.1).** *(we own openivm — fix the fork; this is a roadmap task, not a wall)*
>
> **Blocker (empirical, 2026‑06‑28).** Activating `assume_insert_only` recompiles to openivm insert‑only SQL that is **not execution‑correct in the compile→Spark path**: a CDF parity run failed **15/36 `EXCEPT ALL` checks even on append‑only INSERTs** (SUM, MIN/MAX, projection), off by 1–2 rows vs the general affected‑group recompute. The bridge's pre‑existing `SET openivm_minmax_incremental=false` guard already documented this ("silently produces wrong values").
>
> **Fix.** Make the insert‑only emission **bag‑equal to the general affected‑group recompute when executed in Spark**, decomposed by assembler (disjoint, parallel‑safe): **(A)** projection `skip_proj_delete`; **(B)** `CompileAggregateGroups` — both SUM/COUNT `skip_agg_delete` and MIN/MAX `minmax_incremental`; **(C)** `CompileSimpleAggregates` (scalar).
>
> **Diagnose algorithm vs dialect (fast loop).** Compile with `target_dialect:duckdb + assume_insert_only`, run the emitted DuckDB SQL on real append‑only data, bidirectional `EXCEPT ALL` vs base. Fails in DuckDB ⇒ assembler/algorithm fix (fast `dev.sh dev-build` loop — layer worktree source onto the `openivm-tester` base image, incremental `make` ~1–2 min). Passes in DuckDB but fails in Spark ⇒ LPTS / `SparkRefreshRewriter` Spark‑dialect fix.
>
> **Files:** `src/upsert/refresh_sql.cpp` (655/675/684/704/710/733), the `Compile*` assemblers in `src/upsert/refresh_compiler*.cpp`, `third_party/lpts` Spark render; new `test/sql/*.test` per assembler. **Gate:** spark‑ext CDF parity with insert‑only **active** (the exact 15 checks that failed) + the golden‑SQL net (P0.3) + full `verify`. **Deps:** P1.0, P0.2. **Effort:** L. **Win:** unblocks every insert‑only fast path (P1.1) — removes the full‑MV DELETE / group recompute on every *proven* append‑only refresh. The conservative `DeltaCommitClassifier` keeps mixed‑DML on the correct general SQL, so correctness is preserved throughout.
