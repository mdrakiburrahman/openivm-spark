...
>
> - **Issue #13 (Delta table layout, SF10‑tested 2026‑07‑01):** per‑`RefreshType` `CLUSTER BY` (`MvLayoutPolicy`) + out‑of‑band maintenance daemon (`DeltaMaintenanceCoordinator` — OPTIMIZE compaction + VACUUM, in the extension, **never** ivm‑bench). All **default‑OFF**. Measured: **(a) no layout lever reduces the heavy‑MV refresh `records_read`** (~400M in every arm — `trades_history` reads 134M to write 230 regardless); MV‑table layout does not prune the refresh — the amplification is openivm‑compile‑side. (b) out‑of‑band compaction cuts `files_scanned` (real, ~6% B2 wall) but not reads. (c) liquid clustering's −11% B2 is not read‑backed (locality/noise) + a +10% CREATE penalty (net ≈ even). (d) ⛔ **out‑of‑band ZORDER REJECTED + removed** — `OPTIMIZE ZORDER` re‑sorts the full table each tick → **1570M maint reads/batch vs LC's 22M (~70×)**; liquid clustering strictly better. (e) `delta.targetFileSize`/`tuneFileSizesForRewrites` removed (Databricks‑only, OSS‑invalid). **Net: Delta layout is a marginal lever here; the real bottleneck is refresh‑SQL read amplification (→ P5.2 / W7.7).**
