...
- [ ] **⭐ P5.2a — Persisted per‑partition aux‑state for the running‑window fast path (the confirmed P5.2 beat‑vanilla lever).** _Target: `daily_market` openivm B2 74 s / 20.5 M reads → ≈ vanilla 4 s / O(delta)._ **SF3 telemetry (2026‑07‑01) proved P5.2's residual cost is the full‑MV `bounds`/`state` scans + the full‑source `openivm_new` recompute.** **openivm‑side foundation DONE + native‑validated + pushed (openivm `79844ab` on `wt/p52-auxstate`):** emits a persisted `openivm_aux_<view>` table (1 row/partition: `openivm_aux_max_order` + last running value per window expr), backfilled on first refresh (`CREATE TABLE IF NOT EXISTS … AS ROW_NUMBER()=1 last row`), and `bounds` (`LEFT JOIN openivm_aux … m.openivm_aux_max_order`) + `state` (`FROM openivm_aux …`) now read it instead of scanning `openivm_data_<view>`; maintained each refresh via `openivm_run_aux_update_<view>` (fast rows ∪ fallback recompute) → `DELETE … WHERE EXISTS` + `INSERT`. Native tests green (auxstate 32 assertions; baseline 12 / incremental 21 / cascade 26 unchanged — no regression; gated behind `openivm_running_window_incremental`, flag‑off byte‑identical). **spark‑ext integration IN PROGRESS (`wt/p52-auxstate-sparkext`):** map `openivm_aux_<view>` → a STABLE per‑MV Delta path (`<warehouse>/_ivm/aux_state/<mv>`, persisted across refreshes, cleaned on DROP), the `DELETE … WHERE EXISTS` → Delta MERGE, and the `openivm_run_aux_update`/`openivm_run_fast_rows` temps → cached version‑pinned temp views; correctness‑validated via `WindowRunningIncrementalSpec` (multi‑refresh, exercises aux persistence). **THEN:** bump the openivm pin to `79844ab`, rebuild image, SF3/SF10 measure `daily_market` `records_read` (gate: → O(delta); `fact_market_history` stays 38×; EXCEPT‑ALL green; no MV → FULL_REFRESH). Building‑block bounds‑filter in `3c2f942`; foundation shipped default‑off (`ebe659b` + openivm `1fd2528`).
