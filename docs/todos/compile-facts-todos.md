...
- [ ] **P5.2 — Running‑aggregate auxiliary state (openivm‑C++).** _Target: the ~99 s WINDOW full‑partition recompute — the #1 residual cost, and the ONLY un‑disproven lever against it._ **STATUS 2026‑06‑30: FOUNDATION BUILT + VALIDATED END‑TO‑END, but it fires on ZERO real SF10 windows yet (extensions needed).** The full chain works and is correct: openivm‑side suffix‑extend emission (fork `wt/p52-running-agg` `1e16616`: gated `running_window_incremental`, splits affected partitions into a suffix‑append fast path `delta_min>old_max` vs a per‑partition backdated full‑recompute fallback; Spark‑portable, no `EXCLUDE`/`::`) → openivm‑spark flag wiring (`WorkloadFacts.runningWindowIncremental`, `FeatureGate.WindowRunningIncrementalEnabledKey`, `MaterializedViewCommands` sets it per‑batch at the refresh‑time compile, narrowly: flag ∧ WINDOW_PARTITION ∧ proven‑insert‑only batch) → `SparkRefreshRewriter` translation (`RunningWindowTempCreate/FastInsert/TempDrop`) → correct Spark/Delta exec. **Validated:** `WindowRunningIncrementalSpec`+`CdfSpec` 3/3 each (suffix‑append + backdated‑fallback + sequential, bidirectional `EXCEPT ALL`, MV stays WINDOW_PARTITION); `FeatureGateSpec`/`WorkloadFactsSpec`/`WindowPartitionSpec`/`SparkRefreshRewriterSpec` green (openivm‑spark `a209878`). **BUT — empirically, P5.2 DECLINES on every real SF10 window** (tested through the `1e16616` compile): `daily_market` is **window‑over‑window** (cumulative `min/max` CTE → CASE flag columns → cumulative `max` over the flags); `trades` is `min/max` with **no `ORDER BY`** (not cumulative); `dim_customer` is `last_value`; `trades_history` is `lag/row_number`. The first increment only handles a SINGLE‑LEVEL cumulative `SUM/MIN/MAX/COUNT/AVG` with simple column args — so **running SF10 now yields no win.** **NEXT increments (each substantial; needed to actually move SF10 / beat vanilla):** (a) **window‑over‑window** support → `daily_market` (24 s) — _in progress_; (b) **`lag`/`row_number`** → `trades_history` (33 s, biggest, cleanest suffix math: `rn = prior_count + offset`); (c) **`last_value` forward‑fill** → `dim_customer` (18 s); `trades` (unordered, 17 s) is NOT cheaply suffix‑extendable. Beat‑vanilla needs ~most of the 92 s window cost covered (then B2 ≈ 202 < vanilla 222) **and** the suffix‑append assumption to hold per shape at SF10 — i.e. several validated extension cycles, not one. **Build/validation loop (now fast, no image rebuild):** build openivm with `GEN=ninja make`; stage the `duckdb`+`openivm.duckdb_extension` under `spark-ext/target/` and run any parity spec via `docker compose run -e OPENIVM_CLI_PATH=… -e OPENIVM_EXTENSION_PATH=… build sbt 'ivmIt/testOnly …'`.
