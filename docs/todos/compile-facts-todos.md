...
- [ ] **⭐ P5.2a — Persisted per‑partition aux‑state for the running‑window fast path (the confirmed P5.2 beat‑vanilla lever).** _Target: `daily_market` openivm B2 74 s / 20.5 M reads → ≈ vanilla 4 s / O(delta)._ **SF3 telemetry (2026‑07‑01) proved P5.2's residual cost is the full‑MV `bounds`/`state` scans + the full‑source `openivm_new` recompute.** Fix, two parts: (1) maintain an openivm‑managed per‑partition aux‑state Delta table (1 row/partition: `{last running value per window expr, max order}`), populated at initial load and UPDATE'd by each suffix refresh, so `bounds`/`state` read O(partitions) instead of scanning the MV — building‑block bounds‑filter already in `3c2f942`; (2) scope/skip the fallback recompute (`openivm_old`/`openivm_new` + the DELETE+INSERT) so a strict‑suffix (empty‑fallback) batch does **not** trigger a full‑source window recompute. Files: openivm `refresh_compiler_aux.cpp` (`BuildRunningWindowSuffixRefreshSQL` + a new aux‑state CREATE/maintain), `SparkRefreshRewriter` (persisted aux‑state Delta table handling), `MaterializedViewCommands` (aux‑state lifecycle on CREATE/DROP). Gate: SF3/SF10 `daily_market` `records_read` ≈ O(delta) + EXCEPT‑ALL green + `fact_market_history` stays 38× + no MV → FULL_REFRESH. Foundation SHIPPED default‑off (`7e4c8d8` + openivm `1fd2528`).
