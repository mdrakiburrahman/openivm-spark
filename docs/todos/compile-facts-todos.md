...
> - **W5:** ⚠️ P5.1 _(window‑suffix‑skip — no‑op at SF10)_ 🔭 P5.2 ⚠️ P5.3 _(window‑partition‑prune — FATAL; use clustering, **W7.2**)_ 🔭 P5.4 ✅ P5.5 🔭 P5.6 🔭 P5.7
...
- [ ] **W7.2 — WINDOW recompute via Delta liquid clustering + data‑skipping.** _Target: ~99 s → ~20 s of batch‑2 (the single biggest lever)._ `CLUSTER BY` the window key on the MV data table + push the delta key min/max as a skipping predicate into the recompute CTE. **Do NOT use Hive `PARTITIONED BY` (that's P5.3, FATAL).** Files: spark‑ext window assembler, MV DDL, Delta `filesForScan`. Gate: EXCEPT‑ALL green on all `*Window*`/`*ChainedWindow*`/`*DucklakeWindow*` specs **and** SF10 B1 not regressed.
- [ ] **W7.1 — Compile‑classification cache, lifecycle‑separated.** _Target: shrink the 4–5× batch‑1 overhead + ~36 s batch‑2 fixed cost, on every MV._ Cache compiled SQL per `(view, schema‑fingerprint, tier)`; **recreate the per‑refresh snapshot temp views each refresh.** Files: `MvCatalog`, `OpenIvmCompiler`, `MaterializedViewCommands`. Gate: the exact `*PipelineCascade*`/`*ChainedAggregate*` specs that P1.5 broke must stay green.
- [ ] **W7.6 — NOOP / empty‑delta per‑MV fast‑exit.** _Target: remove per‑MV overhead for unchanged MVs each batch (S effort, quick win)._ Skip classify/index/system‑table/lock work when P1.0 says the MV's delta is empty. Files: `MaterializedViewCommands` entry, `CdfWatermarkCatalog`. Gate: golden‑SQL net + watermark correctness.

**TIER 2 — finish the structural pruning already half‑shipped:**

- [ ] **W7.3 — Make FK term‑pruning compose with `semiJoinPrune`.** _Target: unlock the 2^N−1→1 star‑join prune **on top of** the shipped win, without the 274→412 s regression._ Port `DetectFKRelations`/`ComputeSkipBits` to the `SparkRefreshRewriter` side, **or** teach `semiJoinPrune` the FK‑pruned shape. Gate: SF10 B2 **< 274 s** (must beat the semiJoinPrune‑only number) + EXCEPT‑ALL green.
- [ ] **W7.4 — Zero‑shuffle `Δ ⋈ current‑base` (co‑bucketing + bloom).** _Target: the ~84 s range‑join residual after P4.2 (watches 30 s, dim_trade 23 s)._ Co‑bucket MV + staging on the join key; lower the bloom scan‑size threshold. Files: MV/staging DDL, refresh confs. Gate: SF10 B2 drop + EXCEPT‑ALL green.
- [ ] **W7.5 — `fact_holdings` via Delta `enableRowTracking`.** _Target: convert a genuine `FULL_REFRESH` to incremental._ Map openivm's virtual `rowid` to Delta stable row‑ids so the self‑join delete applies. Files: `SparkRefreshRewriter` `referencesRowid` sites, MV DDL. Gate: `MultiSourceScd2SelfJoinFactHoldings*` EXCEPT‑ALL green; classifier no longer emits `simple_projection_no_apply`.
