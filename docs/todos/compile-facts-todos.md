...
> **Mandate.** Make incremental view maintenance (IVM) on Spark 3.5 / Delta Lake 3.2 via openivm **dramatically faster than full refresh**, by (1) hooking spark‑ext into _far more_ Spark/Delta metadata and transmitting it to openivm, and (2) tuning openivm _as hard as possible_ for the actual workload — **detected per refresh from the Delta transaction log, never assumed.** Code changes are permitted in every repo under `.temp` **except `delta` and `spark`**, plus `spark-ext`.
>
> **Design principle — translate, don't assume.** Never _assume_ an append‑only workload and flipped openivm into insert‑only fast‑path mode on faith. That is unsafe: a single DELETE/UPDATE/MERGE batch makes insert‑only fast‑path SQL **wrong**, and no‑op commits (OPTIMIZE/VACUUM) trigger wasteful refreshes. Instead, the extension becomes an **intelligent Delta→openivm translator bridge** (Workstream 1): it reads the actual commits since the last processed transaction, classifies the batch shape, and selects the _precisely‑correct‑and‑fastest_ openivm `CompileFacts` tier. Append‑only is then the _common, detected_ case — not a global assumption.
>
...
> The original roadmap below was authored from source‑reading _before_ anything was built or benchmarked. It has
...
>   batch‑2)** and (b) **per‑MV fixed overhead** (openivm batch‑1 is **4–5× vanilla** on _identical_ full‑refresh work).
...
> vanilla full‑refresh**. Beating vanilla outright is now a _narrow, well‑identified_ target (windows + overhead),
...
1. **openivm‑spark only invokes openivm's _compiler_, never its _runtime_.** It calls `openivm_compile_with_facts(view, facts_json)` in an ephemeral `duckdb :memory:` subprocess with **empty tables**, gets back Spark‑dialect SQL, and executes that SQL on Spark itself.[^bridge][^lpts] openivm's FK pruning, insert‑only fast‑path detection, empty‑delta term skipping, and adaptive cost model **all live on a different code path** (the DML‑time optimizer rules `IncrementalRewriteRule` → `CompileJoinDelta` → `BuildInclusionExclusionTerms`) that `openivm_compile_with_facts` _never reaches_.[^plumbing]

2. **The single flag the bridge hardcodes — `compile_only=true` — actively _disables_ every append‑only optimization.** It forces an early return in `ResolveDeltaFastPathFlags` that zeroes `insert_only`, `skip_agg_delete`, `skip_proj_delete`, and `minmax_incremental`, so the emitted SQL is valid for _any_ delta shape (the most expensive form).[^plumbing][^inputsurface] The bridge's lone `SET openivm_minmax_incremental=false` is **provably inert** — it is read _after_ that early return.[^plumbing]

3. **~~The emitted SQL full‑scans base tables and Spark is told not to broadcast.~~ → PARTLY FIXED + RE‑MEASURED.** The Möbius "current‑base" join terms (`Δfact ⋈ dim_now`, `fact_now ⋈ Δdim`) full‑scan the complete Delta tables; broadcast is disabled (`autoBroadcastJoinThreshold=-1`) for the heavy statements. **Original claim: `gold.fact_market_history` = 977.8 s (29× vs DuckDB).** **This is now OBSOLETE:** P1.1's insert‑only fast path took `fact_market_history` to **6.7 s (38×)**, and P4.2 `semiJoinPrune` injects the `WHERE <fk> IN (SELECT <key> FROM Δsrc)` pre‑filter that prunes the `FULL_SOURCE ⋈ Δdim` term for the rest of the SCD‑2 fact/dim tail. **What remains:** the `Δ ⋈ current‑base` _residual_ still costs ~30 s (`silver.watches`) / ~23 s (`gold.dim_trade`) at SF10 even after the prune — finishing that needs co‑bucketing + runtime bloom on the join key (W7.4), not broadcast config (P4.5/P4.6 were measured **neutral** on `local[*]`).[^cdf][^bench]

4. **~~The hard query shapes recompute the entire partition.~~ → THIS IS NOW THE #1 BOTTLENECK (and the tried fixes FAILED).** WINDOW_PARTITION MVs (`silver.trades_history` 33 s, `silver.daily_market` 24 s, `gold.dim_customer` 18 s, `silver.trades` 17 s ≈ **99 s of batch‑2**) do a partition‑scoped DELETE + recompute that still **scans the full source** because neither MV nor source is physically clustered by the window key. **Two "obvious" fixes were built and measured NEGATIVE:** the append‑only **window‑suffix‑skip is now implemented but is a no‑op at SF10** (TPC‑DI window deltas update _existing_ partitions, not a strict‑increasing suffix), and **Hive `PARTITIONED BY` on the window key (P5.3) is FATAL** (3× slower batch‑1 + EXCEPT‑ALL validation crash from small‑file skew). The live lever is **Delta liquid clustering / data‑skipping on the window key (W7.2)** or running‑aggregate aux state (P5.2). SCD‑2 `BETWEEN` cross‑product is addressed by P4.3 (`scd2RangeAccel`, shipped, default‑on); global ranking still `FULL_REFRESH`.[^window]
...
**Non‑negotiable invariant (the user's hard rule):** _no project may regress an MV's ability to incrementalize down to `FULL_REFRESH`._ Workstream 0 builds the golden‑SQL regression net that enforces this for everyone.[^testconv] One subtlety the translator makes explicit (§5, Workstream 1): choosing `FULL_REFRESH` for the _specific batch_ where the Delta commit is an `OVERWRITE`/`TRUNCATE`/`REPLACE` is **required correctness, not a regression** — the MV remains incremental for every subsequent append batch; the golden‑SQL net still guards the append/default path against silent downgrades.
...
> **⭐ 2026‑06‑30 — PER‑MV TELEMETRY (ivm‑bench `metrics_by_model.parquet`, Spark event‑log, SF3) is the new ground
> truth.** Batch wall‑clock has ±15% noise that hid the real bottleneck; the per‑MV `records_read` / `records_written`
> are deterministic and decisive. The B2 map (spark = vanilla full‑refresh, spark‑openivm = incremental):
>
> | MV | vanilla read / wall | openivm read / **write** / wall | verdict |
> |---|---|---|---|
> | `fact_market_history` | 1.34M / 33.4 s | 115K / 115K / **15.7 s** | ✅ openivm **WINS** (insert‑only fast path) |
> | `daily_market` (window) | 1.28M / 4.1 s | **7.72M** / **52K** / **34.4 s** | ❌ reads **6× vanilla** |
> | `dim_customer` (window) | 21K / 6.4 s | **202K** / – / **51.9 s** | ❌ **worst window** (`last_value`) |
>
> **The window MVs are the openivm bottleneck and they READ 6–10× vanilla.** Crucially, `daily_market` **writes only
> 52K rows (= O(delta))** — so the **P5.2 running‑window fast path DOES fire and the suffix‑append assumption HOLDS** —
> but it **reads 7.72M because its `bounds` (`MAX(order) … GROUP BY` _all_ partitions) and `state` (`ROW_NUMBER … FROM
> openivm_data`) scan the FULL MV**. The write win is negated by the read cost. **Proper fix: a persisted per‑partition
> aux‑state table** (1 row/partition: last running values + max order), read in O(partitions) — clustering won't help
> because a TPC‑DI batch touches _most_ symbols. See P5.2 in THE TODO LIST.

> **⭐⭐ 2026‑06‑30 — THE BEAT‑VANILLA STRATEGY (telemetry‑discovered).** Per‑execution telemetry showed the 7.72M is
> **statement 491 — a Delta MERGE scanning the full (one‑file) MV ~5×** (match‑scan + rewrite + deletion‑vectors +
> bounds + state/cascade) to write only 52K. **Clustering does NOT prune it** (re‑measured: 7.24M — the batch touches
> _most_ symbols). The decisive observation: **vanilla recomputes `daily_market` in 4.1 s / 1.28M reads with ONE clean
> CTAS; openivm's 5‑pass incremental MERGE takes 34 s / 7.2M.** For a window MV where _most_ partitions change every
> batch, **the incremental MERGE is ~8× slower than a clean full‑refresh.** ⇒ **The fix is cost‑based routing: when a
> WINDOW_PARTITION MV's affected‑fraction is high, FULL‑REFRESH it (single‑pass CTAS ≈ vanilla speed) instead of the
> 5‑pass partition‑scoped MERGE.** Then the windows _match_ vanilla while `fact_market_history` (38×) etc. _beat_ it →
> openivm total **beats vanilla**. (Cascade caveat: a full‑refreshed window MV that feeds a downstream MV must also
> full‑refresh that downstream, or emit a view‑delta — but daily_market's downstream is a small gold aggregate.) This
> is more promising than P5.2 suffix‑extend, which falls back on TPC‑DI's backdated window batches anyway. See the TODO.



### ✅ SHIPPED + VALIDATED (full verify green, EXCEPT‑ALL green, SF10‑measured) — _do not re‑implement_
...
| **P5.1** window‑suffix‑skip (for cascade producers)                   | "append‑only windows only need the new suffix"        | **No‑op at SF10** — TPC‑DI window batches update _existing_ partitions, not a strict‑ASC suffix, so the fast path never fires; only adds probe overhead. Window MV times unchanged (daily_market 24 s, etc.). Default‑off.                                                                                                                                              |
| **P5.3** window‑partition‑prune (Hive `PARTITIONED BY` on window key) | "partition the MV so `WHERE k IN (...)` prunes files" | **FATAL** — partitioned WINDOW data tables made batch‑1 **3× slower** _and_ crashed the EXCEPT‑ALL validation (`diff=-1`) from small‑file/partition skew. Default‑off. Use **liquid clustering** instead (→ W7.2).                                                                                                                                                      |
| **P2.1** openivm‑side FK term‑pruning (C++)                           | "prune 2^N−1 join terms to 1 via declared FKs"        | **Correct but a net regression** — it _changes the compiled SQL shape_, so openivm‑spark's `semiJoinPrune` (P4.2) rewriter **no longer pattern‑matches** and stops firing → loses the bigger win (**B2 274 → 412 s**). The two pruners **conflict**. Built + unit‑test‑green (19 assertions, openivm `0a7f3e4`) but **pin reverted**. → must be made to compose (W7.3). |
...
| **W7.3** FK term‑prune ported to `SparkRefreshRewriter` ("composes")  | "run FK‑term‑prune _before_ `semiJoinPrune` to preserve its shape" | **Still a regression at SF10** — even rewriter‑side and ordered before P4.2, it took **B2 274 → 393 s, B3 261 → 478 s** (ratio 1.75× / 2.21×) while EXCEPT‑ALL stayed green. The "compose" hypothesis (W7.3) **did not hold**: FK‑term‑pruning the view‑delta still changes the join shape enough that `semiJoinPrune` mis‑fires on the SCD‑2 range‑join tail. Same failure class as P2.1. Code kept, **default‑off, DO‑NOT‑RETRY as built** (pin `dba4ada`, run `sf10-b2b`). |
| **W7.2** window via Delta **liquid clustering** (`CLUSTER BY` key)    | "cluster the WINDOW MV so `WHERE k IN (...)` prunes files — the #1 lever" | **Exec‑metric win that does NOT reproduce on wall.** It _fires_ + is _correct_ (EXCEPT‑ALL green; B1 not inflated, unlike P5.3) and the profiler `execute_refresh_sql` dropped **297 → 185 s** with window MVs vanishing (trades_history 33 → gone). **But wall never beat the validated baseline:** windowClusterPrune+cache+noop = B2 293 s (ratio 1.13×), windowClusterPrune‑only = B2 **406 s** (ratio 1.80×) — both ≥ the 274 s of `semiJoinPrune`+`scd2` alone. The Delta liquid‑clustering write/commit maintenance during the incremental DELETE+INSERT eats the file‑pruning read savings (the 185 s exec excludes Delta commit time). **Default‑off; not a wall‑clock win at SF10.** (runs `sf10-batch1`, `sf10-win`.) |
...
> **2026‑06‑30 (update after the W7 SF10 fleet): the window cost is genuinely hard — all FOUR direct attacks on it
> have now failed at SF10.** P5.1 suffix‑skip (no‑op), P5.3 Hive partitioning (FATAL), **W7.2 liquid clustering
> (exec‑win that doesn't reach wall — Delta clustering write overhead)**, and W7.3 FK‑prune‑composes (regression,
> like P2.1). The shipped, reproducible result is **`semiJoinPrune` + `scd2RangeAccel` (1.23× / 1.21×)** and it stands.
> **Beating vanilla outright now requires a _fundamentally different_ approach to the WINDOW recompute — running‑
...
> (`watches`, `dim_trade`) is pruned by P4.2/P4.3. The _current_ per‑MV SF10 attribution and the live bottleneck
> (WINDOW recompute ~99 s) are in **§1.5** and the [^sf10] footnote.

| MV                         | RefreshType       | spark‑openivm                          | duckdb‑openivm | gap               |
| -------------------------- | ----------------- | -------------------------------------- | -------------- | ----------------- |
| `gold.fact_market_history` | SIMPLE_PROJECTION | **977.8 s** _(→ 6.7 s, FIXED by P1.1)_ | 33.7 s         | **29×** _(→ ~1×)_ |
...
The benchmark _happens_ to be append‑only, but the production goal is a system that is **fast when it can be and always correct**. The extension already persists, per `(MV, source)`, the **last‑processed Delta version** in a RocksDB `cdf_watermarks` column family, and seeds `_ivm_watermark:<src>` = `v:<version>`.[^watermark] What it does _not_ do is look at _what those new commits were_. The translator (Workstream 1) closes that gap:
...
**Exploit (B):** the things the _generated refresh SQL_ can leverage but currently does not:[^sparkstats]
...
| AQE runtime rule                  | `injectRuntimeOptimizerRule`                            | Read _actual_ materialized delta size, then decide broadcast/coalesce |
...
The proposed **`WorkloadFacts` v2** is the backbone artifact that every metadata project writes into and openivm consumes. It is intentionally a _superset_ — fields are optional and forward‑compatible (openivm already silently ignores unknown keys[^inputsurface]).
...
Each project carries: **ID**, goal, the **CS/database technique** (so it is not shallow), files to touch, dependencies, **relative effort** (T‑shirt size S/M/L/XL, or _Spike_ for research — per owning squad, sized for parallel execution, **not** a calendar duration), and the expected win. The **anti‑regression** rule from §1 applies throughout and is enforced by P0.3.
...
> - **W1:** ✅ P1.0 ✅ P1.1 ✅ P1.2 ✅ P1.3 ✅ P1.4 ⚠️ P1.5 _(SQL cache broke cascades — redo as **W7.1**)_
> - **W2:** 🔭 P2.1 _(openivm‑side FK‑prune **conflicts with P4.2** — redo as **W7.3**)_ ✅ P2.2 🔭 P2.3 ✅ P2.4
> - **W3:** ✅ P3.1 🔭 P3.2 🔭 P3.3 ⚠️ P3.4 _(plan cache — same cascade bug as P1.5 → **W7.1**)_
> - **W4:** ✅ P4.1 ✅ **P4.2 (semiJoinPrune — DEFAULT‑ON win)** ✅ **P4.3 (scd2RangeAccel — DEFAULT‑ON win)** ✅ P4.4 🧊 P4.5 🧊 P4.6 _(neutral on `local[_]`; finish the bucketing half as **W7.4**)\*
> - **W5:** ⚠️ P5.1 _(window‑suffix‑skip — no‑op at SF10)_ ⭐ **P5.2 _(running‑aggregate aux state — now the #1 beat‑vanilla lever; all rewrite/clustering attacks on WINDOW failed)_** ⚠️ P5.3 _(window‑partition‑prune — FATAL)_ 🔭 P5.4 ✅ P5.5 🔭 P5.6 🔭 P5.7
> - **W6:** 🔭 P6.1 🔭 P6.2 🔭 P6.3 🔭 P6.4 (all still open; architecture spikes)
> - **W7 (measurement‑grounded; SF10‑tested 2026‑06‑30):** ✅‑code/⚠️‑perf W7.1 cache _(implemented, default‑off; no reproducible wall win)_ · ⚠️ **W7.2 window liquid clustering** _(fires + correct, but exec‑win ≠ wall‑win — clustering write overhead; default‑off)_ · ⚠️ W7.3 FK‑prune‑composes _(still regresses B2 274→393 — like P2.1; default‑off)_ · 🔭 W7.4 zero‑shuffle Δ⋈base _(bucketing half unbuilt)_ · 🧊 W7.5 `fact_holdings` row‑tracking _(BLOCKED — Delta 3.2 `_metadata.row_id` unusable in MERGE‑ON/DELETE‑subquery)_ · ✅‑code/⚠️‑perf W7.6 NOOP fast‑exit _(implemented, default‑off)_ · 🔭 W7.7 cascade‑delta minimization
> - **➡️ The remaining beat‑vanilla lever is P5.2 (running‑aggregate aux state, XL openivm‑C++) — every rewrite/clustering/config attack on the WINDOW recompute has now been measured negative at SF10.**

### Workstream 0 — Foundations & Observability _(unblocks everything; staff first and heavily)_

| ID       | Project                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          | Technique / depth                              | Key files                                                                                                                                                                                              | Deps | Effort | Win                                                                          |
| -------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ---- | ------ | ---------------------------------------------------------------------------- |
| **P0.1** | **Per‑statement refresh profiler + emitted‑SQL capture.** Time each rewritten statement; attribute wall‑clock to view‑delta CTAS vs DELETE vs INSERT; persist the _actual_ SQL + Spark physical plan (`EXPLAIN FORMATTED`) per refresh.                                                                                                                                                                                                                                                                                                                                                                                          | Workload telemetry; flame‑attribution          | `MaterializedViewCommands` exec loop; extend existing `SHOW OPENIVM REFRESH PROFILE`/`QUERY LOG`[^bench]                                                                                               | —    | S      | Turns guesswork into data; ranks MVs by cost                                 |
...
### Workstream 1 — The Delta→openivm Workload Translator + Append‑Only Tuning _(fastest ROI; "detect, don't assume")_
...
| **P1.3** | **Per‑source delta shape into the compile facts.** Thread classifier `delta_shape: {table → INSERT_ONLY/UNCHANGED/GENERAL}` so one‑sided‑change joins and FK pruning fire even when _some_ sources see deletes.                                                                                                                                                                                                                                                                                                                                                                                                                                                                      | Per‑relation Z‑set sign analysis                           | `WorkloadFacts`; openivm fast‑path resolver                                                                                                      | P1.0, P2.1       | S      | Enables star‑schema one‑sided pruning under mixed workloads                            |
...
> **P1.1‑PRE — RESOLVED + REASSESSED (empirical, 2026‑06‑28).** _(we own openivm/openivm‑spark — the fork was fixed; the finding re‑scopes P1.1)_
>
...
### Workstream 2 — Constraints & FK‑Aware Pruning _(unlocks star‑schema joins; deeper because FK pruning is DML‑only today)_
...
### Workstream 3 — Statistics‑Driven Compilation _(brings CBO rigor to openivm)_
...
| **P3.2** | **Compile‑time cost model.** Give `GenerateRefreshSQL` a cost estimator (port openivm's `EstimateRefreshCost` formulas) fed by injected stats, choosing: incremental vs full, GROUP_RECOMPUTE vs CURRENT_DIFF, join‑term order. _Guardrail: never select FULL when an incremental path is sound (P0.3)._ | Cost‑based optimization; cardinality estimation | `openivm/src/upsert/refresh_cost_model.cpp` (decouple from runtime tables); cost_model.md[^costmodel][^engine] | P3.1, P2.1 | XL     | Right strategy per MV, deterministically, without live probes |
...
### Workstream 4 — Spark‑Native Execution of Refresh SQL _(mostly SHIPPED — P4.2/P4.3 are the −32% win; P4.5/P4.6 measured neutral on `local[_]`)\*

| ID       | Project                                                                                                                                                                                                                                                                                                                                              | Technique / depth                                               | Key files                                                                                                                 | Deps       | Effort | Win                                                                   |
| -------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------- | ---------- | ------ | --------------------------------------------------------------------- |
| **P4.1** | **Selective broadcast injection.** Replace the blunt `autoBroadcastJoinThreshold=-1` with _targeted_ `/*+ BROADCAST(dim) */` hints (or a raised threshold) only for join sides whose extracted `size_bytes` is provably small, so the 8 GiB cap can't trip. Inject via the `LptsSparkDialect`/`postProcess` hook (an established extension point).   | Cost‑guided physical‑join selection                             | `SparkRefreshRewriter` `postProcess`; `MaterializedViewCommands` broadcast‑disable sites; sizes from P0.4[^cdf][^lptsext] | P0.4       | M      | Directly attacks the shuffle/sort‑merge cost in the 977 s MV          |
| **P4.2** | **Kill the full‑base‑table scan via key/partition pre‑pruning.** Before the Möbius "current‑base" join, restrict the base scan to the delta's key domain and partition values (push a semi‑join / `IN (partition values)` _through_ the rewriter, using Delta `partitionValues` + min/max skipping), so `fact_now ⋈ Δdim` reads only relevant files. | Sideways information passing; data skipping; DPP                | `SparkRefreshRewriter` rewrite passes; Delta `filesForScan`/min‑max[^cdf][^deltameta]                                     | P0.4, P4.1 | XL     | Converts O(full table) scans to O(affected) — the core structural fix |
...
### Workstream 5 — Deep IVM Algorithmics _(the hardcore CS; lifts the ceiling on pathological shapes)_

| ID       | Project                                                                                                                                                                                                                                                                                                                                                   | Technique / depth                                         | Key files                                                                                                                       | Deps       | Effort | Win                                                             |
| -------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------- | ---------- | ------ | --------------------------------------------------------------- |
| **P5.1** | **Append‑only window suffix skip (Rule 6).** Implement the documented‑but‑absent rule: for backward‑looking frames, if every delta key's order value exceeds the partition's current `MAX(order_col)`, _append only the suffix_ — no DELETE, no full‑partition recompute. Correctness: prior rows' backward frames are unchanged.                         | Monotone window incrementalization                        | `openivm/src/upsert/refresh_window.cpp` (new branch before `CompileWindowRecompute`); `delta-skipping-rules.md` Rule 6[^window] | P1.1       | L      | O(partition)→O(delta): ~1,260× per key on `silver.daily_market` |
...
| **P5.6** | **Cascade‑delta minimization.** Make an upstream window/SCD MV emit only _truly changed_ rows to downstream (not the whole recomputed partition), so `gold.fact_market_history` receives O(new rows), not O(K·P̄).                                                                                                                                         | Signed‑multiset diffing; change minimization              | `openivm refresh_compiler_aux.cpp` cascade emit; companion‑rows[^window][^optcat]                                               | P5.1/P5.2  | L      | Breaks the partition→downstream amplification chain             |
...
### Workstream 6 — Architectural Unification _(deepest dependencies, highest ceiling)_
...
### Workstream 7 — Post‑Empirical High‑Value Levers _(NEW, 2026‑06‑30 — every item is pointed at a MEASURED SF10 cost; this is where the next real wins are)_
...
> **⭐ UPDATE 2026‑06‑30: W7.2 was the _hypothesized_ critical‑path lever; SF10 disproved it.** Liquid clustering
> fires and is correct, but its exec‑metric win (297→185 s) does **not** reach wall clock (B2 stayed ≥ 274 s) — the
> Delta clustering write/commit maintenance offsets the read pruning. **The real remaining beat‑vanilla lever is
> P5.2 (running‑aggregate auxiliary state, openivm‑C++).** The dashed `redo` edges mark where a W7 item _supersedes_
> a measured dead‑end (P1.5→W7.1, P5.1/P5.3→W7.2, P2.1→W7.3) — but note W7.2 and W7.3 were themselves then measured
> negative at SF10 (§1.5). See §1.5 + THE TODO LIST.
...
> always: _never regress to `FULL_REFRESH`; bidirectional `EXCEPT ALL` green; full verify green; validate a flag at
> SF10 before default‑on._ Current standing: openivm SF10 **B2/B3 = 274/261 s vs vanilla 220/220 s (1.23×/1.21×)** —
> the goal is to get **< 1.0×**.

### ⛔ DO NOT RETRY — proven dead ends (each already cost a ~70‑min SF10 cycle; evidence in §1.5)

- [x] ~~Per‑tier compiled‑SQL cache as built (P1.5 / P3.4)~~ — broke MV‑over‑MV cascades. _Redo only as **W7.1** (lifecycle‑separated)._
- [x] ~~Window‑suffix‑skip for the SF10 windows (P5.1 default‑on)~~ — no‑op; TPC‑DI windows aren't strict‑ASC suffixes.
- [x] ~~Window‑partition‑prune via Hive `PARTITIONED BY` (P5.3)~~ — FATAL: 3× slower B1 + EXCEPT‑ALL crash. _Use clustering (**W7.2**)._
- [x] ~~openivm‑side FK term‑pruning stacked under `semiJoinPrune` (P2.1 as‑is)~~ — regresses B2 274→412 s. _Make it compose (**W7.3**)._
- [x] ~~Broadcast‑threshold / runtime‑bloom config tuning for the SF10 number (P4.5 / P4.6 bloom half)~~ — neutral on `local[*]`.
- [x] ~~WINDOW recompute via Delta liquid clustering (W7.2)~~ — **fires + correct (EXCEPT‑ALL green, B1 not inflated) but exec‑win ≠ wall‑win**: profiler exec 297→185 s, but wall B2 stayed ≥ 274 s (293 s with cache/noop, **406 s** clustering‑only) — Delta clustering write/commit maintenance eats the read pruning. Code landed + default‑off. _The window cost needs P5.2, not clustering._
- [x] ~~FK term‑pruning ported to `SparkRefreshRewriter` to "compose" with `semiJoinPrune` (W7.3)~~ — **still regresses: B2 274→393, B3 261→478.** Same conflict as P2.1; pruning the view‑delta changes the shape `semiJoinPrune` matches on. Code landed + default‑off.
...
- [x] Count‑monoid `FULL_REFRESH` arity fix (the SF10 batch‑2 "hang") + the W7.1 cache‑gating follow‑up (`dba4ada`: legacy compiled‑SQL props must be written cache‑independently) · 6 cascade/regression fixes (see PR #10)
- [x] **W7.1 cache (lifecycle‑separated)** + **W7.6 NOOP fast‑exit** — _implemented, correct, integrated, default‑off._ Both are correct redos of the P1.5/P3.4 dead‑end but neither produced a **reproducible wall‑clock win** at SF10 (exec < wall ⇒ serial overhead offset). Kept opt‑in; not a default‑on win.

### 🔭 OPEN — work the top of this list first

> **Reality check (2026‑06‑30): the shipped, reproducible result is `semiJoinPrune`+`scd2RangeAccel` = 1.23×/1.21×.**
> Every _rewrite/clustering/config_ attack on the residual (W7.2, W7.3, P4.5/P4.6, P5.1, P5.3) is now measured
> negative. The only remaining path to beat vanilla is the **deep openivm‑C++ work (P5.2)** — promoted to Tier 1.

**TIER 1 — the remaining beat‑vanilla lever (deep; the cheap levers are exhausted):**

- [ ] **⭐ W‑FULL / W7.7 — Single‑pass window recompute + incremental cascade‑delta (the telemetry‑scoped beat‑vanilla rework).** _Target: openivm windows read 6–10× vanilla; close it._ **Per‑execution telemetry (SF3) decomposed `daily_market`'s 7.72M reads into ~5 full‑MV passes: ~3 for the recompute MERGE (`DELETE` affected + `INSERT` recomputed + deletion‑vectors) + ~2 for the cascade view‑delta (old/new snapshot diff).** Vanilla does the whole thing in **one CTAS (1.28M, 4.1 s)**. **Cascade is the blocker:** `daily_market` feeds `fact_market_history` (openivm's **38× win**, insert‑only) plus `daily_market_pulse`/`market_volatility`, so `daily_market` **must** emit a view‑delta — you cannot simply full‑refresh it (that would force `fact_market_history` to full‑refresh and lose the 38×). **Two coordinated fixes needed:** (1) replace the partition‑scoped DELETE+INSERT **MERGE** with a single‑pass clean recompute (`INSERT OVERWRITE` of the affected rows / full CTAS) when the affected‑fraction is high — saves the ~3 MERGE scans; (2) **make the cascade view‑delta incremental** (emit only the truly‑changed rows as ±1, not a full old/new snapshot diff) — saves the ~2 cascade scans (this is **W7.7**, now precisely justified). Together → window read ≈ vanilla → openivm total **beats vanilla** (windows match, `fact_market_history` etc. win). Files: openivm `refresh_window.cpp` / `refresh_compiler_aux.cpp` (recompute + cascade emit), openivm‑spark window dispatch. **This is the real, deep frontier — a multi‑increment openivm‑core rework, not a flag.** Gate: EXCEPT‑ALL green window + downstream; `daily_market`/`dim_customer` openivm `records_read` ≈ vanilla; `fact_market_history` stays 38×. **⚠️ Design constraint discovered 2026‑07‑01 (P5.2a aux‑state was measured a DEAD‑END — reduced `bounds`/`state` reads 20.5M→12.9M but net‑negative wall from Delta aux‑maintenance + a multi‑refresh B3 correctness bug; the recompute cost is NOT localised to bounds/state → this single‑pass rework is the only path left):** the "single‑pass DATA update" half is NOT a pure rewriter tweak — Delta 3.2 DML predicates do **not** accept subqueries (that is exactly why the partition‑scoped window `DELETE … WHERE part IN (subquery)` is already lowered to a MERGE). So `INSERT INTO delta.\`mv\` REPLACE WHERE part IN (…) SELECT * FROM openivm_new` needs the affected‑partition set as a **collected literal predicate** (an execution‑path change in `MaterializedViewCommands` — collect the small affected‑key set to the driver, build a literal `REPLACE WHERE`), or a bucketed/partitioned dynamic‑overwrite. Prototype + validate that replaceWhere path (correctness via the WINDOW parity specs, perf via SF `records_read`) BEFORE committing to the cascade‑minimization half.
- [ ] **P5.2 — Running‑aggregate auxiliary state (openivm‑C++).** _Target: the ~99 s WINDOW full‑partition recompute — the #1 residual cost._ **STATUS 2026‑07‑01 (cascade‑compose + correctness DONE; perf regression CONFIRMED → aux‑state mandatory):** P5.2 now **fires on cascade SOURCES** — `daily_market` (which feeds `fact_market_history` 38×, `daily_market_pulse`, `market_volatility`) — because the suffix fast path now **composes with the view‑delta cascade** (openivm `wt/p52-running-agg` `1fd2528`: emits `openivm_run_fast` AND the `openivm_delta_<view>` cascade even when `emit_cascade_delta=true`; gate `refresh_compiler_aux.cpp:1181` relaxed to `if (running_window_incremental)`). Two **spark‑ext rewriter** bugs fixed to make it correct end‑to‑end (openivm‑spark `7e4c8d8`): (a) the fast cascade `INSERT INTO openivm_delta_<view>` is classified `RunningWindowCascadeInsert` and **APPENDS** to the view‑delta the fallback signed‑multiset CTAS created (was a 2nd CTAS overwriting the 1st); (b) the `openivm_run_*` helpers are **materialised snapshots** — the rewriter now pins their MV reads to `VERSION AS OF <pre‑refresh>` + eager `CACHE TABLE` (lazy views re‑evaluated against the MID‑REFRESH‑mutated MV, blanking the backdated partition + corrupting the fast cascade seed). Validated: `WindowRunningIncrementalSpec`+`CdfSpec` **5/5** each (suffix, backdated‑fallback, sequential, WoW `daily_market`, **cascade‑consumer downstream EXCEPT‑ALL**); `SparkRefreshRewriterSpec` 58/58; **SF3 all 3 batches EXCEPT‑ALL green.** **SF3 per‑MV telemetry (`metrics_by_model.parquet`, first run where P5.2 actually fires):** `daily_market` openivm B2 = **74.3 s / 20.5 M reads** vs vanilla **4.3 s / 1.28 M** → **P5.2 as‑built is a NET REGRESSION** (kept **default‑off**). Root cause (exactly as predicted): `bounds` (`MAX(order) … GROUP BY part`) + `state` (`ROW_NUMBER() … FROM MV`) each **scan the full MV**, and `openivm_new` recomputes the **full‑source** window even when the fallback set is empty. **The correctness foundation is the hard unblock; the perf win now needs the persisted aux‑state (below).** `fact_market_history` stays the **38× insert‑only win** (16.3 s / 0.063 M vs 32.5 s / 1.34 M) — unaffected. ⟪Prior 2026‑06‑30 note (superseded on the “fires” claim): P5.2 appeared to decline for cascade consumers because it was gated off whenever `emit_cascade_delta=true`; the cascade‑compose fix above resolves that.⟫ Progress this session: openivm‑side suffix‑extend emission + flag wiring + `SparkRefreshRewriter` translation (openivm‑spark `a209878`/`e85ade7`, openivm `wt/p52-running-agg` `3c2f942`), all validated end‑to‑end — `WindowRunningIncrementalSpec`+`CdfSpec` 4/4 (incl. the real `daily_market` window‑over‑window: cumulative `min/max` → CASE flags → cumulative `max`), bidirectional `EXCEPT ALL`, MV stays WINDOW_PARTITION, no regressions; **SF3 all 3 batches EXCEPT‑ALL green.** **Per‑MV telemetry verdict:** `daily_market` openivm B2 **writes 52K (= O(delta))** but **reads 7.72M (6× vanilla)** because the recompute **MERGE scans the full one‑file MV ~5×** (`bounds`/`state`/match‑scan/rewrite/DVs). _Per‑execution telemetry showed this is dominated by the MERGE + cascade snapshot, not just bounds/state — see **W‑FULL/W7.7** above, which supersedes this for the high‑affected TPC‑DI window workload._ **If pursued for strict‑suffix workloads:** persisted per‑partition aux‑state table (1 row/partition, read O(partitions)); building‑block bounds‑filter already in `3c2f942`. Other windows each need their own increment: `dim_customer` (52 s, **worst**, `last_value`), `trades_history` (33 s @ SF10, `lag/row_number`); `trades` (unordered) not cheaply suffix‑extendable. **Tooling:** validate openivm changes fast via `GEN=ninja make` + stage `duckdb`/`extension` under `spark-ext/target/` + `docker compose run -e OPENIVM_CLI_PATH=… -e OPENIVM_EXTENSION_PATH=…`; measure with ivm‑bench `metrics_by_model.parquet` (`records_read`/`records_written` per MV — deterministic, decisive).
...
- [ ] **W7.4 — Zero‑shuffle `Δ ⋈ current‑base` (co‑bucketing + bloom).** _Target: the ~84 s range‑join residual after P4.2 (watches 30 s, dim_trade 23 s)._ Co‑bucket MV + staging on the join key; lower the bloom scan‑size threshold. Files: MV/staging DDL, refresh confs. Gate: SF10 B2 drop + EXCEPT‑ALL green. _(Note: P4.6's bloom‑only half was neutral on `local[*]`; the unbuilt bucketing/SPJ half is the real lever.)_
- [ ] **W7.5 — `fact_holdings` via Delta `enableRowTracking`. BLOCKED.** Delta 3.2's `_metadata.row_id` is selectable but **not usable in `MERGE ON` / `DELETE WHERE … IN (subquery)`**, so openivm's rowid‑keyed self‑join delete can't bind to it → `fact_holdings` stays `FULL_REFRESH`. Re‑open if a future Delta release lifts the restriction.

**TIER 3 — deep ceiling‑raisers (XL; openivm‑C++):**

- [x] ~~**⭐ P5.2a — Persisted per‑partition aux‑state for the running‑window fast path.**~~ **BUILT + END‑TO‑END‑CORRECTNESS‑VALIDATED, but SF3‑MEASURED A DEAD‑END (2026‑07‑01).** openivm‑side (openivm `79844ab` `wt/p52-auxstate`, native 32 assertions) + spark‑ext (openivm‑spark `5e92673` `wt/p52-auxstate-sparkext`, parity 6/6 incl. a 3‑batch aux case) both built + validated: a persisted `openivm_aux_<view>` Delta table (1 row/partition) so `bounds`/`state` read it instead of scanning the full MV. **SF3 verdict (openivm 79844ab + openivm‑spark 5e92673, windowRunningIncremental ON):** `daily_market` B2 reads **20.5 M → 12.9 M** (the bounds/state scans WERE removed) **but wall ROSE 74 → 99 s** (the per‑refresh aux Delta maintenance — `CREATE TABLE IF NOT EXISTS` backfill + `MERGE`‑delete + `INSERT` — costs more than the scan it saves), AND **B3 FAILED validation (`daily_market diff=1`)** — a multi‑refresh aux‑maintenance bug the small‑data parity specs never hit. **Reads still 10× vanilla (1.28 M)** because `openivm_new`/cascade scans remain untouched. **Conclusion: aux‑state fixes only ONE of several full scans, is net‑negative on wall (Delta overhead), and is subtly incorrect across many refreshes → NOT the beat‑vanilla lever.** Branches kept (not merged); main unaffected (aux‑state was never on the shipped pin). **The window recompute cost is not localised to bounds/state — it needs the single‑pass rework below (W‑FULL/W7.7), not incremental aux‑state.**
...
- [ ] **P3.2/P3.3** compile‑time cost model + skew‑aware fanout · **P2.3** wire `openivm_constraints_cache` _(code‑complete in openivm fork `4bbac28`; needs an openivm build/image to validate)_.
- [ ] **P6.1** unify compile/runtime intelligence · **P6.2** DuckLake‑runtime spike · **P6.3** LPTS hint codegen _(lpts fork ready, pin `db2ea07`)_ · **P6.4** cost calibration.
...
- **Isolated Docker containers per squad.** Container‑based dev (`dev.sh`, docker‑compose `build`); per‑squad image tag; forked‑test concurrency (`-Dopenivm.test.forks=N`). **Note the serial bottleneck: one SF10 run (~70 min) validates one change at a time** — fleet the _code_, serialize the _benchmark_.
...
- **Never regress to FULL_REFRESH (hard rule).** Enforced by P0.3. Every openivm emit change and every rewriter change must pass the RefreshType‑freeze + golden‑SQL hash. This is the user's most emphatic constraint. **Nuance:** the translator choosing `FULL_REFRESH` for a _specific_ `OVERWRITE`/`TRUNCATE` batch (P1.2) is required correctness — not a capability regression; the golden‑SQL net asserts the _append/default_ path is never silently downgraded.
- **`compile_only` correctness — the translator is the interlock.** `assume_insert_only` (P1.1) is set _only_ when the `DeltaCommitClassifier` (P1.0) has **proven** from the commit log that the batch is append‑only (every new commit `isBlindAppend` / AddFile‑only, no `RemoveFile` with `dataChange=true`); a single delete/update row otherwise makes the fast‑path SQL wrong. Classification is **conservative** — any unrecognized operation ⇒ `MUTATING`/`REPLACE`, never `INSERT_ONLY`.[^plumbing][^watermark]
- **Delta Kernel cannot classify commits (caveat).** Kernel 3.2.1 has no public per‑commit‑range API (only internal `ActionsIterator`/`CommitInfo`). P1.0 therefore uses Spark `DeltaLog.getChanges`/`history` (already on the classpath); Kernel is used _only_ for fast snapshot/version/stats reads (P0.4). If `DeltaLog.getChanges` accessibility is constrained from the extension package, the fallback is a direct `_delta_log/*.json` `commitInfo` parse with Jackson.[^kernel][^deltachanges]
- **Broadcast safety (P4.1/P4.5).** The existing `autoBroadcastJoinThreshold=-1` exists for a real reason — mis‑estimated SCD‑2 expansion hitting the 8 GiB cap. Replace it _only_ with size‑proven targeted hints or AQE‑runtime decisions, never a blunt global re‑enable.[^cdf]
