#!/usr/bin/env bash
# Local-Docker replica of the lpts GitHub CI for the openivm-spark fork.
#
# Lives at tools/lpts/ci/ and drives four jobs against a checkout of the lpts
# repo (default: .temp/lpts, which is gitignored). Mirrors the two reusable
# workflows lpts calls from duckdb/extension-ci-tools @ v1.5-variegata
...
#
# Logs are written under .logs/lpts-ci/<UTC-timestamp>/ (gitignored).
#
# Usage:
#   ./run-all.sh [--only build|test|format|tidy|all] [--clean] [--no-chown]
#                [--unittest <glob>] [--merge-base <ref>]
#                [--lpts-dir <path>] [-h|--help]

set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
REPO_ROOT="$(git -C "${SCRIPT_DIR}" rev-parse --show-toplevel)"
LPTS_DIR="${LPTS_DIR:-${REPO_ROOT}/.temp/lpts}"
DOCKER_DIR="${SCRIPT_DIR}/docker"
LOG_ROOT="${REPO_ROOT}/.logs/lpts-ci"
...
# Remote (in the lpts checkout) that hosts the PR base branch, used by
# --merge-base to test the merge result. lpts PR #13 targets cwida/lpts.
MERGE_BASE_REMOTE="${MERGE_BASE_REMOTE:-cwida}"
MERGE_BASE_URL="${MERGE_BASE_URL:-https://github.com/cwida/lpts.git}"

IMG_BUILD="lpts-ci-build:${CI_TOOLS_VERSION}"
IMG_FORMAT="lpts-ci-format:${CI_TOOLS_VERSION}"
IMG_TIDY="lpts-ci-tidy:${CI_TOOLS_VERSION}"

CCACHE_VOLUME="lpts-ci-ccache"
...
if [[ ! -e "${LPTS_DIR}/.git" ]]; then
    cat >&2 <<EOF
ERROR: lpts checkout not found at ${LPTS_DIR}

Bootstrap it (uses the SHA pinned in spark-ext/dev/pins.env):

    ${SCRIPT_DIR}/setup-lpts.sh

Or point this script at an existing checkout:

    ./run-all.sh --lpts-dir /path/to/lpts
    LPTS_DIR=/path/to/lpts ./run-all.sh
EOF
    exit 1
fi
[[ -d "${DOCKER_DIR}" ]] || fail "${DOCKER_DIR} missing"

CUR_BRANCH="$(cd "${LPTS_DIR}" && git rev-parse --abbrev-ref HEAD)"
CUR_SHA="$(cd "${LPTS_DIR}" && git rev-parse --short HEAD)"
log "lpts dir    : ${LPTS_DIR}"
...
bootstrap_submodules() {
    ...
    git submodule update --init -- third_party/ducklake >/dev/null
    (cd duckdb && \
    ...
    log "ducklake @ $(cd third_party/ducklake && git describe --tags --always 2>/dev/null || git rev-parse --short HEAD)"
    if [[ -f "third_party/ducklake/src/include/storage/ducklake_scan.hpp" ]]; then
        log "ducklake headers present"
    else
        log "WARN: ducklake headers missing; tidy and build will fail"
    fi
}
...
apply_merge_base() {
    ...
    cd "${LPTS_DIR}"
    if ! git remote get-url "${MERGE_BASE_REMOTE}" >/dev/null 2>&1; then
        log "adding remote ${MERGE_BASE_REMOTE} -> ${MERGE_BASE_URL}"
        git remote add "${MERGE_BASE_REMOTE}" "${MERGE_BASE_URL}"
    fi
    log "fetching ${MERGE_BASE_REMOTE}/${MERGE_BASE} for merge-result test"
    git fetch --quiet "${MERGE_BASE_REMOTE}" "${MERGE_BASE}"
    git -c user.name=ci -c user.email=ci@local merge --no-edit --no-ff "${MERGE_BASE_REMOTE}/${MERGE_BASE}" \
        || fail "merge with ${MERGE_BASE_REMOTE}/${MERGE_BASE} failed; resolve manually"
}
...
clean_build_artifacts() {
    [[ "${CLEAN}" -eq 1 ]] || return 0
    log "cleaning lpts build dirs (--clean)"
    rm -rf "${LPTS_DIR}/build" "${LPTS_DIR}/duckdb/build" 2>/dev/null || true
}
...
chown_back() {
    ...
    [[ ! -d "${LPTS_DIR}/build" && ! -d "${LPTS_DIR}/duckdb/build" ]] && return 0
    local uid gid
    uid="$(id -u)"; gid="$(id -g)"
...
case "${ONLY}" in
    ...
    test)
        if [[ ! -x "${LPTS_DIR}/build/release/test/unittest" ]]; then
